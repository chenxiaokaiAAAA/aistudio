...
- **请求参数**:
  ...
  - `machineSerialNumber` 或 `machine_serial_number` 或 `selfie_machine_id`: **设备序列号（建议必填）**
- **返回数据**（成功时 `success: true`，`order` 包含以下信息）:
  ```json
  {
    "success": true,
    "order": {
      "order_id": "MP20260101XXX",
      ...
      "payment_time": "2026-02-09T10:01:00",
      "store_name": "厦门SM商城",
  ...
  ```
  - **订单号、设备信息**：`order_number`、`store_name`、`machine_serial_number`、`machine_name` 等字段会在核销时一并返回，便于安卓APP展示完整上下文。
- **错误返回**:
  ```json
  {
    "success": false,
  ...
  ```
  - 若已拍摄过，会返回 400，且 `order` 中仍包含订单信息及 `store_name`、`machine_serial_number`、`machine_name`。
- **关键功能**:
