# 生产环境地址配置指南（未注册域名）

## 📋 需要配置的地方

由于你还没有注册域名，当前使用 IP 地址 `121.43.143.59`，需要在以下地方配置：

---

## 1. 服务器配置文件（server_config.py）

**位置**：`/root/project_code/server_config.py`

**需要修改**：

```python
# 修改第11行，将 'local' 改为 'production'
ENV = os.environ.get('SERVER_ENV', 'production')  # 改为 production

# 修改第22-27行，将域名改为IP地址
'production': {
    'base_url': 'http://121.43.143.59',  # 改为你的IP
    'api_base_url': 'http://121.43.143.59/api',
    'static_url': 'http://121.43.143.59/static',
    'media_url': 'http://121.43.143.59/media',
    'notify_url': 'http://121.43.143.59/api/payment/notify'
}
```

**在服务器上执行**：

```bash
cd /root/project_code

# 备份原文件
cp server_config.py server_config.py.bak

# 修改配置
sed -i "s/ENV = os.environ.get('SERVER_ENV', 'local')/ENV = os.environ.get('SERVER_ENV', 'production')/" server_config.py
sed -i "s|'base_url': 'https://moeart.cc'|'base_url': 'http://121.43.143.59'|" server_config.py
sed -i "s|'api_base_url': 'https://moeart.cc/api'|'api_base_url': 'http://121.43.143.59/api'|" server_config.py
sed -i "s|'static_url': 'https://moeart.cc/static'|'static_url': 'http://121.43.143.59/static'|" server_config.py
sed -i "s|'media_url': 'https://moeart.cc/media'|'media_url': 'http://121.43.143.59/media'|" server_config.py
sed -i "s|'notify_url': 'https://moeart.cc/api/payment/notify'|'notify_url': 'http://121.43.143.59/api/payment/notify'|" server_config.py

# 验证修改
grep -A 6 "'production':" server_config.py
```

---

## 2. 管理后台系统配置（数据库）

**位置**：管理后台 → 系统配置 → 服务器URL配置

**需要修改的URL**：
- **服务器基础URL**：`http://121.43.143.59`
- **API基础URL**：`http://121.43.143.59/api`
- **媒体文件URL**：`http://121.43.143.59/media`
- **静态文件URL**：`http://121.43.143.59/static`

**操作步骤**：
1. 登录管理后台：`http://121.43.143.59/admin/`
2. 进入：**系统配置** → **服务器URL配置**
3. 修改上述4个URL
4. 点击保存

---

## 3. Nginx配置（可选，如果使用域名）

**位置**：`/etc/nginx/sites-available/aistudio`

**当前配置**（使用IP，无需修改）：
```nginx
server {
    listen 80;
    server_name 121.43.143.59;  # 使用IP，无需修改
    # ...
}
```

**如果将来注册了域名**，需要修改：
```nginx
server {
    listen 80;
    server_name your-domain.com www.your-domain.com;  # 改为你的域名
    # ...
}
```

---

## 4. 环境变量（可选）

如果使用环境变量，可以在 `.env` 文件中设置：

```bash
cd /root/project_code
echo "SERVER_ENV=production" >> .env
```

---

## 🔧 一键配置脚本

在服务器上执行：

```bash
cd /root/project_code

# 备份
cp server_config.py server_config.py.bak

# 修改配置
python3 << 'PYEOF'
import re

with open('server_config.py', 'r', encoding='utf-8') as f:
    content = f.read()

# 修改环境
content = re.sub(r"ENV = os\.environ\.get\('SERVER_ENV', 'local'\)", 
                 "ENV = os.environ.get('SERVER_ENV', 'production')", content)

# 修改生产环境URL
content = re.sub(r"'base_url': 'https://moeart\.cc'", 
                 "'base_url': 'http://121.43.143.59'", content)
content = re.sub(r"'api_base_url': 'https://moeart\.cc/api'", 
                 "'api_base_url': 'http://121.43.143.59/api'", content)
content = re.sub(r"'static_url': 'https://moeart\.cc/static'", 
                 "'static_url': 'http://121.43.143.59/static'", content)
content = re.sub(r"'media_url': 'https://moeart\.cc/media'", 
                 "'media_url': 'http://121.43.143.59/media'", content)
content = re.sub(r"'notify_url': 'https://moeart\.cc/api/payment/notify'", 
                 "'notify_url': 'http://121.43.143.59/api/payment/notify'", content)

with open('server_config.py', 'w', encoding='utf-8') as f:
    f.write(content)

print("✅ 配置已更新")
PYEOF

# 验证
echo "验证配置："
grep -A 6 "'production':" server_config.py

# 重启服务
systemctl restart aistudio
echo "✅ 服务已重启"
```

---

## 📝 后续注册域名后

注册域名后，需要：

1. **修改 server_config.py**：将 IP 改为域名（HTTPS）
2. **修改管理后台配置**：更新服务器URL配置
3. **修改 Nginx 配置**：更新 `server_name` 和 SSL 证书
4. **重启服务**：`systemctl restart aistudio && systemctl restart nginx`

---

## ✅ 验证配置

配置完成后，验证：

```bash
# 检查 server_config.py
python3 << 'PYEOF'
from server_config import get_base_url, get_api_base_url, get_media_url
print(f"基础URL: {get_base_url()}")
print(f"API URL: {get_api_base_url()}")
print(f"媒体URL: {get_media_url()}")
PYEOF
```

应该输出：
```
基础URL: http://121.43.143.59
API URL: http://121.43.143.59/api
媒体URL: http://121.43.143.59/media
```
