...
### 1. 用户注册测试
```bash
curl -X POST https://moeart.cc/api/user/register \
...
### 2. 分佣测试
```bash
curl -X POST https://moeart.cc/api/test/commission \
  -H "Content-Type: application/json" \
  -d '{
    "userId": "USER1758272437351",
    "testAmount": 100
...
### 3. 获取分佣数据
```bash
curl "https://moeart.cc/api/user/commission?userId=USER1758272437351"
```
