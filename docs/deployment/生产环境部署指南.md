...
# 上传您的SSL证书
# 方式1: 使用scp从本地上传
# scp your-cert.pem user@server:/etc/nginx/ssl/moeart.cc.pem
# scp your-key.key user@server:/etc/nginx/ssl/moeart.cc.key

# 方式2: 使用Let's Encrypt（推荐）
sudo apt install certbot python3-certbot-nginx -y
sudo certbot --nginx -d moeart.cc -d www.moeart.cc
```
