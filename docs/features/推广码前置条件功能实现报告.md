...
const registerResult = await wx.request({
    url: 'https://moeart.cc/api/user/register',
...
// 订单支付成功后，更新推广资格
await wx.request({
    url: 'https://moeart.cc/api/user/update-promotion-eligibility',
    method: 'POST',
    data: { openId: userOpenId }
});
...
const checkResult = await wx.request({
    url: 'https://moeart.cc/api/user/check-promotion-eligibility',
    method: 'POST',
    data: { openId: userOpenId }
});
