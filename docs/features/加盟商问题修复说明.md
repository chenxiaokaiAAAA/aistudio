...
### 解决方案
1. **修改二维码内容**：从 `FRANCHISE:{qr_code}` 改为 `http://photogooo:8000/franchisee/scan/{qr_code}`
...
```python
# 生成二维码内容（指向扫码处理路由）
qr_content = f"http://photogooo:8000/franchisee/scan/{franchisee.qr_code}"
```
...
### 1. 加盟商登录
1. 访问：`http://photogooo:8000/franchisee/login`
2. 输入用户名：`cs001`
3. 输入密码：`123456`
4. 点击登录
