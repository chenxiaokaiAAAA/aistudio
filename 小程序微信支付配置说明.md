...
4. **配置微信商户平台**：
   - 登录微信商户平台
   - 设置支付通知URL: `https://photogooo/api/payment/notify`

## 测试接口

### 测试获取openid
```bash
curl -X POST https://photogooo/api/user/openid \
...
### 测试创建支付订单
```bash
curl -X POST https://photogooo/api/payment/create \
  -H "Content-Type: application/json" \
...
// 2. 创建支付订单
wx.request({
  url: 'https://photogooo/api/payment/create',
  method: 'POST',
  data: {
    orderId: 'ORDER123',
    totalPrice: '15.00',
