# add_shooting_completed_at_field.py
# 数据库迁移脚本：为订单表添加拍摄完成时间字段
...
def add_shooting_completed_at_field():
    """为order表添加拍摄完成时间字段"""
    ...
    with app.app_context():
        try:
            ...
            # 需要添加的字段
            field_name = 'shooting_completed_at'
            field_type = 'DATETIME'
...
if __name__ == '__main__':
    print("=" * 50)
    print("订单表添加拍摄完成时间字段脚本")
    print("=" * 50)
    print(f"执行时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()
    
    success = add_shooting_completed_at_field()
