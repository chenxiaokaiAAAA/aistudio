...
def generate_thumbnail(original_path, thumbnail_path=None, max_size=None, quality=None):
    """
    ...
    Args:
        ...
        max_size: 最大边长（如果为None，从配置读取，默认1920px）
        quality: JPEG质量（如果为None，从配置读取，默认85）
    ...
    if max_size is None or quality is None:
        try:
            ...
            logger.debug(f"读取缩略图配置失败，使用默认值: {e}")
            if max_size is None:
                max_size = 1920
            if quality is None:
                quality = 85
    try:
        if not os.path.exists(original_path):
            logger.error("原图文件不存在: {original_path}")
            return None
        ...
        with Image.open(original_path) as img:
            ...
            logger.info("✅ 缩略图生成成功:")
            logger.info(f"   原图: {original_path} ({original_size / 1024:.2f} KB)")
            logger.info(f"   缩略图: {thumbnail_path} ({thumbnail_size / 1024:.2f} KB)")
            logger.info(f"   压缩率: {compression_ratio:.1f}%")
            logger.info(f"   尺寸: {img.size[0]}x{img.size[1]}")

            return thumbnail_path

    except Exception as e:
        logger.error("生成缩略图失败: {str(e)}")
        import traceback
