...
def get_db_models():
    ...
    try:
        from app.models import (
            PromotionUser, Coupon, UserCoupon, Order
        )
        return {
...
def generate_stable_promotion_code(openid):
    ...
    print(f"基于OpenID生成推广码: {openid} -> {promotion_code}")
    return promotion_code
...
def generate_stable_user_id(openid):
    ...
    print(f"基于OpenID生成用户ID: {openid} -> {user_id}")
    return user_id
    ...
    try:
        ...
        if user_id.startswith('USER'):
            ...
            if user and user.open_id:
                ...
                print(f"用户 {user_id} (openid: {user.open_id}) 下过 {orders} 个有效订单")
                return orders > 0
