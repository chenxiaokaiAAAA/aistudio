            ...
            if not machine_id and store_name:
                ...
                try:
                    ...
                    if SelfieMachine:
                        # 查找该门店的第一个活跃自拍机
                        machine = (
                            SelfieMachine.query.filter_by(status="active")
                            .join(models.get("FranchiseeAccount"))
                            .filter(models.get("FranchiseeAccount").store_name == store_name)
                            .first()
                        )
            ...
            if printer_path_config and printer_path_config.config_value:
                ...
                return {
                    ...
                    "local_printer_proxy_url": (
                        proxy_url_config.config_value if proxy_url_config else ""
                    ),
                    "local_printer_proxy_api_key": (
                        api_key_config.config_value if api_key_config else ""
                    ),
...
def save_printer_config_for_machine(
    machine_id, printer_path=None, proxy_url=None, api_key=None, models=None
):
    """
    保存指定自拍机的打印机配置
    ...
    try:
        ...
        for config_key, config_value, description in configs_to_save:
            ...
            if config:
                ...
                config = AIConfig(
                    config_key=config_key, config_value=config_value, description=description
                )
            ...
            result.append(
                {
                    ...
                    "store_name": store_name,
                    "local_printer_path": (
                        printer_path_config.config_value if printer_path_config else ""
                    ),
                    "local_printer_proxy_url": (
                        proxy_url_config.config_value if proxy_url_config else ""
                    ),
