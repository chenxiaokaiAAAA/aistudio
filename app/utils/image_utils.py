...
def add_watermark_to_image(image_path, watermark_path, output_path=None, opacity=1.0, position='tiled'):
    """
    为图片添加水印
    ...
    try:
        ...
        if os.path.getsize(image_path) == 0:
            print(f"❌ 原图文件为空: {image_path}")
            return False
        
        if os.path.getsize(watermark_path) == 0:
            print(f"❌ 水印文件为空: {watermark_path}")
            return False
        ...
        with Image.open(image_path) as base_image:
            with Image.open(watermark_path) as watermark:
                ...
                if position == 'tiled':
                    ...
                    print(f"水印缩放: {wm_width}x{wm_height} -> {new_size[0]}x{new_size[1]}")
                ...
                if os.path.exists(temp_path) and os.path.getsize(temp_path) > 0:
                    ...
                    try:
                        ...
                        print(f"❌ 水印图片验证失败: {str(verify_error)}")
                        if os.path.exists(temp_path):
                            os.remove(temp_path)
                        return False
                else:
                    print(f"❌ 临时文件保存失败: {temp_path}")
                    if os.path.exists(temp_path):
                        os.remove(temp_path)
                    return False
