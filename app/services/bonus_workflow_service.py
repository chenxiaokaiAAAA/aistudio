...
def create_bonus_workflows_for_order(order_id, db=None, Order=None, Product=None, 
                                     ProductBonusWorkflow=None, APITemplate=None, 
                                     StyleImage=None, AITask=None, OrderImage=None):
    """
    为订单创建产品的额外赠送工作流任务
    ...
    try:
        ...
        if not all([db, Order, Product, ProductBonusWorkflow]):
            ...
            if 'test_server' in sys.modules:
                ...
                ProductBonusWorkflow = ProductBonusWorkflow or (test_server_module.ProductBonusWorkflow if hasattr(test_server_module, 'ProductBonusWorkflow') else None)
                APITemplate = APITemplate or (test_server_module.APITemplate if hasattr(test_server_module, 'APITemplate') else None)
        ...
        if hasattr(order, 'product_type') and order.product_type:
            ...
            if product_name_parts:
                # 尝试通过产品名称查找
                product = Product.query.filter(Product.name.like(f"%{product_name_parts[0]}%")).first()
                if product:
                    product_code = product.code
        
        if not product_code:
            print(f"⚠️  订单 {order.order_number} 无法确定产品代码，跳过赠送工作流")
            return True, 0, "无法确定产品代码"
        
        # 查找产品
        product = Product.query.filter_by(code=product_code, is_active=True).first()
        if not product:
            print(f"⚠️  订单 {order.order_number} 的产品 {product_code} 不存在，跳过赠送工作流")
            return True, 0, "产品不存在"
        
        # 查找产品的赠送工作流配置
        bonus_workflows = ProductBonusWorkflow.query.filter_by(
            product_id=product.id,
            is_active=True
        ).order_by(ProductBonusWorkflow.sort_order).all()
        
        if not bonus_workflows:
            print(f"ℹ️  产品 {product.name} 没有配置赠送工作流")
            return True, 0, "产品没有配置赠送工作流"
        
        print(f"🎁 为订单 {order.order_number} 创建 {len(bonus_workflows)} 个赠送工作流任务")
                ...
                if bonus_workflow.workflow_type == 'api_template':
                    ...
                    if upload_config and 'uploads' in upload_config:
                        ...
                        for upload_item in upload_config['uploads']:
                            if upload_item.get('key') == 'reference' or upload_item.get('name') == '参考图':
                                # 上传参考图
                    ...
                    if not uploaded_images:
                        uploaded_images = [{'url': f"file://{input_image_path}", 'key': 'reference'}]
                    
                    # 创建API任务
                    success, api_task, error_message = create_api_task(
                        style_image_id=api_template.style_image_id or (api_template.style_category_id and None),
                        prompt=prompt,
                    ...
                    if success and ai_task:
                        print(f"✅ 创建赠送ComfyUI任务成功: {bonus_workflow.workflow_name or style_image.name}")
                        created_count += 1
                    else:
                        print(f"❌ 创建赠送ComfyUI任务失败: {error_message}")
                
            except Exception as e:
                print(f"❌ 创建赠送工作流任务异常: {str(e)}")
                import traceback
                traceback.print_exc()
                continue
        
        print(f"🎁 订单 {order.order_number} 共创建 {created_count}/{len(bonus_workflows)} 个赠送工作流任务")
        return True, created_count, None
        
    except Exception as e:
        print(f"❌ 创建赠送工作流任务失败: {str(e)}")
        import traceback
        traceback.print_exc()
