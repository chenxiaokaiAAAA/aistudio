    ...
    try:
        ...
        try:
            ...
            if result.get('success'):
                print(f"✅ 图片已上传到OSS: {result['url']}")
                return True, result['url'], None
            else:
                error_msg = result.get('message', 'OSS上传失败')
                print(f"❌ OSS上传失败: {error_msg}")
                return False, None, error_msg
        except ImportError:
            print("⚠️  OSS配置未找到，跳过OSS上传")
    ...
    try:
        ...
        if use_oss:
            ...
            if success:
                ...
                print(f"⚠️  OSS上传失败，尝试使用本地URL: {error_msg}")
...
"""
    ...
    try:
        ...
        if image_path.startswith('http://') or image_path.startswith('https://'):
            ...
            print(f"✅ 使用提供的图片URL: {media_data_url}")
        else:
            # 是本地文件路径，需要检查文件是否存在
            ...
            media_data_url = upload_image_to_public_url(image_path)
        ...
        print(f"📤 美图API请求URL: {request_url}")
        print(f"📤 美图API请求参数: {json.dumps(request_data, ensure_ascii=False)}")
        ...
        try:
            ...
            print(f"📥 美图API响应状态码: {response.status_code}")
            print(f"📥 美图API响应内容: {response.text[:500]}")
            ...
            if response.status_code == 200:
                ...
                if result.get('code') == 0 and 'data' in result and 'msg_id' in result['data']:
                    ...
                    print(f"✅ 美图API调用成功，收到msg_id: {msg_id}")
                    # 异步接口，不立即返回结果图片
                    # 需要通过回调或轮询获取最终结果
...
def download_result_image(image_url, order_number=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ 美图结果图片已保存: {filepath}")
            return filepath
        else:
            print(f"❌ 下载美图结果图片失败: HTTP {response.status_code}")
            return None
    except Exception as e:
        print(f"❌ 下载美图结果图片异常: {str(e)}")
        return None
