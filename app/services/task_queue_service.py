...
def _init_queue():
    ...
    if TASK_QUEUE is None:
        ...
        print(f"✅ 任务队列已初始化: 最大大小={max_size}, 工作线程数={MAX_WORKERS}")
    return TASK_QUEUE
...
def submit_task(task_type: str, task_data: Dict[str, Any], priority: int = 0) -> bool:
    ...
    try:
        ...
        if queue_instance.full():
            print(f"⚠️ 任务队列已满（{queue_instance.qsize()}个任务），无法提交新任务")
            return False
        ...
        print(f"✅ 任务已提交到队列: {task_item['task_id']} (队列大小: {TASK_QUEUE.qsize()})")
        return True
        
    except queue.Full:
        print(f"❌ 任务队列已满，无法提交任务")
        return False
    except Exception as e:
        print(f"❌ 提交任务到队列失败: {str(e)}")
        return False
...
def process_task_worker(worker_id: int):
    ...
    Args:
        worker_id: 工作线程ID
    """
    print(f"🚀 任务处理工作线程 {worker_id} 已启动")
    ...
    while QUEUE_RUNNING:
        try:
            ...
            print(f"📦 工作线程 {worker_id} 开始处理任务: {task_id} (类型: {task_type})")
            ...
            try:
                ...
                QUEUE_STATS['total_failed'] += 1
                print(f"❌ 处理任务异常: {task_id}, 错误: {str(e)}")
                import traceback
                traceback.print_exc()
    ...
    print(f"🛑 任务处理工作线程 {worker_id} 已停止")
...
def start_task_queue():
    """启动任务队列服务"""
    global QUEUE_RUNNING, WORKER_THREADS, MAX_WORKERS
    ...
    for i in range(MAX_WORKERS):
        ...
        print(f"✅ 任务处理工作线程 {i + 1} 已启动")
    
    print(f"🚀 任务队列服务已启动，工作线程数: {MAX_WORKERS}")
