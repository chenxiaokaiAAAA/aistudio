...
def get_db_models():
    ...
    try:
        ...
        return {"Order": Order, "UserCoupon": UserCoupon}
    except ImportError:
        return None
...
def get_miniprogram_login_config():
    """获取小程序登录配置（用于 jscode2session 等，仅需 appid 和 app_secret）
    优先从数据库 miniprogram_appid/miniprogram_app_secret 读取，不依赖支付配置"""
    try:
        import sys
        ...
        if "test_server" in sys.modules:
            ...
            try:
                if hasattr(test_server_module, "AIConfig") and hasattr(test_server_module, "db"):
                    ...
                    appid_config = AIConfig.query.filter_by(config_key="miniprogram_appid").first()
                    secret_config = AIConfig.query.filter_by(config_key="miniprogram_app_secret").first()
                    if appid_config and secret_config and appid_config.config_value and secret_config.config_value:
                        return {
                            "appid": appid_config.config_value,
                            "app_secret": secret_config.config_value,
                        }
            except Exception as e:
                logger.warning("从数据库读取小程序登录配置失败: %s", e)
            if hasattr(test_server_module, "WECHAT_PAY_CONFIG"):
                cfg = test_server_module.WECHAT_PAY_CONFIG
                return {"appid": cfg.get("appid", ""), "app_secret": cfg.get("app_secret", "")}
    except (ImportError, AttributeError) as e:
        logger.warning("获取小程序登录配置失败: %s", e)
    return None
                ...
                if hasattr(test_server_module, "AIConfig") and hasattr(test_server_module, "db"):
                    ...
                    if configs.get("pay_appid") or configs.get("appid"):
                        ...
                        if result["appid"] and result["mch_id"] and result["api_key"]:
                            logger.info(f"✅ 使用数据库中的微信支付配置: appid={result['appid']}")
                            return result
            except Exception as e:
                logger.warning("从数据库读取微信支付配置失败: {str(e)}")

            # 如果数据库没有配置或读取失败，使用test_server.py中的默认配置
            if hasattr(test_server_module, "WECHAT_PAY_CONFIG"):
                logger.info("ℹ️ 使用test_server.py中的默认微信支付配置")
                return test_server_module.WECHAT_PAY_CONFIG
    except (ImportError, AttributeError) as e:
        logger.warning("获取微信支付配置失败: {str(e)}")
    return None
...
def create_payment_order(
    ...
    from app.utils.helpers import dict_to_xml, generate_nonce_str, generate_sign, xml_to_dict
...
def handle_payment_notify(xml_data):
    ...
    from app.utils.helpers import dict_to_xml, verify_sign, xml_to_dict
    ...
    try:
        ...
        return (
            False,
            dict_to_xml({"return_code": "FAIL", "return_msg": f"处理失败: {str(e)}"}),
            None,
        )
...
def get_user_openid(code, dev_mode=False):
    ...
    if dev_mode:
        ...
        return True, {"openid": test_openid, "message": "开发模式：使用测试openid"}, None
    ...
    try:
        ...
        if "openid" in result:
            return (
                True,
                {"openid": result["openid"], "session_key": result.get("session_key", "")},
                None,
            )
        else:
