...
class VeoVideoProvider(BaseAPIProvider):
    ...
    def build_request_body(self, prompt: str, model_name: str,
        ...
        if image_urls_to_process and len(image_urls_to_process) > 0:
            ...
            print(f"📸 [VEO] 使用 {len(images_to_send)} 张图片（模型 {model_name} 限制最多 {max_images} 张）")
        ...
        if aspect_ratio and aspect_ratio != 'auto':
            if aspect_ratio in ['16:9', '9:16']:
                ...
                print(f"⚠️ [VEO] 不支持的比例 {aspect_ratio}，VEO只支持16:9和9:16")
        ...
        if self.is_t8star:
            payload["async"] = "true"
            print(f"📝 [VEO] T8Star服务商：启用异步模式（async=true）")
    ...
    def call_api(self, draw_url: str, request_data: Dict[str, Any],
        ...
        print(f"📤 调用 VEO 视频生成 API: {draw_url}")
        print(f"📤 请求参数: {json.dumps(request_data, ensure_ascii=False)}")
        print(f"📤 超时设置: 连接30秒, 读取300秒（视频生成需要较长时间）")
        ...
        print(f"✅ VEO API响应状态码: {response.status_code}")
        return response
        ...
        if response.status_code == 200:
            try:
                ...
                if 'data' in data and isinstance(data.get('data'), dict):
                    task_id = data.get('data', {}).get('task_id') or data.get('data', {}).get('taskId') or data.get('data', {}).get('id')
    ...
    def parse_polling_response(self, response: requests.Response) -> Dict[str, Any]:
        ...
        try:
            ...
            if data.get('code') == 0:
                ...
                video_url = result_data.get('video_url') or result_data.get('url') or result_data.get('videoUrl')
