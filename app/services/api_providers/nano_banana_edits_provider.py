...
class NanoBananaEditsProvider(BaseAPIProvider):
    ...
    def _prepare_image_files(self, image_urls: List[str]) -> List[Tuple[str, Tuple[str, bytes, str]]]:
        """
        准备图片文件（下载并转换为文件元组）
        ...
        for idx, img_url in enumerate(image_urls):
            ...
            try:
                print(f"📥 正在下载图片 {idx+1}/{len(image_urls)}: {img_url}")
                ...
                print(f"✅ 已下载图片 {idx+1}/{len(image_urls)}: {filename}, 大小: {len(img_content)} bytes")
    ...
    def parse_response(self, response: requests.Response) -> Dict[str, Any]:
        ...
        if response.status_code == 200:
            try:
                ...
                if 'data' in result and isinstance(result.get('data'), list) and len(result['data']) > 0:
            ...
            if self.is_t8star:
                ...
                if result_data.get('code') == 'success':
                    ...
                    if status == 'SUCCESS':
                        ...
                        return {
                            "status": "failed",
                            "error": data.get('fail_reason', '任务失败')
                        }
            else:
                # 其他服务商的响应格式
