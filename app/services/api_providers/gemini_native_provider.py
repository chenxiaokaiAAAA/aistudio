...
class GeminiNativeProvider(BaseAPIProvider):
    ...
    def __init__(self, api_config):
        ...
        self.is_proxy_server = self.host and "/api/gemini/generate" in (
            self.api_config.draw_endpoint or ""
        )
    ...
    def build_request_body(
        ...
        **kwargs,
        ...
        if uploaded_images:
            ...
            logger.info(
                f"📸 [gemini-native] 请求中包含 {len(uploaded_images)} 张图片（已转换为base64，包含在payload中）"
            )
        else:
    ...
    def call_api(
        ...
        try:
            ...
            if elapsed_time > 5:
                ...
                raise Exception(
                    f"同步API代理连接失败。请检查代理服务器是否正常运行。错误: {error_str}"
                )
            ...
            try:
                ...
                if "candidates" in data and len(data["candidates"]) > 0:
                    ...
                    if "content" in candidate and "parts" in candidate["content"]:
                        ...
                        for part in parts:
                            if "text" in part:
                                return {"success": True, "text": part["text"], "data": data}
