...
class RunningHubComfyUIWorkflowProvider(BaseAPIProvider):
    ...
    def build_request_body(self, prompt: str, model_name: str,
        ...
        if kwargs.get('request_data') and isinstance(kwargs.get('request_data'), dict):
            ...
            # 检查是否已经包含完整的请求体
            if 'apiKey' in request_data and 'workflowId' in request_data and 'nodeInfoList' in request_data:
                print(f"✅ RunningHub ComfyUI 工作流：使用已构建的请求体")
                return request_data
        ...
        print(f"   - 提示词: {final_prompt[:50] if final_prompt else 'None'}...")
            ...
            if 'fieldName' in node_info and 'fieldValue' in node_info:
                ...
                if field_name in ['image', 'imageUrls']:
                    if (field_value == '{{image_url}}' or field_value == '' or field_value == '{{ref_image_url}}') and image_index < len(image_urls_to_process):
                        ...
                        print(f"   ✅ 替换节点 {node_id} 的 {field_name}: {image_urls_to_process[image_index]}")
                        image_index += 1
                elif field_name == 'text':
                    if field_value == '{{prompt}}' and final_prompt:
                        field_value = final_prompt
                        print(f"   ✅ 替换节点 {node_id} 的 {field_name}: {final_prompt[:50]}...")
    ...
    def call_api(self, draw_url: str, request_data: Dict[str, Any],
        ...
        print(f"📤 API Key 长度: {len(self.api_key) if self.api_key else 0} 字符")
        ...
        if response.status_code == 401:
            ...
            print(f"   当前使用的 API Key 长度: {len(self.api_key) if self.api_key else 0} 字符")
            print(f"   当前使用的认证方式: apiKey (请求体)")
            ...
            try:
                ...
                if task_id:
                    ...
                    if code != 0:
                        ...
                        print(f"⚠️ RunningHub ComfyUI 工作流任务已创建，但有警告: {error_msg}")
