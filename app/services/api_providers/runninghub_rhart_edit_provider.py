...
class RunningHubRhartEditProvider(BaseAPIProvider):
    ...
    def build_request_body(
        ...
        **kwargs,
        ...
        logger.info(f"📸 [RunningHub] 请求包含 {len(image_urls_to_process)} 张图片")
    ...
    def call_api(
        ...
        logger.info(f"📤 图片数量: {len(request_data.get('imageUrls', []))}")
        ...
        if response.status_code == 200:
            try:
                ...
                if task_id:
                    ...
                    return {"success": False, "error": data.get("msg", "API调用失败，未返回taskId")}
            except Exception:
