...
class BaseAPIProvider(ABC):
    ...
    def call_api(
        ...
        response = requests.post(
            draw_url, json=request_data, headers=headers, timeout=timeout, proxies=proxies
        )
        return response
    ...
    def parse_polling_response(self, response: requests.Response) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "failed", "error": data.get("msg", "未知错误")}
        except Exception:
    ...
    def get_proxy_settings(self) -> Optional[Dict[str, str]]:
        ...
        is_known_domestic_domain = self.host and any(
