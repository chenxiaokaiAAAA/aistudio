...
def register_provider(api_type: str, provider_class: Type[BaseAPIProvider]):
    ...
    logger.info(f"✅ 注册服务商: {api_type} -> {provider_class.__name__}")
...
def get_provider(api_config) -> Optional[BaseAPIProvider]:
    ...
    try:
        ...
        logger.warning("创建服务商实例失败 ({api_type}): {str(e)}")
        return None
...
def _lazy_import_providers():
    ...
    try:
        ...
        logger.warning("导入 nano-banana 服务商实现失败: {str(e)}")
    ...
    try:
        ...
        if hasattr(nano_banana_edits_provider, "NanoBananaEditsProvider"):
            register_provider(
                "nano-banana-edits", nano_banana_edits_provider.NanoBananaEditsProvider
            )
    except ImportError as e:
        logger.warning("导入 nano-banana-edits 服务商实现失败: {str(e)}")
    ...
    try:
        ...
        logger.warning("导入 gemini-native 服务商实现失败: {str(e)}")
    ...
    try:
        ...
        if hasattr(runninghub_rhart_edit_provider, "RunningHubRhartEditProvider"):
            register_provider(
                "runninghub-rhart-edit", runninghub_rhart_edit_provider.RunningHubRhartEditProvider
            )
    except ImportError as e:
        logger.warning("导入 runninghub-rhart-edit 服务商实现失败: {str(e)}")
    ...
    try:
        ...
        logger.warning("导入 veo-video 服务商实现失败: {str(e)}")
    ...
    try:
        ...
        if hasattr(runninghub_comfyui_workflow_provider, "RunningHubComfyUIWorkflowProvider"):
            register_provider(
                ...
                runninghub_comfyui_workflow_provider.RunningHubComfyUIWorkflowProvider,
            )
    except ImportError as e:
        logger.warning("导入 runninghub-comfyui-workflow 服务商实现失败: {str(e)}")
