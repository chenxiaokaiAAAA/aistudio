...
def get_server_config():
    ...
    try:
        ...
        return {"get_base_url": get_base_url, "get_media_url": get_media_url}
    except ImportError:
...
def create_miniprogram_order(order_data):
    ...
    from app.utils.helpers import (
        check_user_eligible_for_commission,
    ...
    try:
        ...
        logger.info("🔍 订单创建模式检查:")
        logger.info(f"   add_to_existing_order: {add_to_existing_order}")
        logger.info(f"   existing_order_number: {existing_order_number}")
        logger.info(f"   is_multiple_items: {is_multiple_items}")
        logger.info(f"   items数量: {len(items) if items else 0}")

        # 如果是追加产品模式，即使只有一个商品，也使用多个商品模式处理（确保使用原订单号）
        if add_to_existing_order and existing_order_number:
            logger.info("✅ 进入追加产品模式")
            # 追加产品模式：使用多个商品模式处理
            ...
            logger.info("✅ 进入多个商品模式（正常下单）")
            # 多个商品模式：为每个商品创建订单，使用相同的订单号
            return create_multiple_items_order(order_data, items, models, db, app)
        else:
            logger.info("✅ 进入单个商品模式（正常下单）")
        ...
        # 打印调试信息
        logger.info("🔍 create_miniprogram_order 单个商品模式订单号生成检查:")
        logger.info(f"   add_to_existing_order: {add_to_existing_order}")
        logger.info(f"   existing_order_number: {existing_order_number}")
        logger.info(f"   order_data.get('orderId'): {order_data.get('orderId', 'None')}")
        ...
        if add_to_existing_order and existing_order_number:
            ...
            logger.info(f"✅ 追加产品模式：使用原订单号 {order_number}")
        else:
            # 正常下单模式：始终生成新的订单号，忽略前端传入的临时订单号
            # 前端可能传入PET开头的临时订单号或"待生成"，这些都应该被忽略
            order_number = (
                f"MP{datetime.now().strftime('%Y%m%d%H%M%S')}{uuid.uuid4().hex[:4].upper()}"
            )
            logger.info(f"✅ 新建订单模式：生成新订单号 {order_number}（忽略前端传入的临时订单号）")
        ...
        if franchisee_qr_code:
            franchisee = FranchiseeAccount.query.filter_by(
                qr_code=franchisee_qr_code, status="active"
            ).first()
            if franchisee:
                ...
                logger.info(
                    f"✅ 通过门店二维码下单，关联门店: {franchisee.company_name} (ID: {franchisee_id})"
                )
            else:
                logger.warning("门店二维码无效或门店已禁用: {franchisee_qr_code}")
                # 不返回错误，允许订单继续创建（向后兼容）
            ...
            try:
                ...
                if promotion_user:
                    ...
                    if not promotion_user.eligible_for_promotion and has_order:
                        ...
                        if (
                            promotion_user.promotion_code
                            and promotion_user.promotion_code.startswith("TEMP_")
                        ):
                            promotion_code = generate_stable_promotion_code(new_order.openid)

                            original_code = promotion_code
                            counter = 1
                            while PromotionUser.query.filter_by(
            ...
            try:
                ...
                if validate_promotion_code(referrer_promotion_code) == referrer_user_id:
                    ...
                    if referrer_eligible:
                        ...
                        promotion_user = PromotionUser.query.filter_by(
        ...
        if order_data.get("uploadedImages"):
            for img_data in order_data["uploadedImages"]:
                if img_data.get("filename"):
                    db.session.add(
                        OrderImage(order_id=new_order.id, path=img_data["filename"], is_main=False)
                    )
                    ...
                    if app:
                        ...
                        if img_url.startswith("http://tmp/") or img_url.startswith("tmp/"):
                            ...
                            db.session.add(
                                OrderImage(order_id=new_order.id, path=img_filename, is_main=False)
                            )
                            ...
                            db.session.add(
                                OrderImage(order_id=new_order.id, path=img_url, is_main=False)
                            )
        ...
        try:
            ...
            if shared_user_id and (share_record_id or work_id):
                ...
                if result.get("status") == "success":
                    logger.info(f"✅ 分享奖励已发放: {result.get('message')}")
        except Exception as e:
            logger.warning("分享奖励处理异常: {e}")

        return True, {"orderId": order_number, "orderId_db": new_order.id}, None

    except Exception as e:
        if db:
            db.session.rollback()
        logger.info(f"创建订单失败: {str(e)}")
...
def create_multiple_items_order(order_data, items, models, db, app):
    ...
    logger.info(
        f"🔍 追加产品模式检查: add_to_existing_order={add_to_existing_order}, existing_order_number={existing_order_number}"
    )

    if add_to_existing_order and existing_order_number:
        existing_order = Order.query.filter_by(order_number=existing_order_number).first()
        if not existing_order:
            logger.error("原订单不存在: {existing_order_number}")
            return False, None, f"原订单不存在: {existing_order_number}"

        logger.info(
            f"✅ 找到原订单: ID={existing_order.id}, 订单号={existing_order.order_number}, 状态={existing_order.status}, 订单类型={existing_order.order_mode}"
        )

        # 检查订单状态：必须是已支付且未拍摄（状态为paid）
        # 如果状态是shooting或更后面的状态，说明已经拍摄了，不能追加
        if existing_order.status not in ["paid", "已支付"]:
            logger.error("订单状态不正确，无法追加: {existing_order.status}")
            return False, None, "订单已拍摄或已上传图片，无法追加产品。请重新下单"

        # 检查是否已上传图片：如果已上传图片，不能追加产品
        existing_images = OrderImage.query.filter_by(order_id=existing_order.id).all()
        if existing_images:
            logger.error("订单已上传图片，无法追加: 图片数量={len(existing_images)}")
            return False, None, "订单已上传图片，无法追加产品。请重新下单"

        logger.info("✅ 原订单验证通过，可以追加产品")
    ...
    try:
        # 打印调试信息
        logger.info("🔍 create_multiple_items_order 订单号生成检查:")
        logger.info(f"   add_to_existing_order: {add_to_existing_order}")
        logger.info(f"   existing_order_number: {existing_order_number}")
        logger.info(f"   order_data.get('orderId'): {order_data.get('orderId', 'None')}")
        ...
        if franchisee_qr_code:
            ...
            if franchisee:
                ...
                logger.info(
                    f"✅ 通过门店二维码下单（多个商品），关联门店: {franchisee.company_name} (ID: {franchisee_id})"
                )
            else:
                logger.warning("门店二维码无效或门店已禁用: {franchisee_qr_code}")
                # 不返回错误，允许订单继续创建（向后兼容）
        ...
        if add_to_existing_order and existing_order_number:
            ...
            logger.info(
                f"🔍 追加产品模式：当前订单号 {order_number} 已有 {existing_orders_count} 条记录"
            )
        ...
        for idx, item in enumerate(items):
            ...
            new_order = Order(
                ...
                # 追加产品时继承原订单类型
                order_mode=(
                    existing_order.order_mode
                    if (add_to_existing_order and existing_order)
                    else order_data.get("orderMode", None)
                ),
            )
        ...
        if add_to_existing_order and existing_order:
            logger.info("✅ 追加产品成功！")
            logger.info(f"   原订单号: {order_number}")
            logger.info(f"   新创建的订单数量: {len(created_orders)}")
            logger.info(f"   新订单ID列表: {[o.id for o in created_orders]}")
            logger.info(f"   所有订单共享订单号: {order_number}")

        # 更新用户推广资格（只处理一次）
        if created_orders and created_orders[0].openid:
            try:
                promotion_user = PromotionUser.query.filter_by(
                    open_id=created_orders[0].openid
                ).first()
                if promotion_user:
                    has_order = check_user_has_placed_order(promotion_user.user_id)
        ...
        if order_data.get("uploadedImages"):
            for img_data in order_data["uploadedImages"]:
                for order in created_orders:
                    if img_data.get("filename"):
                        db.session.add(
                            OrderImage(order_id=order.id, path=img_data["filename"], is_main=False)
                        )
                    elif img_data.get("url"):
                        img_url = img_data["url"]
                        img_filename = f"mp_{uuid.uuid4().hex[:8]}.jpg"
                        ...
                        if app:
                            ...
                            if img_url.startswith("http://tmp/") or img_url.startswith("tmp/"):
                                ...
                                db.session.add(
                                    OrderImage(order_id=order.id, path=img_filename, is_main=False)
                                )
                            elif img_url.startswith("http"):
                                from PIL import Image
                                ...
                                db.session.add(
                                    OrderImage(order_id=order.id, path=img_url, is_main=False)
                                )
        ...
        if add_to_existing_order and existing_order and existing_order.order_mode == "shooting":
            ...
            db.session.commit()
            logger.info(
                f"✅ 追加产品完成，已更新所有新订单的订单类型为'立即拍摄'，订单号: {order_number}"
            )
            logger.info(
                f"   原订单类型: {existing_order.order_mode}, 新订单数量: {len(created_orders)}"
            )
        ...
        if db:
            db.session.rollback()
        logger.info(f"创建多个商品订单失败: {str(e)}")
        import traceback
    ...
    try:
        ...
        for order in orders:
            ...
            if order.final_image:
                ...
                if order.status in ["manufacturing", "completed", "shipped", "delivered"]:
                    item_final_image_no_watermark_url = (
                        f"{get_media_url()}/final/clean_{order.final_image}"
                    )
            ...
            if order.hd_image:
                ...
                if order.status in ["manufacturing", "completed", "shipped", "delivered"]:
                    ...
                    item_hd_image_no_watermark_url = (
                ...
                try:
                    ...
                    if Product and ProductSize:
                        ...
                        product = Product.query.filter_by(
                            name=order.product_name, is_active=True
                        ).first()
                        if product:
                            # 通过尺寸名称查找尺寸
                            size = ProductSize.query.filter_by(
                                product_id=product.id, size_name=order.size, is_active=True
                            ).first()
                            if size and size.effect_image_url:
                                # 构建完整的图片URL
                                if size.effect_image_url.startswith("/"):
                                    size_effect_image_url = (
                                        f"{get_base_url()}{size.effect_image_url}"
                                    )
        ...
        if main_order.final_image:
            ...
            if main_order.status in ["manufacturing", "completed", "shipped", "delivered"]:
                main_final_image_no_watermark_url = (
        ...
        if main_order.hd_image:
            ...
            if main_order.status in ["manufacturing", "completed", "shipped", "delivered"]:
                ...
                encoded_clean_filename = quote(clean_filename, safe="")
                main_hd_image_no_watermark_url = (
                    f"{get_base_url()}/public/hd/{encoded_clean_filename}"
                )
    ...
    try:
        ...
        if not order.franchisee_id and machine_serial_number:
            try:
                ...
                logger.info(f"通过自拍机序列号关联加盟商失败: {e}")
        ...
        if has_photos:
            machine_name = None
            machine_serial = order.selfie_machine_id or machine_serial_number
            if machine_serial:
                machine = SelfieMachine.query.filter_by(
        ...
        # 设备名称、序列号：订单可能已关联 selfie_machine_id，或通过 machine_serial_number 查询
        machine_name = None
        machine_serial = order.selfie_machine_id or machine_serial_number
        if machine_serial:
            machine = SelfieMachine.query.filter_by(
