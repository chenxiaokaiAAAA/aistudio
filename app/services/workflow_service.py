...
def _get_comfyui_semaphore():
    ...
    if COMFYUI_SEMAPHORE is None:
        ...
        logger.info(f"✅ ComfyUI并发信号量已初始化: {max_concurrency}")
    return COMFYUI_SEMAPHORE


def get_workflow_config(
    style_category_id, style_image_id=None, db=None, StyleCategory=None, StyleImage=None
):
    """
    ...
    if not all([db, StyleCategory, StyleImage]):
        logger.error("无法获取数据库模型，请确保db、StyleCategory、StyleImage已初始化")
        return None

    # 1. 获取分类级别配置
    category = StyleCategory.query.get(style_category_id)
    if not category:
        logger.error("风格分类不存在，ID: {style_category_id}")
        return None
    ...
    if style_image_id:
        ...
        if style_image:
            ...
            if style_image.is_ai_enabled is True:
                logger.info(
                    f"✅ 风格图片 '{style_image.name}' (ID: {style_image_id}) 已启用AI工作流，使用图片级别配置"
                )
                config = {
                    "workflow_name": style_image.workflow_name,
                    "workflow_file": style_image.workflow_file,
                    "workflow_input_ids": (
                        json.loads(style_image.workflow_input_ids)
                ...
                if not config["workflow_file"] and category.is_ai_enabled:
                    logger.warning("图片级别配置不完整，尝试从分类级别补充")
            ...
            # 如果图片级别明确禁用AI，返回None
                ...
                logger.error("风格图片 '{style_image.name}' (ID: {style_image_id}) 已禁用AI工作流")
                return None
            # 如果图片级别 is_ai_enabled 为 None，继承分类配置（继续执行下面的逻辑）
            elif style_image.is_ai_enabled is None:
                logger.info(
                    f"ℹ️ 风格图片 '{style_image.name}' (ID: {style_image_id}) 继承分类配置 (is_ai_enabled=None)"
                )

    # 3. 如果图片级别未启用或未指定或继承分类配置，使用分类级别配置
    if not category.is_ai_enabled:
        logger.warning(
            "风格分类 '{category.name}' (ID: {style_category_id}) 的AI工作流未启用 (is_ai_enabled={category.is_ai_enabled})"
        )
        return None

    logger.info(f"✅ 使用风格分类 '{category.name}' (ID: {style_category_id}) 的分类级别配置")
    config = {
    ...
    if not config["workflow_file"]:
        logger.warning("工作流配置缺少必要字段 workflow_file")
        return None
    if not config["workflow_input_ids"]:
        logger.warning("工作流配置缺少必要字段 workflow_input_ids")
        return None
    if not config["workflow_output_id"]:
        logger.warning("工作流配置缺少必要字段 workflow_output_id")
        return None

    logger.info(
        f"✅ 工作流配置验证通过: workflow_file={config['workflow_file']}, workflow_input_ids={config['workflow_input_ids']}, workflow_output_id={config['workflow_output_id']}"
    )
    return config
...
def get_retouched_image_path(order, db=None, OrderImage=None):
    ...
    if hasattr(order, "retouched_image") and order.retouched_image:
        upload_folder = (
            current_app.config.get("UPLOAD_FOLDER", "uploads") if current_app else "uploads"
        )
...
def get_comfyui_config(db=None, AIConfig=None):
    ...
    if db and AIConfig:
        ...
        if base_url_config:
            ...
            logger.info(f"📝 从数据库读取ComfyUI地址: {base_url_config.config_value}")
        ...
        logger.warning("无法获取数据库配置，使用默认值: {result['base_url']}")
...
def create_ai_task(
    ...
    try:
        ...
        if order.order_mode == "making":
            logger.info(
                f"ℹ️ 订单 {order.order_number} 类型为'立即制作'，跳过AI任务创建，等待人工处理"
            )
        ...
        if order.order_mode and order.order_mode != "shooting":
            logger.warning(
                "订单 {order.order_number} 类型为未知类型 '{order.order_mode}'，跳过AI任务创建"
            )
            return False, None, f"订单类型 '{order.order_mode}' 不支持AI任务创建"

        logger.info(f"✅ 订单 {order.order_number} 类型为'立即拍摄'，开始创建AI任务")
        ...
        try:
            if order_image_id:
                ...
                if target_image:
                    ...
                    for existing_task in existing_tasks:
                        if (
                            existing_task.input_image_path == expected_path
                            or existing_task.input_image_path == target_image.path
                        ):
                            logger.warning(
                                "订单 {order_id} 的图片 {order_image_id} 已有待处理/处理中的任务（ID: {existing_task.id}），跳过重复创建"
                            )
                ...
                existing_task = db.session.execute(
                    select(AITask)
                    .where(
                        AITask.order_id == order_id, AITask.status.in_(["pending", "processing"])
                    )
                    .with_for_update()
                ).scalar_one_or_none()
                if existing_task:
                    logger.warning(
                        "订单 {order_id} 已有待处理/处理中的任务（ID: {existing_task.id}），跳过重复创建"
                    )
                    step_times["1.1_检查重复任务"] = time_module.time() - step_start
                    return True, existing_task, None
            step_times["1.1_检查重复任务"] = time_module.time() - step_start
            ...
            if order_image_id:
                existing_task = (
                    AITask.query.filter_by(order_id=order_id)
                ...
                if existing_task:
                    ...
                    if target_image:
                        upload_folder = (
                            app.config.get("UPLOAD_FOLDER", "uploads") if app else "uploads"
                        )
                        expected_path = os.path.join(upload_folder, target_image.path)
                        if (
                            existing_task.input_image_path == expected_path
        ...
        if order_image_id:
            ...
            if not target_image or target_image.order_id != order_id:
                return (
                    False,
                    None,
                    f"指定的图片不存在或不属于该订单: order_image_id={order_image_id}",
                )
            ...
            if (
                prefer_retouched
                and prefer_retouched.lower() == "true"
                and order.retouch_completed_at
                and target_image.is_main
            ):
                retouched_image = get_retouched_image_path(order, db=db, OrderImage=OrderImage)
                if retouched_image and os.path.exists(retouched_image):
            ...
            logger.info(f"📸 处理指定的图片: {target_image.path} (order_image_id={order_image_id})")
        else:
            # 处理主图或第一张（保持向后兼容）
            input_image_path, input_image_type = get_input_image(
                order, db=db, OrderImage=OrderImage
            )
        ...
        if order.status in ["retouching", "shooting", "paid"]:
            ...
            logger.info(f"✅ 订单 {order.order_number} 状态已更新为: ai_processing")
        ...
        if input_ids and len(input_ids) > 0:
            ...
            try:
                ...
                logger.info(f"   本地图片路径: {input_image_path}")
                ...
                logger.info(
                    f"   文件大小: {file_size_mb:.2f} MB (检查耗时: {time_module.time() - file_check_start:.3f}秒)"
                )
                ...
                with open(input_image_path, "rb") as f:
                    ...
                    logger.info(f"   读取文件耗时: {file_read_duration:.3f} 秒")
                    ...
                    logger.info(
                        f"   上传耗时: {upload_duration:.2f} 秒 (超时设置: {upload_timeout}秒)"
                    )
                ...
                logger.warning("上传图片超时，使用原始文件名: {comfyui_image_filename}")
            ...
            if isinstance(workflow_data, dict) and input_ids[0] in workflow_data:
                ...
                logger.info(
                    f"📸 工作流节点 {input_ids[0]} 的图片路径已设置为: {comfyui_image_filename}"
                )
        ...
        try:
            ...
            try:
                ...
                logger.info(f"   提交到ComfyUI耗时: {submit_duration:.2f} 秒")
            ...
            if response.status_code == 200:
                ...
                logger.info(f"   📋 ComfyUI响应预览: {response_preview}...")

                # 检查响应是否包含完成信息（不应该有）
                # ComfyUI的/api/prompt接口应该只返回prompt_id，不应该包含outputs
                if "outputs" in result or "status" in result:
                    logger.info(
                        "   ⚠️ 警告: ComfyUI响应包含outputs或status字段，这可能不是提交响应而是完成响应"
                    )
                    logger.info("   ⚠️ 如果ComfyUI已经完成，应该立即检查history API获取结果")
                ...
                try:
                    ...
                    logger.info(f"   🔍 立即检查任务是否已完成: {history_url}")
                    check_response = requests.get(
                        history_url, timeout=5, proxies={"http": None, "https": None}
                    )
                                ...
                                if images and len(images) > 0:
                                    ...
                                    try:
                                        ...
                                        local_path = download_api_result_image(
                                            image_url, prompt_id, None
                                        )
                                        if local_path:
                                            # 更新为本地路径
                                            ai_task.output_image_path = local_path
                                            logger.info(f"   ✅ 结果图已下载到本地: {local_path}")
                                            ...
                                            try:
                                                ...
                                                if thumbnail_path:
                                                    logger.info(
                                                        f"   ✅ 缩略图生成成功: {thumbnail_path}"
                                                    )
                                            except Exception as thumb_error:
                                                logger.info(
                                                    f"   ⚠️ 生成缩略图失败: {str(thumb_error)}"
                                                )
                                        else:
                                            logger.info(f"   ⚠️ 下载失败，保留云端URL: {image_url}")
                                    except Exception as download_error:
                                        logger.info(f"   ⚠️ 下载结果图失败: {str(download_error)}")
                                        import traceback
                                    ...
                                    ai_task.processing_log = json.dumps(
                                    ...
                                    if order_id and order_id > 0:
                                        try:
                                            ...
                                            if Order:
                                                ...
                                                logger.info(
                                                    f"   🔍 检查订单 {order_id} 的AI任务完成情况: 总任务数={len(all_tasks)}"
                                                )
                                                ...
                                                logger.info(
                                                    f"   🔍 有效任务数={len(valid_tasks)}, 已完成任务数={len(completed_tasks)}"
                                                )
                                                ...
                                                if len(valid_tasks) > 0 and len(
                                                    ...
                                                    logger.info(
                                                        f"   ⏳ 订单 {order_id} 还有未完成的任务: {len(completed_tasks)}/{len(valid_tasks)}"
                                                    )
                                            else:
                                                logger.info(
                                                    "   ⚠️ 无法获取Order模型，跳过订单状态检查"
                                                )
                                        except Exception as status_check_error:
                                            logger.info(
                                                f"   ⚠️ 检查订单状态失败: {str(status_check_error)}"
                                            )
                                            import traceback
                                    ...
                                    db.session.commit()
                                    logger.info("   ✅ 任务已立即完成并更新状态，无需等待轮询")
                except Exception as immediate_check_error:
                    # 立即检查失败不影响主流程，继续正常流程
                    logger.info(
                        f"   ℹ️ 立即检查任务状态失败（不影响主流程）: {immediate_check_error}"
                    )
...
def retry_ai_task(
    task_id, db=None, AITask=None, Order=None, StyleCategory=None, StyleImage=None, OrderImage=None
):
    """
    重新处理AI任务
