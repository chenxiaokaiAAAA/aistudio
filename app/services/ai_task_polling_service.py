...
def poll_meitu_api_tasks():
    ...
    try:
        ...
        if 'test_server' not in sys.modules:
            print("⚠️ [美图轮询] test_server模块未加载，无法轮询")
            return 0
        ...
        with test_server_module.app.app_context():
            ...
            # 先查询所有pending任务（用于调试）
            all_pending = MeituAPICallLog.query.filter(
            ...
            pending_tasks = MeituAPICallLog.query.filter(
                MeituAPICallLog.status == 'pending',
                MeituAPICallLog.created_at <= cutoff_time
            ).all()
            
            # 只在有待处理任务时才输出调试信息（避免无任务时产生过多日志）
            if pending_tasks:
                print(f"🔍 [美图轮询] 发现 {len(all_pending)} 个pending任务，其中 {len(pending_tasks)} 个满足轮询条件（创建时间超过30秒）")
                for task in pending_tasks[:3]:  # 只显示前3个待轮询任务的详情
                    age_seconds = (datetime.now() - task.created_at).total_seconds() if task.created_at else 0
                    ...
                    print(f"   - 任务 {task.id}: 创建于 {age_seconds:.1f}秒前, msg_id={msg_id_str}, 状态={task.status}")
            
            if not pending_tasks:
                return 0
            
            print(f"🔄 [美图轮询] 开始轮询 {len(pending_tasks)} 个待处理任务...")
            ...
            for task in pending_tasks:
                try:
                    ...
                    if not msg_id:
                        print(f"⚠️ [美图轮询] 任务 {task.id} 没有msg_id，跳过轮询")
                        continue
                    
                    print(f"🔄 [美图轮询] 开始轮询任务 {task.id}，msg_id={msg_id}")
                    ...
                    if not api_key or not api_secret:
                        ...
                        api_secret = getattr(config, 'api_secret', None) or getattr(config, 'secret_id', '')
                    ...
                    print(f"📤 [美图轮询] 查询任务 {task.id}，URL: {query_url}")
                    response = requests.post(query_url, json=query_data, headers=headers, timeout=30, proxies=proxies)
                    print(f"📥 [美图轮询] 任务 {task.id} 查询响应: HTTP {response.status_code}")
                    
                    if response.status_code == 200:
                        result = response.json()
                        print(f"📋 [美图轮询] 任务 {task.id} 查询响应内容: {json.dumps(result, ensure_ascii=False)[:300]}")
                        ...
                        if result.get('code') == 0 and 'data' in result:
                            ...
                            if not result_url:
                                ...
                                print(f"📊 [美图轮询] 任务 {task.id} 解析状态: {status}")
                                
                                if status == 'success':
                                    # 从其他字段获取结果URL
                                    result_url = data.get('result_url') or data.get('result_image') or data.get('url')
                            
                            if result_url:
                                print(f"📊 [美图轮询] 任务 {task.id} 找到结果URL: {result_url}")
                                task.status = 'success'
                                task.result_image_url = result_url
                                # 更新response_data为完整的查询响应
                                if task.response_data:
                                    try:
                                        original_response = json.loads(task.response_data) if isinstance(task.response_data, str) else task.response_data
                                        if isinstance(original_response, dict):
                                            original_response['query_response'] = result
                                            task.response_data = json.dumps(original_response, ensure_ascii=False)
                                        else:
                                ...
                                if task.created_at:
                                    ...
                                    print(f"⏱️ [美图轮询] 任务 {task.id} 总处理时间: {total_duration_ms}ms ({total_duration_seconds:.2f}秒)")
                                ...
                                try:
                                    ...
                                    if local_path:
                                        ...
                                        print(f"✅ [美图轮询] 任务 {task.id} 结果图已下载到本地: {local_path}")
                                except Exception as download_error:
                                    print(f"⚠️ [美图轮询] 下载图片失败: {str(download_error)}")
                                
                                db.session.commit()
                                updated_count += 1
                                print(f"✅ [美图轮询] 任务 {task.id} 状态已更新为成功，图片URL: {result_url}")
                            else:
                                # 没有找到结果URL，可能仍在处理中
                                # 检查是否有status字段
                                status = data.get('status', '')
                                if status == 'processing':
                                    # 仍在处理中，不更新状态，等待下次轮询
                                    age_seconds = (datetime.now() - task.created_at).total_seconds() if task.created_at else 0
                                    print(f"🔄 [美图轮询] 任务 {task.id} 仍在处理中 (已等待 {age_seconds:.1f}秒)")
                                elif status == 'failed' or status == 'error':
                                    # 任务失败
                                    error_msg = data.get('message') or data.get('error') or '任务失败'
                                    task.status = 'failed'
                                    ...
                                    if task.created_at:
                                        ...
                                        total_duration_ms = int(total_duration_seconds * 1000)
                                        task.duration_ms = total_duration_ms
                                        print(f"⏱️ [美图轮询] 任务 {task.id} 失败前总耗时: {total_duration_ms}ms ({total_duration_seconds:.2f}秒)")
                                    
                                    task.completed_at = datetime.now()
                                    db.session.commit()
                                    updated_count += 1
                                    print(f"✅ [美图轮询] 任务 {task.id} 状态已更新为失败: {error_msg}")
                                else:
                                    # 未知状态，输出详细信息用于调试
                                    print(f"⚠️ [美图轮询] 任务 {task.id} 未找到结果URL，状态: {status if status else '无status字段'}, 完整响应: {json.dumps(result, ensure_ascii=False)[:300]}")
                        else:
                            # API返回错误
                            error_msg = result.get('message', '查询失败')
                            print(f"⚠️ [美图轮询] 任务 {task.id} 查询失败: {error_msg}")
                    else:
                        print(f"⚠️ [美图轮询] 任务 {task.id} 查询请求失败: HTTP {response.status_code}")
                        
                except Exception as e:
                    print(f"⚠️ [美图轮询] 处理任务 {task.id} 时出错: {str(e)}")
                    import traceback
                    traceback.print_exc()
                    continue
        ...
        print(f"❌ [美图轮询] 轮询美图API任务失败: {str(e)}")
        import traceback
        traceback.print_exc()
        return 0
...
def poll_processing_tasks():
    ...
    try:
        import sys
        if 'test_server' not in sys.modules:
            print("⚠️ [轮询] test_server模块未加载，无法轮询")
            return 0
        ...
        with test_server_module.app.app_context():
            ...
            # 过滤掉过旧的任务（超过20分钟）
            processing_tasks = [t for t in processing_tasks if t.created_at and t.created_at >= max_age_cutoff]
            
            # 添加调试日志：显示找到的任务数量
            if processing_tasks:
                print(f"🔍 [轮询] 找到 {len(processing_tasks)} 个待轮询任务（状态为pending或processing，创建时间超过30秒且不超过20分钟）")
                for t in processing_tasks[:5]:  # 只显示前5个
                    ...
                    print(f"   - 任务 {t.id} (order: {t.order_number}): 状态={t.status}, 创建于{age_minutes:.1f}分钟前, comfyui_prompt_id={t.comfyui_prompt_id}, notes={notes_preview}")
            
            # 检查是否有过旧的任务需要清理（超过15分钟仍为processing状态）
            old_tasks = AITask.query.filter(
                AITask.status.in_(['pending', 'processing']),
                AITask.created_at < max_age_cutoff
            ).all()
            if old_tasks:
                print(f"⚠️ [轮询] 发现 {len(old_tasks)} 个过旧的任务（超过20分钟仍为processing/pending），自动清理...")
                for t in old_tasks:
                    age_minutes = (datetime.now() - t.created_at).total_seconds() / 60 if t.created_at else 0
                    print(f"   - 任务 {t.id} (order: {t.order_number}): 状态={t.status}, 创建于{age_minutes:.1f}分钟前，标记为失败")
                    # 自动标记为失败
                    t.status = 'failed'
                    t.error_message = f"任务超时：超过{max_age_minutes}分钟仍未完成，已自动清理"
                    t.completed_at = datetime.now()
                db.session.commit()
                print(f"✅ [轮询] 已自动清理 {len(old_tasks)} 个过旧任务")
                    ...
                    try:
                        ...
                        if isinstance(parsed_log, dict):
                            ...
                            if api_config_id:
                                ...
                                if api_config and hasattr(api_config, 'is_sync_api') and api_config.is_sync_api:
                                    is_sync = True
                                    # 同步API任务如果长时间处于processing状态，可能是连接断开导致
                                    # 检查任务创建时间，如果超过10分钟还是processing，标记为失败
                                    # 注意：同步API的read_timeout是8分钟（480秒），加上2分钟缓冲，总共10分钟
                                    task_age = (datetime.now() - task.created_at).total_seconds() if task.created_at else 0
                                    if task_age > 600:  # 10分钟（与read_timeout 8分钟 + 2分钟缓冲一致）
                                        print(f"⚠️ [轮询] 任务 {task.id} 是同步API任务，已超过10分钟仍为processing状态，可能是连接断开，标记为失败")
                                        task.status = 'failed'
                                        task.error_message = "同步API任务超时：可能连接断开，未收到响应（已等待10分钟）"
                                        db.session.commit()
                                    else:
                                        print(f"⚠️ [轮询] 任务 {task.id} 是同步API任务，跳过轮询（同步API应该一次性返回结果，当前已等待{task_age:.1f}秒，最多等待10分钟）")
                        elif isinstance(parsed_log, list):
                            print(f"⚠️ [轮询] 任务 {task.id} 的 processing_log 是 list 类型，跳过同步API检查")
                    except Exception as e:
                        print(f"⚠️ [轮询] 检查任务 {task.id} 是否为同步API时出错: {str(e)}")
            ...
            # 添加调试日志：显示过滤后的任务数量
            if filtered_tasks:
                print(f"🔍 [轮询] 过滤后剩余 {len(filtered_tasks)} 个异步任务（已排除同步API任务）")
                ...
                try:
                    ...
                    if task.comfyui_prompt_id and task.workflow_file:
                        ...
                        if not has_api_config:
                            ...
                            print(f"🔍 [轮询] 任务 {task.id} 是本地ComfyUI任务（prompt_id={task.comfyui_prompt_id}, workflow_file={task.workflow_file}），将查询ComfyUI history API")
                    ...
                    if is_local_comfyui_task:
                        try:
                            ...
                            if not prompt_id or not output_id:
                                print(f"⚠️ [轮询] 任务 {task.id} 缺少 prompt_id 或 output_id，跳过")
                                continue
                            
                            # 查询ComfyUI history API
                            history_url = f"{comfyui_config['base_url']}/history/{prompt_id}"
                            print(f"🔄 [轮询] 查询ComfyUI任务状态: {history_url}")
                                    ...
                                    if output_id in outputs:
                                        ...
                                        if images and len(images) > 0:
                                            ...
                                            try:
                                                ...
                                                local_path = download_api_result_image(image_url, prompt_id, test_server_module.app)
                                                if local_path:
                                                    task.output_image_path = local_path
                                                    print(f"✅ [轮询] ComfyUI任务 {task.id} 结果图已下载到本地: {local_path}")
                                                    ...
                                                    try:
                                                        ...
                                                        if thumbnail_path:
                                                            print(f"✅ [轮询] ComfyUI任务 {task.id} 缩略图生成成功: {thumbnail_path}")
                                                    except Exception as thumb_error:
                                                        print(f"⚠️ [轮询] ComfyUI任务 {task.id} 生成缩略图失败: {str(thumb_error)}")
                                            except Exception as download_error:
                                                print(f"⚠️ [轮询] 下载ComfyUI结果图失败: {str(download_error)}")
                                                    ...
                                                    if Order:
                                                        ...
                                                        if len(valid_tasks) > 0 and len(completed_tasks) == len(valid_tasks):
                                                            ...
                                                            if order and order.status in ['ai_processing', 'retouching', 'shooting', 'processing']:
                                                                ...
                                                                print(f"✅ [轮询] 订单 {order.order_number} 所有AI任务已完成 ({len(completed_tasks)}/{len(valid_tasks)})，状态已更新为: pending_selection (从 {old_status} 更新)")
                                                            elif order:
                                                                print(f"ℹ️ [轮询] 订单 {order.order_number} 所有AI任务已完成，但当前状态是 {order.status}，不更新")
                                                except Exception as e:
                                                    print(f"⚠️ [轮询] 检查订单状态失败: {str(e)}")
                                                    import traceback
                                                    traceback.print_exc()
                                            
                                            db.session.commit()
                                            updated_count += 1
                                            print(f"✅ [轮询] ComfyUI任务 {task.id} 已完成，图片URL: {image_url}")
                                            continue
                                        else:
                                            # 任务仍在处理中
                                            print(f"⏳ [轮询] ComfyUI任务 {task.id} 仍在处理中（输出节点 {output_id} 还没有图片）")
                                            continue
                                    else:
                                        # 输出节点还没有结果
                                        print(f"⏳ [轮询] ComfyUI任务 {task.id} 仍在处理中（输出节点 {output_id} 不存在）")
                                        continue
                                else:
                                    # prompt_id不在history中，可能任务还在队列中
                                    print(f"⏳ [轮询] ComfyUI任务 {task.id} 仍在队列中（history中未找到）")
                                    continue
                            else:
                                print(f"⚠️ [轮询] 查询ComfyUI history失败: HTTP {response.status_code}")
                                continue
                        except Exception as e:
                            print(f"⚠️ [轮询] 处理ComfyUI任务 {task.id} 时出错: {str(e)}")
                            import traceback
                            traceback.print_exc()
                            continue
                    ...
                    if task.notes and 'T8_API_TASK_ID:' in task.notes:
                        try:
                            ...
                            if notes_task_id:
                                ...
                                if task.comfyui_prompt_id and task.comfyui_prompt_id != notes_task_id:
                                    ...
                                    if len(task.comfyui_prompt_id) > len(notes_task_id) or task.comfyui_prompt_id.startswith('b1f3b4f8'):
                                        print(f"⚠️ [轮询] 任务 {task.id} notes中的ID({notes_task_id})与comfyui_prompt_id({task.comfyui_prompt_id})不一致，使用comfyui_prompt_id（可能重试后未更新notes）")
                                        api_task_id = task.comfyui_prompt_id
                                    else:
                                        api_task_id = notes_task_id
                                        print(f"✅ [轮询] 从notes中提取到T8_API_TASK_ID: {api_task_id}（优先使用notes中的）")
                                else:
                                    api_task_id = notes_task_id
                                    print(f"✅ [轮询] 从notes中提取到T8_API_TASK_ID: {api_task_id}（优先使用notes中的）")
                        except Exception as e:
                            print(f"⚠️ 解析任务 {task.id} 的notes中的T8_API_TASK_ID失败: {str(e)}")
                    ...
                    if not api_task_id and task.processing_log:
                        try:
                            ...
                            if isinstance(parsed_log, dict):
                                api_info = parsed_log
                                api_task_id = api_info.get('task_id') or api_info.get('api_task_id') or api_info.get('id')
                        except:
                            pass
                    
                    if not api_task_id:
                        print(f"⚠️ [轮询] 任务 {task.id} (order_number: {task.order_number}) 没有API任务ID，跳过轮询")
                        print(f"   - comfyui_prompt_id: {task.comfyui_prompt_id}")
                        print(f"   - notes: {task.notes[:100] if task.notes else 'None'}")
                        if task.processing_log:
                            try:
                                parsed_log = json.loads(task.processing_log)
                                # 检查是否是字典类型
                                if isinstance(parsed_log, dict):
                                    ...
                                    print(f"   - processing_log中的api_config_id: {api_info.get('api_config_id')}")
                                elif isinstance(parsed_log, list):
                                    print(f"   - processing_log是list类型，无法提取信息")
                            except:
                                pass
                        continue
                    
                    print(f"🔄 [轮询] 开始轮询任务 {task.id} (order: {task.order_number}), API任务ID: {api_task_id}")
                    ...
                    if result_endpoint and '{task_id}' in result_endpoint:
                        ...
                        print(f"📝 [轮询] 替换result_endpoint中的{{task_id}}占位符: {result_endpoint}", flush=True)
                    ...
                    if result_endpoint and is_t8star and api_config.api_type == 'nano-banana-edits':
                        ...
                        if '/v1/images/edits/result' in result_endpoint or result_endpoint.endswith('/edits/result'):
                            ...
                            print(f"📝 [轮询] T8Star result_endpoint格式不正确，自动修正为: {result_endpoint}", flush=True)
                        elif '/v1/images/tasks/' not in result_endpoint:
                            # 如果result_endpoint不是/v1/images/tasks/格式，也修正
                            result_endpoint = f'/v1/images/tasks/{api_task_id}'
                            print(f"📝 [轮询] T8Star result_endpoint不是OpenAPI格式，自动修正为: {result_endpoint}", flush=True)
                    ...
                    if api_config.api_type in ['runninghub-rhart-edit', 'runninghub-comfyui-workflow']:
                        ...
                        if not result_endpoint:
                            ...
                            if '/v1/images/generations' in draw_endpoint or '/v1/images/tasks/' in draw_endpoint:
                                ...
                                print(f"📝 [轮询] T8Star nano-banana-edits异步模式：使用OpenAPI格式查询端点 GET /v1/images/tasks/{api_task_id}", flush=True)
                    ...
                    try:
                        if api_config.api_type in ['runninghub-rhart-edit', 'runninghub-comfyui-workflow']:
                            ...
                            use_new_query_format = result_endpoint and '/openapi/v2/query' in result_endpoint
                            ...
                            if use_new_query_format:
                                ...
                                print(f"📤 [轮询] RunningHub 使用新格式查询接口 /openapi/v2/query，请求体: {json.dumps(request_payload, ensure_ascii=False)}")
                            else:
                                # 旧格式：/task/openapi/outputs，请求体需要 apiKey 和 taskId
                                request_payload = {
                                ...
                                print(f"📤 [轮询] RunningHub 使用旧格式查询接口 /task/openapi/outputs，请求体: {json.dumps(request_payload, ensure_ascii=False)}")
                            ...
                            print(f"📥 [轮询] RunningHub API 响应状态码: {response.status_code}")
                            if response.status_code == 200:
                                try:
                                    result_data = response.json()
                                    print(f"📥 [轮询] RunningHub API 响应内容: {json.dumps(result_data, ensure_ascii=False)[:500]}")
                                except Exception as e:
                                    print(f"⚠️ [轮询] RunningHub API 响应解析失败: {str(e)}")
                                    result_data = None
                            else:
                                print(f"⚠️ [轮询] RunningHub API 请求失败，状态码: {response.status_code}")
                                try:
                                    result_data = response.json()  # 即使状态码不是200，也尝试解析响应
                                    print(f"📥 [轮询] RunningHub API 错误响应内容: {json.dumps(result_data, ensure_ascii=False)[:500]}")
                                except:
                                    result_data = None
                        elif use_get_method:
                            ...
                            if response.status_code == 200:
                                try:
                                    ...
                                    print(f"⚠️ [轮询] GET请求响应解析失败: {str(e)}")
                                    result_data = None
                            else:
                                print(f"⚠️ [轮询] GET请求失败，状态码: {response.status_code}")
                                try:
                                    result_data = response.json()  # 即使状态码不是200，也尝试解析响应
                                except:
                                    result_data = None
                            ...
                            request_payloads = [
                                {"Id": api_task_id},  # 参考bk-photo-v4：通用异步API格式（大写Id，优先）
                            ...
                            for payload in request_payloads:
                                try:
                                    print(f"📤 [轮询] POST请求，尝试参数格式: {list(payload.keys())[0]}")
                                    ...
                                    if response.status_code == 200:
                                        ...
                                        if isinstance(result_data, dict):
                                            ...
                                            if 'status' in result_data or result_data.get('code') == 0:
                                                payload_used = payload
                                                print(f"✅ [轮询] 使用参数格式 {list(payload.keys())[0]} 查询成功")
                                                break
                                            elif result_data.get('code') == -22:
                                                # 任务不存在，尝试下一个格式
                                                print(f"⚠️ [轮询] 参数格式 {list(payload.keys())[0]} 返回code=-22，尝试下一个格式")
                                                continue
                                        else:
                                            # 非字典格式，视为成功
                                            payload_used = payload
                                            print(f"✅ [轮询] 使用参数格式 {list(payload.keys())[0]} 查询成功（非字典格式）")
                                            break
                                    else:
                                        print(f"⚠️ [轮询] HTTP错误: {response.status_code}，尝试下一个参数格式")
                                        continue
                                except Exception as e:
                                    print(f"⚠️ [轮询] 请求异常: {str(e)}，尝试下一个参数格式")
                                    continue
                            ...
                            if not result_data:
                                print(f"❌ [轮询] 任务 {task.id} 所有请求体格式都失败，无法获取结果")
                                continue
                        ...
                        if result_data:
                            ...
                            if api_config.api_type in ['runninghub-rhart-edit', 'runninghub-comfyui-workflow']:
                                ...
                                if use_new_query_format:
                                    ...
                                    status = result_data.get('status', '').upper() if result_data.get('status') else ''
                                    ...
                                    if not status or status == '':
                                        ...
                                        if error_code or error_message:
                                            ...
                                            print(f"⚠️ [轮询] RunningHub API 返回错误但 status 为空，识别为失败: errorCode={error_code}, errorMessage={error_message}")
                                    ...
                                    if status == 'SUCCESS':
                                        ...
                                        if isinstance(results, list) and len(results) > 0:
                                            ...
                                            print(f"✅ [轮询] RunningHub 任务 {api_task_id} 成功，图片URL: {image_url}")
                                        else:
                                            print(f"⚠️ [轮询] RunningHub 任务 {api_task_id} 状态为SUCCESS但没有results")
                                    elif status in ['RUNNING', 'QUEUED']:
                                        # 处理中或排队中
                                        print(f"🔄 [轮询] RunningHub 任务 {api_task_id} 状态: {status}")
                                    elif status == 'FAILED':
                                        # 失败：提取错误信息
                                        error_msg = result_data.get('errorMessage', '任务失败')
                                        if not error_msg:
                                            error_code = result_data.get('errorCode')
                                            error_msg = f"API错误 (errorCode={error_code})" if error_code else '任务失败'
                                        status = 'failed'  # 关键修复：确保 status 设置为 'failed'，以便后续逻辑正确处理
                                        print(f"❌ [轮询] RunningHub 任务 {api_task_id} 失败: {error_msg}")
                                        # 关键修复：不立即更新任务状态，让后续的重试逻辑处理
                                        # 保存错误信息到 error_msg 变量，供后续逻辑使用（在 elif status in ['failed', 'error']: 块中处理）
                                    else:
                                        print(f"⚠️ [轮询] RunningHub 任务 {api_task_id} 未知状态: {status}")
                                else:
                                    ...
                                    if response_code == 0:
                                        ...
                                        if isinstance(data, list) and len(data) > 0:
                                            ...
                                            if isinstance(results, list) and len(results) > 0:
                                                image_url = results[0].get('url') or results[0].get('fileUrl')
                                        ...
                                        status = 'RUNNING'
                                        print(f"🔄 [轮询] RunningHub 任务 {api_task_id} 正在运行中")
                                    elif response_code == 813:
                                        # 排队中 (APIKEY_TASK_IS_QUEUED)
                                        status = 'QUEUED'
                                        print(f"⏳ [轮询] RunningHub 任务 {api_task_id} 正在排队中")
                                    elif response_code == 805:
                                        # 失败，返回失败原因
                                        status = 'FAILED'
                                        error_msg = result_data.get('msg', '')
                                        failed_reason = result_data.get('data', {}).get('failedReason', {})
                                        ...
                                        if failed_reason:
                                            ...
                                            if exception_message:
                                                error_details += f" | 错误信息: {exception_message[:200]}"
                                        print(f"❌ [轮询] RunningHub 任务 {api_task_id} 失败: {error_details}")
                                    else:
                                        # 其他错误响应
                                        error_code = result_data.get('code')
                                        error_msg = result_data.get('msg', '')
                                        print(f"⚠️ [轮询] RunningHub 查询任务 {api_task_id} 返回未知状态: code={error_code}, msg={error_msg}")
                                    ...
                                    if result_data.get('code') == 0 and 'data' in result_data:
                                        ...
                                        if isinstance(data, dict):
                                            ...
                                            if status == 'failed':
                                                ...
                                                print(f"⚠️ [轮询] GRSAI任务失败（code=0但status=failed），错误信息: {error_msg}")
                                            ...
                                            if isinstance(inner_data, dict) and 'data' in inner_data:
                                                if isinstance(inner_data.get('data'), list) and len(inner_data.get('data')) > 0:
                                                    ...
                                                    if isinstance(first_item, dict):
                                                        ...
                                                        print(f"✅ [轮询] T8Star从三层嵌套格式（data.data.data数组）提取图片URL: {image_url}")
                                            # 备选：data.data 是数组（两层嵌套格式，可能某些情况下存在）
                                            elif isinstance(inner_data, list) and len(inner_data) > 0:
                                                first_item = inner_data[0]
                                                if isinstance(first_item, dict):
                                                    image_url = first_item.get('url')
                                                    print(f"✅ [轮询] T8Star从两层嵌套格式（data.data数组）提取图片URL: {image_url}")
                                        # 如果还是没有，尝试从data直接获取
                                        if not image_url:
                                            image_url = data.get('url') or data.get('image_url')
                                            if image_url:
                                                print(f"✅ [轮询] T8Star从data字段提取图片URL: {image_url}")
                                        # 状态映射：T8Star返回"SUCCESS"，需要映射为"completed"
                                        if status == 'SUCCESS':
                                            status = 'completed'
                                            print(f"✅ [轮询] T8Star任务状态为SUCCESS，映射为completed")
                                        elif status == 'FAILED':
                                            status = 'failed'
                                            print(f"❌ [轮询] T8Star任务状态为FAILED，映射为failed")
                                        elif status in ['PROCESSING', 'PENDING', 'QUEUED', 'RUNNING']:
                                            status = 'processing'
                                            print(f"🔄 [轮询] T8Star任务状态为{status}，映射为processing")
                                # 标准格式
                                    ...
                                    if estimated_time_from_api:
                                        try:
                                            ...
                                            if estimated_time:
                                                ...
                                                print(f"📅 [轮询] RunningHub 任务 {api_task_id} 预计完成时间（来自API）: {estimated_time.strftime('%Y-%m-%d %H:%M:%S')}")
                                                db.session.commit()
                                        except Exception as e:
                                            print(f"⚠️ [轮询] 解析API返回的预计完成时间失败: {str(e)}")
                                    else:
                                        # 如果API没有返回预计完成时间，打印调试信息
                                        print(f"🔍 [轮询] RunningHub 任务 {api_task_id} API响应中未找到预计完成时间字段，响应字段: {list(result_data.keys())}")
                                        if result_data.get('data') and isinstance(result_data.get('data'), dict):
                                            print(f"🔍 [轮询] data字段中的键: {list(result_data.get('data').keys())}")
                            
                            # 更新任务状态
                            # 关键修复：添加调试日志，确保status变量正确传递
                            if result_data:
                                print(f"🔍 [轮询] 任务 {task.id} 解析后的状态: status={status}, image_url={image_url if image_url else 'None'}, result_data keys={list(result_data.keys()) if isinstance(result_data, dict) else 'N/A'}")
                            else:
                                print(f"⚠️ [轮询] 任务 {task.id} result_data 为空，无法解析状态")
                            ...
                            if status in ['succeeded', 'completed', 'success']:
                                if not image_url:
                                    ...
                                    print(f"⚠️ [轮询] 任务 {task.id} 状态为{status}但没有图片URL，尝试重新提取")
                                    if isinstance(result_data, dict):
                                        if 'data' in result_data:
                                            data = result_data.get('data')
                                            if isinstance(data, dict):
                                                ...
                                                if not image_url:
                                                    ...
                                                    if isinstance(results, list) and len(results) > 0:
                                                        image_url = results[0].get('url') or results[0].get('image_url')
                                                if image_url:
                                                    print(f"✅ [轮询] 重新提取到图片URL: {image_url}")
                            ...
                            if status in ['succeeded', 'completed', 'success'] and image_url:
                                ...
                                if task.order_id and task.order_id > 0:
                                    try:
                                        ...
                                        print(f"⚠️ 检查订单状态失败: {str(e)}")
                                        import traceback
                                        traceback.print_exc()
                                
                                db.session.commit()  # 提交时包含订单状态更新
                                updated_count += 1
                                print(f"✅ 后台轮询：任务 {task.id} 状态已更新为已完成，图片URL: {image_url}")
                                
                                # 自动下载图片（download_api_result_image内部已包含缩略图生成）
                                try:
                                    from app.routes.ai import download_api_result_image
                                    local_path = download_api_result_image(image_url, task.comfyui_prompt_id or str(task.id), test_server_module.app)
                                    if local_path:
                                        task.output_image_path = local_path
                                        db.session.commit()
                                        print(f"✅ 任务 {task.id} 结果图已下载到本地: {local_path} (缩略图已自动生成)")
                                except Exception as download_error:
                                ...
                                print(f"🔍 [轮询] 任务 {task.id} 检测到失败状态: status={status}")
                                error_msg = None
                                ...
                                if isinstance(result_data, dict):
                                    ...
                                    if 'errorMessage' in result_data:
                                        ...
                                        print(f"🔍 [轮询] 从 RunningHub errorMessage 字段提取错误信息: {error_msg}")
                                    
                                    # 检查data字段中的error（GRSAI格式，优先，因为GRSAI的错误信息在这里）
                                    if not error_msg and 'data' in result_data and isinstance(result_data.get('data'), dict):
                                        data = result_data.get('data')
                                        # 关键修复：即使code=0，如果status是failed，也要提取错误信息
                                        if data.get('status') == 'failed':
                                            error_msg = data.get('error') or data.get('error_message') or data.get('failure_reason')
                                            print(f"🔍 [轮询] 从data字段提取错误信息: {error_msg}")
                                ...
                                if current_api_config_id:
                                    ...
                                    if api_info.get('should_not_retry') or api_info.get('connection_closed_but_request_sent'):
                                        print(f"⚠️ [自动重试] 任务 {task.id} 标记为不应重试（连接断开但请求可能已发送），跳过自动重试")
                                        should_retry = False
                                    else:
                                        # 检查当前API配置是否启用了重试
                                        current_api_config = APIProviderConfig.query.get(current_api_config_id)
                                        if current_api_config and current_api_config.enable_retry:
                                            # 关键修复：禁止SSL和UNIR级别的重试
                                            config_name_upper = current_api_config.name.upper() if current_api_config.name else ''
                                            if 'SSL' in config_name_upper or 'UNIR' in config_name_upper:
                                                print(f"⚠️ [自动重试] 任务 {task.id} 当前配置是SSL/UNIR级别，禁止重试: {current_api_config.name}")
                                                should_retry = False
                                            else:
                                                # 获取已尝试的API配置ID列表
                                                retried_ids = api_info.get('retried_api_config_ids', [])
                                                if not isinstance(retried_ids, list):
                                                    retried_ids = []
                                                
                                                # 关键修复：检查当前配置是否已经重试过（一个服务商仅重试一次）
                                                if current_api_config_id in retried_ids:
                                                    print(f"⚠️ [自动重试] 任务 {task.id} 当前配置 {current_api_config.name} (ID: {current_api_config_id}) 已经重试过，不再重试")
                                                    should_retry = False
                                                else:
                                                    # 检查重试次数（最多重试3次）
                                                    max_retry_count = 3
                                                    if task.retry_count < max_retry_count:
                                                        # 获取下一个可用的API配置
                                                        from app.services.ai_provider_service import get_next_retry_api_config
                                                        ...
                                                        if next_api_config:
                                                            ...
                                                            if next_api_config.id in retried_ids:
                                                                print(f"⚠️ [自动重试] 任务 {task.id} 下一个配置 {next_api_config.name} (ID: {next_api_config.id}) 已经重试过，跳过")
                                                                should_retry = False
                                                            else:
                                                                should_retry = True
                                                                print(f"🔄 [自动重试] 任务 {task.id} 失败，将使用下一个API配置重试")
                                                                print(f"   - 当前配置: {current_api_config.name} (ID: {current_api_config_id})")
                                                                print(f"   - 下一个配置: {next_api_config.name} (ID: {next_api_config.id})")
                                                                print(f"   - 已尝试的配置: {retried_ids}")
                                                                print(f"   - 当前重试次数: {task.retry_count}/{max_retry_count}")
                                ...
                                if should_retry and next_api_config:
                                    ...
                                    try:
                                        ...
                                        original_uploaded_images = api_info.get('uploaded_images', [])
                                        ...
                                        if not style_image_id:
                                            print(f"⚠️ [自动重试] 任务 {task.id} 没有style_image_id，无法重试")
                                            should_retry = False
                                        else:
                                            # 更新已尝试的API配置ID列表
                                            ...
                                            if task.notes:
                                                ...
                                                if "【自动重试" in task.notes:
                                                    ...
                                                    print(f"✅ [自动重试] 已追加重试记录到notes: {retry_note}")
                                                else:
                                                    # 如果没有重试记录，添加新的
                                                    task.notes = f"{task.notes}\n{retry_note}"
                                                    print(f"✅ [自动重试] 已添加重试记录到notes: {retry_note}")
                                            else:
                                                task.notes = retry_note
                                                print(f"✅ [自动重试] 已创建notes并添加重试记录: {retry_note}")
                                            
                                            # 打印当前notes内容用于调试
                                            print(f"📝 [自动重试] 任务 {task.id} 当前notes内容: {task.notes}")
                                            ...
                                            print(f"🔄 [自动重试] 任务 {task.id} 已重置为pending状态，准备使用新配置 {next_api_config.name} 重试")
                                            print(f"   - 原始参数: prompt={original_prompt[:50]}, size={original_image_size}, ratio={original_aspect_ratio}")
                                            print(f"   - 图片数量: {len(original_uploaded_images) if original_uploaded_images else 0}")
                                            ...
                                            retry_success, retry_task, retry_error = create_api_task(
                                            ...
                                            if retry_success and retry_task:
                                                ...
                                                if retry_task.processing_log:
                                                    try:
                                                        retry_api_info = json.loads(retry_task.processing_log)
                                                        # 更新当前任务的API信息
                                                        new_api_task_id = retry_task.comfyui_prompt_id
                                                        ...
                                                        if new_api_task_id:
                                                            ...
                                                            if task.notes and 'T8_API_TASK_ID:' in task.notes:
                                                                ...
                                                                print(f"✅ [自动重试] 已更新notes中的T8_API_TASK_ID（保留重试记录）: {old_notes} -> {task.notes}")
                                                            else:
                                                                ...
                                                                print(f"✅ [自动重试] 已添加notes中的T8_API_TASK_ID: {new_api_task_id}")
                                                            print(f"📝 [自动重试] 任务 {task.id} 更新后的notes内容: {task.notes}")
                                                            # 确保立即刷新，让数据库更新生效
                                                            db.session.flush()
                                                        
                                                        # 合并processing_log
                                                        api_info.update({
                                                            'api_task_id': retry_api_info.get('api_task_id') or new_api_task_id,
                                                            'task_id': retry_api_info.get('task_id') or new_api_task_id,
                                                            'id': retry_api_info.get('id') or new_api_task_id,
                                                            'original_response': retry_api_info.get('original_response'),
                                                            'response_data': retry_api_info.get('response_data'),
                                                            'response_status': retry_api_info.get('response_status'),
                                                        })
                                                        task.processing_log = json.dumps(api_info, ensure_ascii=False)
                                                        ...
                                                        print(f"✅ [自动重试] 任务 {task.id} 已使用新配置 {next_api_config.name} 重新提交，API任务ID: {task.comfyui_prompt_id}")
                                                        print(f"   - notes字段: {task.notes[:100] if task.notes else 'None'}")
                                                        print(f"   - comfyui_prompt_id: {task.comfyui_prompt_id}")
                                                        updated_count += 1
                                                    except Exception as merge_error:
                                                        print(f"⚠️ [自动重试] 合并任务信息失败: {str(merge_error)}")
                                                    ...
                                                    task.status = 'failed'
                                                    task.error_message = f"自动重试失败: 新任务没有processing_log"
                                                    task.completed_at = datetime.now()
                                                    db.session.commit()
                                                    print(f"❌ [自动重试] 任务 {task.id} 重试失败: 新任务没有processing_log")
                                                    updated_count += 1
                                            else:
                                                # 重试创建任务失败，标记原任务为最终失败
                                                task.status = 'failed'
                                                task.error_message = f"自动重试失败: {retry_error or '未知错误'}"
                                                task.completed_at = datetime.now()
                                                db.session.commit()
                                                print(f"❌ [自动重试] 任务 {task.id} 重试创建失败: {retry_error}")
                                                updated_count += 1
                                    except Exception as retry_error:
                                        # 重试过程中出错，标记任务为最终失败
                                        import traceback
                                        error_trace = traceback.format_exc()
                                        print(f"❌ [自动重试] 任务 {task.id} 重试过程出错: {str(retry_error)}")
                                        print(error_trace)
                                        
                                        task.status = 'failed'
                                        task.error_message = f"自动重试失败: {str(retry_error)[:200]}"
                                        task.completed_at = datetime.now()
                                        db.session.commit()
                                        updated_count += 1
                                ...
                                if not should_retry:
                                    ...
                                    db.session.commit()
                                    updated_count += 1
                                    print(f"✅ 后台轮询：任务 {task.id} 状态已更新为失败，错误信息: {error_msg}")
                                
                    except Exception as e:
                        print(f"⚠️ 轮询任务 {task.id} 状态失败: {str(e)}")
                        continue
                        
                except Exception as e:
                    print(f"⚠️ 处理任务 {task.id} 时出错: {str(e)}")
                    continue
            
            return updated_count
            
    except Exception as e:
        print(f"❌ 轮询处理中的任务失败: {str(e)}")
        import traceback
        traceback.print_exc()
        return 0
