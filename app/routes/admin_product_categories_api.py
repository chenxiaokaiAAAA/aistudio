...
def admin_get_categories():
    ...
    try:
        ...
        if not ProductCategory:
            return (
                jsonify(
                    {"status": "error", "message": "产品分类模型未找到，请先运行数据库迁移脚本"}
                ),
                500,
            )
            ...
            result.append(
                {
                    ...
                    "style_redirect_page": (
                        category.style_redirect_page
                        if category.style_redirect_page is not None
                        else ""
                    ),  # 确保 None 转换为空字符串
...
@admin_product_categories_api_bp.route(
    "/api/admin/product-categories/upload-image", methods=["POST"]
)
@login_required
def admin_upload_category_image():
    ...
    try:
        ...
        static_categories_dir = os.path.join(
            current_app.root_path, "static", "images", "categories"
        )
        ...
        return jsonify(
            {"status": "success", "message": "图片上传成功", "data": {"image_url": image_url}}
        )

    except Exception as e:
        logger.info(f"上传分类图片失败: {str(e)}")
        import traceback
...
def admin_create_category():
    ...
    try:
        ...
        if not ProductCategory:
            return jsonify({"status": "error", "message": "产品分类模型未找到"}), 500
        ...
        # 使产品分类缓存失效
        try:
            from app.services.cache_service import CACHE_PREFIXES, invalidate_cache

            invalidate_cache(CACHE_PREFIXES["PRODUCT_CATEGORIES"])
            logger.info("产品分类缓存已失效")
        except Exception as e:
            logger.warning(f"失效产品分类缓存失败: {e}")
        ...
        logger.info(f"创建分类失败: {str(e)}")
        import traceback

        traceback.print_exc()
        db.session.rollback()
        return jsonify({"status": "error", "message": f"创建分类失败: {str(e)}"}), 500


@admin_product_categories_api_bp.route(
    "/api/admin/product-categories/<int:category_id>", methods=["GET"]
)
@login_required
...
@admin_product_categories_api_bp.route(
    "/api/admin/product-categories/<int:category_id>", methods=["PUT"]
)
@login_required
def admin_update_category(category_id):
    ...
    try:
        ...
        logger.info(f"更新分类 {category_id}，接收到的数据: {data}")  # 调试日志
        logger.info(f"接收到的数据键列表: {list(data.keys()) if data else 'None'}")  # 调试日志
        logger.info(
            f"是否包含 style_redirect_page: {'style_redirect_page' in data if data else False}"
        )  # 调试日志
        if data and "style_redirect_page" in data:
            logger.info(
                f"style_redirect_page 的值: {repr(data.get('style_redirect_page'))}"
            )  # 调试日志
        ...
        if "style_redirect_page" in data:
            ...
            if style_redirect_value is None:
                ...
                category.style_redirect_page = None
            logger.info(
                f"设置 style_redirect_page: {repr(style_redirect_value)} -> {repr(category.style_redirect_page)}"
            )  # 调试日志
        else:
            # 如果没有传递该字段，保持原值不变
            logger.info(
                f"未传递 style_redirect_page 字段，保持原值: {category.style_redirect_page}"
            )  # 调试日志
        ...
        logger.info(f"保存后的 style_redirect_page 值: {category.style_redirect_page}")  # 调试日志
        ...
        logger.info(f"更新分类失败: {str(e)}")
        import traceback

        traceback.print_exc()
        db.session.rollback()
        return jsonify({"status": "error", "message": f"更新分类失败: {str(e)}"}), 500


@admin_product_categories_api_bp.route(
    "/api/admin/product-categories/<int:category_id>", methods=["DELETE"]
)
@login_required
def admin_delete_category(category_id):
    ...
    try:
        ...
        if hasattr(category, "products") and category.products:
            return jsonify({"status": "error", "message": "该分类下还有产品，无法删除"}), 400
        ...
        logger.info(f"删除分类失败: {str(e)}")
        import traceback

        traceback.print_exc()
        db.session.rollback()
        return jsonify({"status": "error", "message": f"删除分类失败: {str(e)}"}), 500
...
def admin_get_subcategories():
    ...
    try:
        ...
        if not ProductSubcategory:
            return jsonify({"status": "error", "message": "产品二级分类模型未找到"}), 500
        ...
        logger.info(f"获取二级分类失败: {str(e)}")
        import traceback

        traceback.print_exc()
        return jsonify({"status": "error", "message": "获取二级分类失败"}), 500
...
def admin_create_subcategory():
    ...
    try:
        ...
        existing = ProductSubcategory.query.filter_by(
            category_id=data["category_id"], code=data["code"]
        ).first()
        if existing:
            return (
                jsonify({"status": "error", "message": "该一级分类下已存在相同代码的二级分类"}),
                400,
            )
        ...
        return jsonify(
            {
                "status": "success",
                "message": "二级分类创建成功",
                "data": {"id": subcategory.id, "name": subcategory.name, "code": subcategory.code},
            }
        )

    except Exception as e:
        logger.info(f"创建二级分类失败: {str(e)}")
        import traceback

        traceback.print_exc()
        db.session.rollback()
        return jsonify({"status": "error", "message": f"创建二级分类失败: {str(e)}"}), 500


@admin_product_categories_api_bp.route(
    "/api/admin/product-categories/subcategories/<int:subcategory_id>", methods=["GET"]
)
@login_required
def admin_get_subcategory(subcategory_id):
    ...
    try:
        ...
        return jsonify(
            {
                ...
                "data": {
                    ...
                    "created_at": (
                        subcategory.created_at.isoformat() if subcategory.created_at else ""
                    ),
                },
            }
        )
        ...
        logger.info(f"获取二级分类详情失败: {str(e)}")
        import traceback

        traceback.print_exc()
        return jsonify({"status": "error", "message": f"获取二级分类详情失败: {str(e)}"}), 500


@admin_product_categories_api_bp.route(
    "/api/admin/product-subcategories/<int:subcategory_id>", methods=["PUT"]
)
@login_required
def admin_update_subcategory(subcategory_id):
    ...
    try:
        ...
        if "code" in data:
            ...
            existing = ProductSubcategory.query.filter_by(
                category_id=subcategory.category_id, code=data["code"]
            ).first()
        ...
        logger.info(f"更新二级分类失败: {str(e)}")
        import traceback

        traceback.print_exc()
        db.session.rollback()
        return jsonify({"status": "error", "message": f"更新二级分类失败: {str(e)}"}), 500


@admin_product_categories_api_bp.route(
    "/api/admin/product-subcategories/<int:subcategory_id>", methods=["DELETE"]
)
@login_required
def admin_delete_subcategory(subcategory_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "二级分类删除成功"})

    except Exception as e:
        logger.info(f"删除二级分类失败: {str(e)}")
        import traceback

        traceback.print_exc()
        db.session.rollback()
        return jsonify({"status": "error", "message": f"删除二级分类失败: {str(e)}"}), 500
