...
def get_access_token():
    ...
    try:
        ...
        if config:
            ...
            secret = config.get("app_secret", "3cdb890ade31e5673c88fbf1aa8a46d")
        else:
            # 如果配置未初始化，使用默认值
            appid = "wx01e841dfc50052a9"  # 您的AppID（与前端project.config.json保持一致）
            secret = "3cdb890ade31e5673c88fbf1aa8a46d"  # 您的AppSecret（需要与AppID匹配）
        ...
        if "access_token" in data:
            logger.info(f"获取access_token成功: {data['access_token'][:20]}...")
            return data["access_token"]
        else:
            logger.info(f"获取access_token失败: {data}")
            return None

    except Exception as e:
        logger.info(f"获取access_token异常: {e}")
        return None
...
def generate_qrcode():
    ...
    try:
        ...
        if not promotion_code or not user_id:
            return jsonify({"success": False, "message": "推广码和用户ID不能为空"}), 400

        logger.info(f"生成小程序码请求: promotionCode={promotion_code}, userId={user_id}")
        ...
        logger.info("=== 小程序码生成调试 ===")
        logger.info(f"原始promotionCode: {promotion_code}")
        logger.info(f"原始userId: {user_id}")
        logger.info(f"最终scene参数: {scene}")
        logger.info(f"scene参数长度: {len(scene)}")
        ...
        if response.status_code == 200:
            ...
            logger.info(f"✅ 小程序码保存成功: {filepath}")

            # 返回图片URL
            return jsonify(
                {"success": True, "qrcode_url": f"/static/qrcodes/{filename}", "filename": filename}
            )
        else:
            return (
                jsonify(
                    {"success": False, "message": f"生成小程序码失败: HTTP {response.status_code}"}
                ),
                500,
            )

    except Exception as e:
        logger.info(f"生成小程序码失败: {str(e)}")
        import traceback

        traceback.print_exc()
        return jsonify({"success": False, "message": f"生成小程序码失败: {str(e)}"}), 500
...
def generate_franchisee_qrcode():
    ...
    try:
        ...
        if not franchisee_id:
            return jsonify({"success": False, "message": "门店ID不能为空"}), 400
        ...
        logger.info(
            f"生成门店小程序码请求: franchiseeId={franchisee_id}, qrCode={franchisee.qr_code}"
        )
        ...
        logger.info("=== 门店小程序码生成调试 ===")
        logger.info(f"门店ID: {franchisee_id}")
        logger.info(f"门店二维码标识: {franchisee.qr_code}")
        logger.info(f"最终scene参数: {scene}")
        logger.info(f"scene参数长度: {len(scene)}")
        ...
        for attempt in attempts:
            ...
            logger.info(f"调用微信API生成门店小程序码 (版本: {env_version}, 页面: {page_desc})...")
            response = requests.post(url, json=params, timeout=(10, 30))
            ...
            if response.status_code == 200:
                ...
                if "application/json" in content_type:
                    ...
                    error_data = response.json()
                    logger.info(
                        f"微信API返回错误 (版本 {env_version}, 页面 {page_desc}): {error_data}"
                    )
                    last_error = error_data.get("errmsg", "未知错误")
                    # 如果是页面路径错误，继续尝试下一个配置
                    if (
                        "invalid page" in str(error_data).lower()
                        or "page" in str(error_data).lower()
                    ):
                        logger.info("页面路径错误，尝试下一个配置...")
                        continue
                    else:
                        # 其他错误，继续尝试
                        continue
                    ...
                    # 成功返回图片，跳出循环
                    success = True
                    logger.info(f"✅ 成功生成小程序码 (版本: {env_version}, 页面: {page_desc})")
                    break
            else:
        ...
        if not success or not response or response.status_code != 200:
            ...
            return jsonify({"success": False, "message": f"生成小程序码失败: {error_msg}"}), 500
        ...
        logger.info(f"✅ 门店小程序码保存成功: {filepath}")
        ...
        logger.info(f"生成门店小程序码失败: {str(e)}")
        import traceback

        traceback.print_exc()
        return jsonify({"success": False, "message": f"生成门店小程序码失败: {str(e)}"}), 500
