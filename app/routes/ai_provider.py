    ...
    try:
        ...
        try:
            ...
            if "is_sync_api" not in columns:
                logger.info("检测到 is_sync_api 字段不存在，执行数据库迁移...")
                try:
                    db.session.execute(
                        text(
                    ...
                    db.session.commit()
                    logger.info("is_sync_api 字段添加成功")
                    ...
                    db.session.commit()
                    logger.info("已根据 api_type 自动设置 is_sync_api 的值")
        ...
        try:
            ...
            if "is_sync_api" in str(query_error):
                logger.info("查询失败，检测到 is_sync_api 字段缺失，尝试修复...")
                try:
                    ...
                    logger.info(f"修复失败: {fix_error}")
                    raise query_error
            else:
                raise query_error

        return jsonify({"status": "success", "data": [config.to_dict() for config in configs]})

    except Exception as e:
        logger.info(f"获取API配置列表失败: {str(e)}")
        import traceback
...
def create_api_config():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "配置创建成功", "data": config.to_dict()})

    except Exception as e:
        logger.info(f"创建API配置失败: {str(e)}")
        import traceback
...
def update_api_config(config_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "配置更新成功", "data": config.to_dict()})

    except Exception as e:
        logger.info(f"更新API配置失败: {str(e)}")
        import traceback
...
def delete_api_config(config_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "配置删除成功"})

    except Exception as e:
        logger.info(f"删除API配置失败: {str(e)}")
        import traceback
