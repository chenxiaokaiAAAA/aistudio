...
def get_ai_tasks():
    """获取AI任务列表"""
    try:
        models = get_models(['AITask', 'Order', 'StyleCategory', 'StyleImage', 'APIProviderConfig', 'db'])
        if not models:
        ...
        if playground_only:
            ...
            query = query.join(Order, AITask.order_id == Order.id).filter(Order.order_number.like('PLAY_%'))
            # 如果用户已登录，只显示该用户的任务
        ...
        for task in pagination.items:
            ...
            task_data = {
                ...
                # 统一路径格式（将反斜杠转为正斜杠）
                'input_image_path': task.input_image_path.replace('\\', '/') if task.input_image_path else None,
                'input_image_type': task.input_image_type,
                'output_image_path': task.output_image_path.replace('\\', '/') if task.output_image_path else None,
                'status': task.status,
