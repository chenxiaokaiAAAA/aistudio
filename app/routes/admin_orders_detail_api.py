...
def admin_order_detail(order_id):
    ...
    models = get_models(['Order', 'OrderImage', 'Product', 'ProductSize', 'ShopOrder', 'AITask', 'db'])
    if not models:
        flash('系统未初始化', 'error')
    ...
    all_orders = Order.query.filter_by(order_number=order_number).order_by(Order.created_at.asc()).all()
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('admin_orders.admin_orders_detail.admin_order_detail', order_id=order_id))
...
def admin_add_order():
    ...
    if request.method == 'POST':
        try:
            ...
            # 重定向到订单详情页
            return redirect(url_for('admin_orders.admin_orders_detail.admin_order_detail', order_id=order.id))
...
def get_customer_info():
    ...
    try:
        ...
        if PromotionUser:
            ...
            if promotion_user:
                ...
                return jsonify({
                    'success': True,
                    'customer_name': customer_name,
                    'source': 'promotion_user'
                })
        
        # 2. 从订单表中查找最近使用该手机号的订单
        if Order:
            recent_order = Order.query.filter_by(customer_phone=phone)\
        ...
        return jsonify({
            'success': False,
            'message': '未找到该手机号的注册用户，请手动填写客户姓名'
        })
    ...
    try:
        ...
        if task_id and task_id != 0 and AITask:
            ...
            if task:
                ...
                deleted_count += 1
                print(f"删除AITask记录: task_id={task_id}, order_id={order_id}")
        ...
        # 删除原图
        file_paths = [
            os.path.join(hd_folder, filename),
        ...
        if order.hd_image == filename:
            order.hd_image = None
            print(f"清空订单hd_image字段: order_id={order_id}")
        ...
        return jsonify({
            'success': True,
            'message': f'效果图删除成功（删除了 {deleted_count} 个文件/记录）'
        })
        
    except Exception as e:
        db.session.rollback()
        print(f"删除效果图失败: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({
            'success': False,
