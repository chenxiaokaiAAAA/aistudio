...
def send_order_completion_notification():
    ...
    try:
        ...
        template_data = {
            ...
            "time17": {
                "value": (
                    order.completed_at.strftime("%Y年%m月%d日 %H:%M")
        ...
        logger.info(f"发送订单完成通知失败: {str(e)}")
        import traceback
...
def send_commission_notification():
    ...
    try:
        ...
        if not commission_id:
            return jsonify({"success": False, "message": "缺少分佣记录ID"}), 400
        ...
        if not order:
            return jsonify({"success": False, "message": "订单信息不存在"}), 404
        ...
        template_data = {
            ...
            "thing3": {
                "value": "已结算" if commission.status == "completed" else "待结算"
            },  # 金额状态
        }
        ...
        logger.info(f"发送推广收益通知失败: {str(e)}")
        import traceback
...
def send_message_to_user():
    ...
    try:
        ...
        # 获取插入的记录ID
        message_id = db.session.execute(db.text("SELECT last_insert_rowid()")).fetchone()[0]

        logger.info(f"✅ 消息发送成功: messageId={message_id}, userId={data.get('userId')}")

        return jsonify({"success": True, "messageId": message_id, "message": "消息发送成功"})
...
def batch_send_message():
    ...
    try:
        ...
        if target_type == "all":
            result = db.session.execute(
                db.text(
                    "SELECT DISTINCT user_id, session_id FROM user_access_logs WHERE user_id IS NOT NULL AND user_id != ''"
                )
            )
        elif target_type == "promotion_users":
            result = db.session.execute(
                db.text(
                    "SELECT DISTINCT user_id, session_id FROM user_access_logs WHERE final_promotion_code IS NOT NULL"
                )
            )
        ...
        for user_id, session_id in users:
            try:
                ...
                logger.warning("发送消息给用户失败: {e}")
                continue

        db.session.commit()

        logger.info(f"✅ 批量发送消息成功: 发送给 {sent_count} 个用户")
