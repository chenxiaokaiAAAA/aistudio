...
def get_profile():
    ...
    try:
        ...
        if hasattr(current_user, "playground_last_reset_date"):
            if (
                not current_user.playground_last_reset_date
                or current_user.playground_last_reset_date != today
            ):
...
def get_operation_logs():
    """获取操作日志"""
    try:
        models = get_models(["OperationLog", "db"])
        if not models:
            return jsonify({"success": False, "message": "数据库未初始化"}), 500
...
def get_admin_users():
    ...
    try:
        ...
        # 查询admin和operator角色的用户（数据库层面分页）
        query = User.query.filter(User.role.in_(["admin", "operator"]))
        ...
        if search:
            ...
            query = query.filter(
                or_(
                    ...
                    User.contact_person.like(f"%{search}%"),
                    User.contact_phone.like(f"%{search}%"),
                )
            )
    ...
    try:
        ...
        new_user = User(
            ...
            page_permissions=(
                ...
                else json.dumps(page_permissions)
            ),
        )
        ...
        if not check_password_hash(new_user.password, password):
            logger.info(f"警告: 用户 {username} 的密码哈希验证失败")
        else:
            logger.info(f"成功: 用户 {username} 创建成功，密码哈希验证通过")
...
def update_admin_user(user_id):
    ...
    try:
        ...
        if "playground_daily_limit" in data:
            user.playground_daily_limit = (
                int(data["playground_daily_limit"]) if data["playground_daily_limit"] else 0
            )
