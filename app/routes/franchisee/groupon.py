...
def franchisee_groupon_verify():
    ...
    if GrouponPackage:
        try:
            packages = (
                GrouponPackage.query.filter_by(status="active")
                .order_by(GrouponPackage.platform.asc(), GrouponPackage.sort_order.asc())
                .all()
            )
            logger.info(f"[DEBUG] 获取到 {len(packages)} 个套餐")
        except Exception as e:
            logger.info(f"[ERROR] 获取套餐列表失败: {e}")
            import traceback

            traceback.print_exc()
            packages = []
    else:
        logger.info("[WARNING] GrouponPackage 模型未找到，请检查模型导入")
    ...
    logger.info(f"[DEBUG] 平台分组结果: {list(platforms.keys())}, 共 {len(platforms)} 个平台")

    return render_template("franchisee/groupon_verify.html", account=account, platforms=platforms)
...
def franchisee_groupon_records():
    ...
    if staff_ids:
        ...
        # 如果没有店员，只查询加盟商直接创建的
        query = Coupon.query.filter(
            Coupon.source_type == "groupon", Coupon.franchisee_id == franchisee_id
        )
    ...
    if search:
        query = query.filter(
            or_(Coupon.groupon_order_id.like(f"%{search}%"), Coupon.code.like(f"%{search}%"))
        )
    ...
    for coupon in coupons:
        ...
        # 如果是店员创建的，显示店员信息
        if coupon.creator_type == "staf" and coupon.staff_user_id and StaffUser:
    ...
    if GrouponPackage:
        ...
        if staff_ids:
            platforms_query = (
                ...
                .with_entities(Coupon.groupon_platform)
                .distinct()
                .all()
            )
        else:
    ...
    return render_template(
        "franchisee/groupon_records.html",
        account=account,
...
def franchisee_groupon_verify_api():
    ...
    try:
        ...
        if not Coupon:
            return jsonify({"status": "error", "message": "优惠券模型未初始化"}), 500
        ...
        if package_id and GrouponPackage:
            ...
            return jsonify({"status": "error", "message": "请选择团购套餐"}), 400
