...
def cancel_franchisee_order(order_id):
    ...
    try:
        ...
        if not order.franchisee_id:
            return jsonify({"success": False, "message": "该订单不是加盟商订单"}), 400

        if order.status == "cancelled":
            return jsonify({"success": False, "message": "订单已经取消"}), 400
        ...
        if not account:
            return jsonify({"success": False, "message": "加盟商账户不存在"}), 400
        ...
        logger.info(f"✅ 订单已取消: {order.order_number}, 返还金额: {refund_amount}")

        return jsonify(
            {"success": True, "message": "订单已取消，资金已返还", "refund_amount": refund_amount}
        )

    except Exception as e:
        db.session.rollback()
        logger.error("取消订单失败: {str(e)}")
