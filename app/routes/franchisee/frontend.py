    ...
    if request.method == "POST":
        ...
        if account and check_password_hash(account.password, password):
            if account.status != "active":
                logger.warning(
                    f"[加盟商登录] 账户被禁用: username={username}, status={account.status}"
                )
            ...
            logger.info(
                f"[加盟商登录] Session设置: franchisee_id={session.get('franchisee_id')}, company={session.get('franchisee_company')}"
            )

            flash(f"欢迎回来，{account.company_name}", "success")
            redirect_url = url_for("franchisee.franchisee_frontend.dashboard")
            logger.info(f"[加盟商登录] 重定向到: {redirect_url}")
...
def dashboard():
    ...
    # 业绩统计（基于完成时间）
    daily_revenue = (
        base_query.filter(func.date(Order.completed_at) == today, Order.status == "completed")
    ...
    yesterday_revenue = (
        base_query.filter(func.date(Order.completed_at) == yesterday, Order.status == "completed")
        .with_entities(func.sum(Order.price))
        .scalar()
        or 0.0
    )
    ...
    total_revenue = (
        base_query.filter(Order.status == "completed").with_entities(func.sum(Order.price)).scalar()
        or 0.0
    )
    ...
    recent_orders = (
        base_query.filter(Order.created_at >= thirty_days_ago)
    ...
    recent_recharges = (
        FranchiseeRecharge.query.filter(
    ...
    total_deduction = sum(
        order.franchisee_deduction for order in recent_orders if order.franchisee_deduction
    )
...
def franchisee_orders():
    ...
    # 优化：使用数据库层面分页（类似管理后台订单列表的优化方式）
    from sqlalchemy import text

    # 1. 先获取不重复的订单号列表（按创建时间排序）
    ...
    order_numbers_subquery = order_numbers_subquery.group_by(Order.order_number)
    ...
    if not order_numbers:
        ...
        # 查询这些订单号的所有订单记录
        orders_query = (
            query.filter(Order.order_number.in_(order_numbers))
            .options(joinedload(Order.franchisee_account))
    ...
    # 3. 计算总数（不重复的订单号数量）
    total_count_query = db.session.query(func.count(func.distinct(Order.order_number))).filter(
        Order.franchisee_id == franchisee_id
    )

    if status == "unpaid":
        total_count_query = total_count_query.filter(Order.status == "unpaid")
    else:
        total_count_query = total_count_query.filter(Order.status != "unpaid")

    if status and status != "unpaid":
        total_count_query = total_count_query.filter(Order.status == status)

    if order_mode:
        total_count_query = total_count_query.filter(Order.order_mode == order_mode)
    ...
    total_orders = (
        ...
        .filter(Order.franchisee_id == franchisee_id, Order.status != "unpaid")
        .scalar()
        or 0
    )
    ...
    pending_selection_order_numbers = (
...
def franchisee_order_detail(order_id):
    ...
    all_orders = (
        Order.query.filter_by(
            order_number=order_number, franchisee_id=franchisee_id  # 确保只获取该加盟商的订单
        )
    ...
    order_ids_for_images = [o.id for o in all_orders]
    order_images_map = {}
    if order_ids_for_images:
        try:
            # 使用原始SQL批量查询
            placeholders = ",".join([f":order_id_{i}" for i in range(len(order_ids_for_images))])
            params = {f"order_id_{i}": oid for i, oid in enumerate(order_ids_for_images)}
            ...
            for row in images_data:
                ...
                if order_id_val not in order_images_map:
                    order_images_map[order_id_val] = []
                ...
                order_images_map[order_id_val].append(
                    ImageObj(img_id, path, is_main, quote(path, safe=""))
                )
        except Exception as e:
            # 如果原始SQL查询失败，尝试使用SQLAlchemy批量查询
            try:
...
def confirm_order(order_id):
    ...
    try:
        ...
        return jsonify({"success": True, "message": "订单确认成功，已进入制作流程"})
...
def franchisee_recharge_records():
    ...
    # 查询充值记录（数据库层面分页）
    query = FranchiseeRecharge.query.filter_by(franchisee_id=franchisee_id)

    # 分页查询
    pagination = query.order_by(desc(FranchiseeRecharge.created_at)).paginate(
        page=page, per_page=per_page, error_out=False
    )

    recharge_records = pagination.items

    return render_template(
        "franchisee/recharge_records.html",
        account=account,
...
def franchisee_users():
    ...
    if order_numbers:
        orders = Order.query.filter(
            Order.franchisee_id == franchisee_id, Order.order_number.in_(order_numbers)
        ).all()
    else:
        orders = []
    ...
    total_pages = (total_customers + per_page - 1) // per_page if per_page > 0 else 1

    return render_template(
        "franchisee/users.html",
        account=account,
...
def franchisee_order():
    ...
    if product_ids:
        all_sizes = (
            ProductSize.query.filter(
                ProductSize.product_id.in_(product_ids), ProductSize.is_active is True
            )
    ...
    for product in products:
        ...
        for size in sizes:
            ...
            size_data = {
                ...
                "pet_options": [
                    {"id": opt.id, "pet_count_name": opt.pet_count_name, "price": opt.price}
                    for opt in pet_options
                ],
            }
            if not pet_options:
                size_data["pet_options"] = [
                    {"id": 0, "pet_count_name": "默认", "price": size.price}
                ]
            product_sizes[product.id].append(size_data)

        # 从批量查询的映射中获取风格分类绑定（避免N+1查询）
    ...
    # 优化N+1查询：批量查询所有产品的自定义字段
    custom_fields_map = {}
    if product_ids:
        all_custom_fields = (
            ProductCustomField.query.filter(ProductCustomField.product_id.in_(product_ids))
    ...
    # 优化N+1查询：批量查询所有风格图片
    category_ids = [cat.id for cat in all_style_categories_list]
    images_map = {}
    if category_ids:
        all_images = (
    ...
    if request.method == "POST":
        try:
            ...
            if not selected_product or not selected_size:
                ...
                return render_template(
                    "franchisee/fran_order_new.html",
                    account=account,
                    products=products,
            ...
            if account.remaining_quota < price:
                flash(
                    f"加盟商账户额度不足，当前剩余额度：{account.remaining_quota}元，订单金额：{price}元",
                    "error",
                )
                return render_template(
                    "franchisee/fran_order_new.html",
            ...
            for i, file in enumerate(files):
                if file and allowed_file(file.filename):
                    ...
                    if any(keyword in filename for keyword in forbidden_keywords):
                        logger.warning("检测到包含禁用关键字的文件名: {filename}, 跳过此文件")
            ...
            custom_fields_data = {}
            # 优化N+1查询：批量查询所有产品的自定义字段
            if "custom_fields_map" not in locals():
                product_ids_for_fields = [p.id for p in products]
                custom_fields_map = {}
                if product_ids_for_fields:
                    all_custom_fields = (
                        ProductCustomField.query.filter(
                            ProductCustomField.product_id.in_(product_ids_for_fields)
                        )
                        .order_by(ProductCustomField.sort_order.asc())
                        .all()
                    )
            ...
            # 从批量查询的映射中获取自定义字段（避免N+1查询）
            custom_fields = (
                custom_fields_map.get(product_id, []) if "custom_fields_map" in locals() else []
            )
            for field in custom_fields:
            ...
            custom_fields_json = (
                json.dumps(custom_fields_data, ensure_ascii=False) if custom_fields_data else None
            )
            ...
            order = Order(
                ...
                confirmation_deadline=(
                    datetime.now() + timedelta(days=3) if need_confirmation else None
                ),
            ...
            for i, filename in enumerate(saved_filenames):
                order_image = OrderImage(
                    order_id=order.id, path=filename, is_main=(i == main_index)
                )
                db.session.add(order_image)
                logger.info(
                    f"保存订单图片 - 订单ID: {order.id}, 图片索引: {i}, 文件名: {filename}, 是否主图: {i == main_index}"
                )

            logger.info(f"总共保存了 {len(saved_filenames)} 张图片到订单 {order.id}")
            ...
            if WECHAT_NOTIFICATION_AVAILABLE and wechat_notify:
                try:
                    ...
                    logger.info(f"✅ 加盟商订单微信通知已发送: {order_number}")
                except Exception as e:
                    logger.error("加盟商订单微信通知失败: {e}")
            ...
            logger.info(f"订单创建错误: {error_msg}")
    ...
    try:
        ...
        if request.method == "POST":
            try:
                ...
                product_prices = {"standard": 299, "premium": 399, "luxury": 599}

                price = product_prices.get(product_type, 299)

                if franchisee.remaining_quota < price:
                    flash(
                        f"加盟商账户额度不足，当前剩余额度：{franchisee.remaining_quota}元，订单金额：{price}元",
                        "error",
                    )
                ...
                if file and allowed_file(file.filename):
                    ...
                    if WECHAT_NOTIFICATION_AVAILABLE and wechat_notify:
                        try:
                            ...
                            logger.info(f"✅ 加盟商订单（二维码）微信通知已发送: {order.id}")
                        except Exception as e:
                            logger.error("加盟商订单微信通知失败: {e}")
