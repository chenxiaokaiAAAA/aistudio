...
处理图片的访问和下载
"""
from flask import Blueprint, send_from_directory, jsonify, current_app, send_file
...
def download_final_clean(filename):
    ...
    try:
        ...
        return send_from_directory(current_app.config['FINAL_FOLDER'], clean_filename, as_attachment=True)
    except Exception as e:
        print(f"下载无水印效果图失败: {e}")
        return f"下载失败: {str(e)}", 500
        ...
        with zipfile.ZipFile(mem_file, mode='w', compression=zipfile.ZIP_DEFLATED) as zf:
            ...
            for fname in unique_files:
                ...
                if os.path.exists(file_path):
                    # 使用智能文件名
                    smart_image_name = generate_smart_image_name(order, fname, unique_files.index(fname))
...
def public_media_hd(filename):
    ...
    try:
        ...
        print(f"   尝试路径1: {hd_filepath}")
        print(f"   尝试路径2: {final_filepath}")
...
def public_media_hd_original(filename):
    ...
    try:
        ...
        # 文件不存在
        print(f"❌ 原图文件不存在: {original_filename}")
...
def media_original(filename):
    ...
    try:
        ...
        # 打印调试信息
        print(f"📥 访问文件请求:")
        print(f"   - 配置的UPLOAD_FOLDER: {current_app.config.get('UPLOAD_FOLDER', 'uploads')}")
        ...
        if not os.path.exists(filepath):
            ...
            if os.path.exists(upload_folder):
                ...
                print(f"   - 目录中的文件: {files[:10]}")  # 只显示前10个
            else:
                print(f"   - 目录不存在: {upload_folder}")
        ...
        if not real_filepath.startswith(real_upload_folder):
            print(f"❌ 路径遍历攻击尝试: {filepath}")
            return jsonify({'error': '非法路径'}), 403
        
        file_size = os.path.getsize(filepath)
        print(f"✅ 返回文件: {filepath}, 大小: {file_size} bytes")
...
def media_final_clean(filename):
    ...
    try:
        ...
        return send_from_directory(current_app.config['FINAL_FOLDER'], clean_filename, as_attachment=False)
    except Exception as e:
        print(f"访问无水印效果图失败: {e}")
...
def media_hd_clean(filename):
    ...
    try:
        ...
        return send_from_directory(current_app.config['HD_FOLDER'], clean_filename, as_attachment=False)
    except Exception as e:
        print(f"访问无水印高清图片失败: {e}")
