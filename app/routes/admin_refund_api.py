...
def get_refund_list():
    ...
    try:
        ...
        refund_requests = query.order_by(Order.refund_request_time.desc()).offset((page - 1) * per_page).limit(per_page).all()
        ...
        for order in refund_requests:
            ...
            result.append({
                ...
                'refund_request_time': order.refund_request_time.strftime('%Y-%m-%d %H:%M:%S') if order.refund_request_time else '',
        ...
        print(f"获取退款申请列表失败: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({
            'status': 'error',
            'message': f'获取退款申请列表失败: {str(e)}'
        }), 500
...
def approve_refund(order_id):
    ...
    try:
        ...
        if not order.refund_request_status or order.refund_request_status != 'pending':
            return jsonify({
                'status': 'error',
                'message': '该订单没有待处理的退款申请'
            }), 400
        
        if not order.franchisee_id:
            return jsonify({
                'status': 'error',
                'message': '该订单不是加盟商订单，无法退款'
            }), 400
