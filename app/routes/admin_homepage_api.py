...
def admin_create_banner():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"新字段设置失败（数据库兼容性）: {e}")
            # 忽略新字段错误，使用默认值
        ...
        # 使轮播图缓存失效
        try:
            from app.services.cache_service import cache_key, invalidate_cache

            invalidate_cache("homepage_banners")
            logger.info("轮播图缓存已失效")
        except Exception as e:
            logger.warning(f"失效轮播图缓存失败: {e}")
        ...
        logger.info(f"创建轮播图失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "创建轮播图失败"}), 500
...
def admin_update_banner(banner_id):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"新字段更新失败（数据库兼容性）: {e}")
            # 忽略新字段错误
        ...
        return jsonify({"status": "success", "message": "轮播图更新成功"})

    except Exception as e:
        logger.info(f"更新轮播图失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "更新轮播图失败"}), 500
...
def admin_delete_banner(banner_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "轮播图删除成功"})

    except Exception as e:
        logger.info(f"删除轮播图失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "删除轮播图失败"}), 500
...
def admin_update_homepage_config():
    ...
    try:
        ...
        try:
            ...
            invalidate_cache("homepage_config")
            logger.info("首页配置缓存已失效")
        except Exception as e:
            logger.warning(f"失效首页配置缓存失败: {e}")

        return jsonify({"status": "success", "message": "首页配置更新成功"})

    except Exception as e:
        logger.info(f"更新首页配置失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "更新首页配置失败"}), 500
...
def admin_update_homepage_features():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "功能配置更新成功"})

    except Exception as e:
        logger.info(f"更新功能配置失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "更新功能配置失败"}), 500
...
def admin_get_category_navs():
    ...
    try:
        ...
        result = []
        for nav in navs:
            # 保存原始路径，用于编辑时显示；统一旧路径为标准路径
            image_url_original = nav.image_url or ""
            # 将旧路径 /images/category_nav/xxx、/images/category-nav/xxx 转为 /media/category_nav/xxx
            if image_url_original:
                for old_prefix in ("/images/category_nav/", "/images/category-nav/", "/static/images/category_nav/"):
                    if image_url_original.startswith(old_prefix):
                        filename = image_url_original[len(old_prefix):].lstrip("/")
                        image_url_original = f"/media/category_nav/{filename}"
                        break
            # 生成完整URL用于预览
        ...
        logger.info(f"获取分类导航失败: {str(e)}")
        return jsonify({"status": "error", "message": "获取分类导航失败"}), 500
...
def admin_create_category_nav():
    ...
    try:
        ...
        logger.info(f"创建分类导航失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "创建分类导航失败"}), 500
...
def admin_update_category_nav(nav_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "分类导航更新成功"})

    except Exception as e:
        logger.info(f"更新分类导航失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "更新分类导航失败"}), 500
...
def admin_delete_category_nav(nav_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "分类导航删除成功"})

    except Exception as e:
        logger.info(f"删除分类导航失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "删除分类导航失败"}), 500
...
def admin_get_product_sections():
    ...
    try:
        ...
        logger.info(f"获取产品推荐模块失败: {str(e)}")
        return jsonify({"status": "error", "message": "获取产品推荐模块失败"}), 500
...
def admin_create_product_section():
    ...
    try:
        ...
        logger.info(f"创建产品推荐模块失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "创建产品推荐模块失败"}), 500
...
def admin_update_product_section(section_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "产品推荐模块更新成功"})

    except Exception as e:
        logger.info(f"更新产品推荐模块失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "更新产品推荐模块失败"}), 500
...
def admin_delete_product_section(section_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "产品推荐模块删除成功"})

    except Exception as e:
        logger.info(f"删除产品推荐模块失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "删除产品推荐模块失败"}), 500


@admin_homepage_api_bp.route("/products", methods=["GET"])
def admin_get_products_for_selection():
    """获取产品列表（用于产品选择器），支持一级、二级分类筛选"""
    try:
        models = get_models()
        if not models:
            return jsonify({"status": "error", "message": "数据库模型未初始化"}), 500
        ...
        product_images_map = {}
        if ProductImage and product_ids:
            all_images = (
            ...
            for image in all_images:
                if image.product_id not in product_images_map:
                    product_images_map[image.product_id] = image

        # 优化N+1查询：批量加载产品分类
        category_ids = list(
            set([p.category_id for p in products if hasattr(p, "category_id") and p.category_id])
        )
        categories_map = {}
        if ProductCategory and category_ids:
        ...
        subcategory_ids = list(
            set(
                [
                    p.subcategory_id
                    for p in products
                    if hasattr(p, "subcategory_id") and p.subcategory_id
                ]
            )
        )
        subcategories_map = {}
        if ProductSubcategory and subcategory_ids:
            subcategories = (
                ProductSubcategory.query.filter(
                    ProductSubcategory.id.in_(subcategory_ids)
                ).all()
            )
            for sub in subcategories:
                subcategories_map[sub.id] = sub
        ...
        for product in products:
            ...
            if ProductSubcategory and hasattr(product, "subcategory_id") and product.subcategory_id:
                subcat = subcategories_map.get(product.subcategory_id)
                if subcat:
        ...
        logger.info(f"获取产品列表失败: {str(e)}")
        return jsonify({"status": "error", "message": "获取产品列表失败"}), 500


@admin_homepage_api_bp.route("/product-categories-tree", methods=["GET"])
def admin_get_product_categories_tree():
    """获取产品分类树（一级+二级），用于选择器筛选"""
    try:
        models = get_models()
        if not models:
            return jsonify({"status": "error", "message": "数据库模型未初始化"}), 500
        ...
        ProductCategory = models.get("ProductCategory")
        ProductSubcategory = models.get("ProductSubcategory")
        get_base_url = models.get("get_base_url")
        ...
        for cat in categories:
            ...
            if ProductSubcategory:
                subcats = (
                    ProductSubcategory.query.filter_by(
        ...
        logger.info(f"获取产品分类树失败: {str(e)}")
        return jsonify({"status": "error", "message": "获取产品分类树失败"}), 500
