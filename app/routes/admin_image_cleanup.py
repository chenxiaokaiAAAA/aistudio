...
def execute_cleanup():
    ...
    try:
        ...
        print(f"[清理] 开始清理 {days} 天前的订单（截止日期: {cutoff_date.strftime('%Y-%m-%d %H:%M:%S')}）")
        ...
        orders = Order.query.filter(
            or_(
                ...
                (
                    ...
                    (Order.status.in_(['shipped', 'completed', 'hd_ready', 'selection_completed', 'printing', 'pending_shipment']))
                ),
                # 放宽条件：如果 created_at 小于截止日期，即使状态不是已完成，也尝试清理（可能是旧数据）
                (
                    (Order.created_at < cutoff_date) & 
                    (Order.status.notin_(['unpaid', 'paid', 'shooting', 'retouching', 'ai_processing', 'pending_selection']))
                )
            )
        ).all()
        
        print(f"[清理] 找到 {len(orders)} 个符合条件的订单")
        for order in orders:
            print(f"  - 订单: {order.order_number}, 状态: {order.status}, 创建时间: {order.created_at}, 完成时间: {order.completed_at}")
        ...
        print(f"[清理] Flask root_path: {root_path_obj}")
        ...
        print(f"  - final_works: {final_path}")
        print(f"  - hd_images: {hd_path}")
        ...
        for order in orders:
            ...
            if order.original_image:
                ...
                if order.original_image.startswith(('http://', 'https://')):
                    ...
                    if image_path.exists() and image_path.is_file():
                        ...
                        found_files.extend([f for f in upload_path.glob(f"{base_name}*") if f.is_file()])
                        # 模式2: 如果文件名包含订单号，查找包含订单号的文件
                        if order.order_number in filename:
                            found_files.extend([f for f in upload_path.glob(f"*{order.order_number}*") if f.is_file()])
                        # 模式3: 查找文件名中包含原始文件名关键部分的所有文件
                        if len(base_name) > 8:
                            key_part = base_name[-8:]  # 取文件名后8位作为关键部分
                            found_files.extend([f for f in upload_path.glob(f"*{key_part}*") if f.is_file()])
                    
                    # 去重
                    found_files = list(set(found_files))
                    
                    if found_files:
                        print(f"  [原图] 找到 {len(found_files)} 个相关文件，将全部删除")
                        for file_to_delete in found_files:
                            try:
                                ...
                                print(f"  [原图] ✅ 已删除: {file_to_delete.name} ({file_size / 1024:.2f} KB)")
                            except Exception as e:
                                error_count += 1
                                print(f"  [原图] ❌ 删除失败 {file_to_delete.name}: {str(e)}")
                    else:
                        not_found_count += 1
                        print(f"  [原图] ⚠️ 文件不存在: {image_path} (或未找到相关文件)")
            
            # 处理 OrderImage 表中的图片
            if OrderImage:
                order_images = OrderImage.query.filter_by(order_id=order.id).all()
                print(f"  [订单图片] 找到 {len(order_images)} 张图片")
                for order_image in order_images:
                    if order_image.path:
                        # 检查是否是URL（跳过URL格式的路径）
                        if order_image.path.startswith(('http://', 'https://')):
                            print(f"    [订单图片] ⚠️ 跳过URL路径: {order_image.path}")
                        else:
                            # 提取文件名（如果路径包含目录分隔符，只取文件名部分）
                            filename = os.path.basename(order_image.path)
                            image_path = upload_path / filename
                            print(f"    [订单图片] 检查: {image_path} (原始路径: {order_image.path})")
                            ...
                            if found_files:
                                print(f"    [订单图片] 找到 {len(found_files)} 个相关文件，将全部删除")
                                for file_to_delete in found_files:
                                    try:
                                        file_size = file_to_delete.stat().st_size
                                        total_size += file_size
                                        ...
                                        deleted_count += 1
                                        order_processed = True
                                        print(f"    [订单图片] ✅ 已删除: {file_to_delete.name} ({file_size / 1024:.2f} KB)")
                                    except Exception as e:
                                        error_count += 1
                                        print(f"    [订单图片] ❌ 删除失败 {file_to_delete.name}: {str(e)}")
                            else:
                                not_found_count += 1
                                print(f"    [订单图片] ⚠️ 文件不存在: {image_path} (或未找到相关文件)")
            
            # 2. 删除 final_works 目录中的效果图
            if order.final_image:
                # 检查是否是URL（跳过URL格式的路径）
                if order.final_image.startswith(('http://', 'https://')):
                    print(f"  [效果图] ⚠️ 跳过URL路径: {order.final_image}")
                else:
                    # 提取文件名（如果路径包含目录分隔符，只取文件名部分）
                    filename = os.path.basename(order.final_image)
                    image_path = final_path / filename
                    print(f"  [效果图] 检查: {image_path} (原始路径: {order.final_image})")
                    ...
                    if not image_path.exists():
                        ...
                        # 模式2: 包含订单号
                        if not order_files:
                            order_files.extend([f for f in final_path.glob("*") if f.is_file() and order.order_number in f.name])
                        # 模式3: 如果文件名包含订单号的一部分
                        if not order_files and len(order.order_number) > 8:
                            short_order = order.order_number[:8]
                            order_files.extend([f for f in final_path.glob("*") if f.is_file() and short_order in f.name])
                        
                        if order_files:
                            print(f"  [效果图] 找到 {len(order_files)} 个相关文件，将全部删除")
                            for order_file in order_files:
                                if order_file.is_file():
                                    try:
                                        ...
                                        deleted_count += 1
                                        order_processed = True
                                        print(f"  [效果图] ✅ 已删除: {order_file.name} ({file_size / 1024:.2f} KB)")
                                    except Exception as e:
                                        error_count += 1
                                        print(f"  [效果图] ❌ 删除失败 {order_file.name}: {str(e)}")
                        ...
                        try:
                            ...
                            deleted_count += 1
                            order_processed = True
                            print(f"  [效果图] ✅ 已删除: {order.final_image} ({file_size / 1024:.2f} KB)")
                        except Exception as e:
                            error_count += 1
                            print(f"  [效果图] ❌ 删除失败 {order.final_image}: {str(e)}")
                    else:
                        not_found_count += 1
                        print(f"  [效果图] ⚠️ 文件不存在: {image_path}")
            
            # 删除无水印版本
            if order.final_image_clean:
                # 检查是否是URL
                if order.final_image_clean.startswith(('http://', 'https://')):
                    print(f"  [无水印效果图] ⚠️ 跳过URL路径: {order.final_image_clean}")
                else:
                    filename = os.path.basename(order.final_image_clean)
                    clean_image_path = final_path / filename
                    print(f"  [无水印效果图] 检查: {clean_image_path} (原始路径: {order.final_image_clean})")
                    if clean_image_path.exists() and clean_image_path.is_file():
                        try:
                            ...
                            deleted_count += 1
                            order_processed = True
                            print(f"  [无水印效果图] ✅ 已删除: {order.final_image_clean} ({file_size / 1024:.2f} KB)")
                        except Exception as e:
                            error_count += 1
                            print(f"  [无水印效果图] ❌ 删除失败 {order.final_image_clean}: {str(e)}")
                    else:
                        not_found_count += 1
                        print(f"  [无水印效果图] ⚠️ 文件不存在: {clean_image_path}")
            ...
            if order.final_image and not order.final_image.startswith(('http://', 'https://')):
                ...
                print(f"  [clean_效果图] 检查: {clean_image_path}")
                if clean_image_path.exists() and clean_image_path.is_file():
            ...
            if order.hd_image:
                ...
                if order.hd_image.startswith(('http://', 'https://')):
                    ...
                    if not image_path.exists():
                        ...
                        # 模式2: 包含订单号
                        if not order_files:
                            order_files.extend([f for f in hd_path.glob("*") if f.is_file() and order.order_number in f.name])
                        # 模式3: 如果文件名包含订单号的一部分
                        if not order_files and len(order.order_number) > 8:
                            short_order = order.order_number[:8]
                            order_files.extend([f for f in hd_path.glob("*") if f.is_file() and short_order in f.name])
                        
                        if order_files:
                            print(f"  [高清图] 找到 {len(order_files)} 个相关文件，将全部删除")
                            for order_file in order_files:
                                if order_file.is_file():
                                    try:
                                        file_size = order_file.stat().st_size
                                        ...
                                        deleted_count += 1
                                        order_processed = True
                                        print(f"  [高清图] ✅ 已删除: {order_file.name} ({file_size / 1024:.2f} KB)")
                                    except Exception as e:
                                        error_count += 1
                                        print(f"  [高清图] ❌ 删除失败 {order_file.name}: {str(e)}")
                    elif image_path.exists() and image_path.is_file():
                        try:
                            file_size = image_path.stat().st_size
                            total_size += file_size
                            ...
                            deleted_count += 1
                            order_processed = True
                            print(f"  [高清图] ✅ 已删除: {order.hd_image} ({file_size / 1024:.2f} KB)")
                        except Exception as e:
                            error_count += 1
                            print(f"  [高清图] ❌ 删除失败 {order.hd_image}: {str(e)}")
                    else:
                        not_found_count += 1
                        print(f"  [高清图] ⚠️ 文件不存在: {image_path}")
            
            # 删除无水印高清图
            if order.hd_image_clean:
                # 检查是否是URL
                if order.hd_image_clean.startswith(('http://', 'https://')):
                    print(f"  [无水印高清图] ⚠️ 跳过URL路径: {order.hd_image_clean}")
                else:
                    filename = os.path.basename(order.hd_image_clean)
                    clean_image_path = hd_path / filename
                    print(f"  [无水印高清图] 检查: {clean_image_path} (原始路径: {order.hd_image_clean})")
                    if clean_image_path.exists() and clean_image_path.is_file():
                        try:
                            file_size = clean_image_path.stat().st_size
                            total_size += file_size
                            ...
                            deleted_count += 1
                            order_processed = True
                            print(f"  [无水印高清图] ✅ 已删除: {order.hd_image_clean} ({file_size / 1024:.2f} KB)")
                        except Exception as e:
                            error_count += 1
                            print(f"  [无水印高清图] ❌ 删除失败 {order.hd_image_clean}: {str(e)}")
                    else:
                        not_found_count += 1
                        print(f"  [无水印高清图] ⚠️ 文件不存在: {clean_image_path}")
            ...
            if order.hd_image and not order.hd_image.startswith(('http://', 'https://')):
                ...
                print(f"  [clean_高清图] 检查: {clean_image_path}")
                if clean_image_path.exists() and clean_image_path.is_file():
                    # 如果数据库中没有记录，但文件存在，也删除
                    if not order.hd_image_clean or order.hd_image_clean != clean_filename:
                        try:
                            ...
                            error_count += 1
                            print(f"删除clean_高清图失败 {clean_filename}: {str(e)}")
            
            if order_processed:
                processed_orders += 1
                print(f"  [订单] ✅ 订单 {order.order_number} 已处理")
            else:
                print(f"  [订单] ⚠️ 订单 {order.order_number} 没有找到可删除的文件")
