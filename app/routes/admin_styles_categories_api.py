...
def admin_get_categories():
    ...
    try:
        ...
        for category in categories:
            ...
            if StyleSubcategory:
                subcategories = (
                    StyleSubcategory.query.filter_by(category_id=category.id)
                    ...
                    subcategories_data.append(
                        {
                            ...
                            "created_at": (
                                subcategory.created_at.isoformat()
        ...
        logger.info(f"获取分类失败: {str(e)}")
        return jsonify({"status": "error", "message": "获取分类失败"}), 500
...
def admin_create_category():
    ...
    try:
        ...
        if workflow_file and workflow_name:
            try:
                ...
                if os.path.exists(old_filepath) and old_filepath != new_filepath:
                    ...
                    logger.info(f"✅ 工作流文件已重命名: {workflow_file} -> {new_filename}")
        ...
        # 使风格分类缓存失效
        try:
            from app.services.cache_service import CACHE_PREFIXES, invalidate_cache_pattern

            invalidate_cache_pattern(f"cache:{CACHE_PREFIXES['STYLE_CATEGORIES']}:*")
            logger.info("风格分类缓存已失效")
        except Exception as e:
            logger.warning(f"失效风格分类缓存失败: {e}")
        ...
        db.session.rollback()
        return jsonify({"status": "error", "message": "创建分类失败"}), 500
    ...
    try:
        ...
        if "workflow_file" in data:
            ...
            if not new_workflow_file:
                ...
                new_workflow_file
                and category.workflow_name
                and new_workflow_file != old_workflow_file
            ):
                try:
                    workflows_dir = "workflows"
                    os.makedirs(workflows_dir, exist_ok=True)
        ...
        return jsonify({"status": "success", "message": "分类更新成功"})

    except Exception as e:
        logger.info(f"更新分类失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "更新分类失败"}), 500
...
def admin_delete_category(category_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"status": "error", "message": "删除分类失败"}), 500
...
def admin_get_subcategories():
    ...
    try:
        ...
        if category_id:
            # 获取指定一级分类下的二级分类
            subcategories = (
                StyleSubcategory.query.filter_by(category_id=category_id)
                .order_by(StyleSubcategory.sort_order)
                .all()
            )
        else:
        ...
        for subcategory in subcategories:
            subcategory_data = {
                ...
                "is_active": subcategory.is_active,
                "created_at": (
                    subcategory.created_at.isoformat() if subcategory.created_at else None
                ),
            }
...
def admin_create_subcategory():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"status": "error", "message": "创建二级分类失败"}), 500
...
def admin_update_subcategory(subcategory_id):
    ...
    try:
        ...
        if data.get("code") and data["code"] != subcategory.code:
            existing = StyleSubcategory.query.filter_by(
                category_id=data.get("category_id", subcategory.category_id), code=data["code"]
            ).first()
            if existing:
                return (
                    jsonify({"status": "error", "message": "该一级分类下已存在相同代码的二级分类"}),
        ...
        return jsonify({"status": "success", "message": "二级分类更新成功"})

    except Exception as e:
        logger.info(f"更新二级分类失败: {str(e)}")
        db.session.rollback()
        return jsonify({"status": "error", "message": "更新二级分类失败"}), 500
...
def admin_delete_subcategory(subcategory_id):
    ...
    try:
        ...
        if StyleImage:
            StyleImage.query.filter_by(subcategory_id=subcategory_id).update(
                {"subcategory_id": None}
            )
        ...
        db.session.rollback()
        return jsonify({"status": "error", "message": "删除二级分类失败"}), 500
...
def admin_upload_category_cover_image():
    ...
    try:
        ...
        # 检查文件类型
        allowed_extensions = {"png", "jpg", "jpeg", "gi", "webp"}
        ...
        return jsonify(
            {
                "status": "success",
                "message": "上传成功",
                "data": {"image_url": image_url, "url": image_url},  # 兼容字段
            }
        )

    except Exception as e:
        logger.info(f"上传封面图片失败: {str(e)}")
