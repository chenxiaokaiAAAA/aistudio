...
def bind_promotion_relation():
    ...
    try:
        ...
        logger.info(f"推广关系绑定请求: userId={user_id}, promotion_params={promotion_params}")

        if not user_id:
            return jsonify({"success": False, "message": "用户ID不能为空"}), 400

        if not promotion_params:
            return jsonify({"success": False, "message": "缺少推广参数"}), 400
        ...
        logger.info(
            f"解析推广参数: promotion_code={promotion_code}, referrer_user_id={referrer_user_id}"
        )

        if promotion_code and referrer_user_id:
            # 验证推广码有效性
            if validate_promotion_code(promotion_code) != referrer_user_id:
                return jsonify({"success": False, "message": "推广码无效"}), 400

            # 检查是否已经绑定过
            existing_track = PromotionTrack.query.filter_by(
                visitor_user_id=user_id, referrer_user_id=referrer_user_id
            ).first()

            if existing_track:
                logger.info(f"推广关系已存在: {referrer_user_id} -> {user_id}")
                return jsonify({"success": True, "message": "推广关系已存在"})
            ...
            db.session.commit()
            logger.info(f"推广关系绑定成功：推广者={referrer_user_id}, 用户={user_id}")

            return jsonify({"success": True, "message": "推广关系绑定成功"})
        else:
            return jsonify({"success": False, "message": "推广参数解析失败"}), 400

    except Exception as e:
        logger.info(f"推广关系绑定失败: {e}")
        models = get_models()
        if models:
...
def track_promotion():
    ...
    try:
        ...
        if not promotion_code or not referrer_user_id:
            return jsonify({"success": False, "message": "推广码和推广者用户ID不能为空"}), 400
        ...
        return jsonify({"success": True, "message": "推广访问记录成功"})

    except Exception as e:
        logger.info(f"推广访问记录失败: {e}")
        models = get_models()
        if models:
            models["db"].session.rollback()
        return jsonify({"success": False, "message": f"推广访问记录失败: {str(e)}"}), 500
            ...
            return jsonify(
                {
                    ...
                    "referrerInfo": {"nickname": user.nickname, "avatarUrl": user.avatar_url},
                }
            )
        else:
            return jsonify({"success": False, "message": "推广码无效"}), 404

    except Exception as e:
        logger.info(f"验证推广码失败: {e}")
        return jsonify({"success": False, "message": f"验证推广码失败: {str(e)}"}), 500
...
def get_promotion_stats():
    ...
    try:
        ...
        completed_commissions = Commission.query.filter_by(
            referrer_user_id=user_id, status="completed"
        ).count()
        ...
        logger.info(f"获取推广统计失败: {e}")
        return jsonify({"success": False, "message": f"获取推广统计失败: {str(e)}"}), 500
...
def test_commission():
    ...
    try:
        ...
        logger.info(f"测试分佣失败: {e}")
        models = get_models()
        if models:
            models["db"].session.rollback()
        return jsonify({"success": False, "message": f"测试分佣失败: {str(e)}"}), 500
