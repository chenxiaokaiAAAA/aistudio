    ...
    try:
        ...
        if image_url and image_url.startswith("http"):
            ...
            if "/view?" in image_url:
                ...
                download_url = image_url
                logger.info(f"📥 下载ComfyUI图片: {download_url}")
            ...
            if response.status_code == 200:
                ...
                logger.info(f"✅ API结果图片已下载到本地: {local_path}")
                return relative_path
            else:
                logger.error("下载API结果图片失败: HTTP {response.status_code}")
                return None
        else:
            logger.warning("无效的图片URL: {image_url}")
            return None

    except Exception as e:
        logger.error("下载API结果图片异常: {str(e)}")
        import traceback
