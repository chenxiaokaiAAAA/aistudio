...
def admin_send_to_printer(order_id):
    ...
    models = get_models(
        ["Order", "db", "PRINTER_SYSTEM_AVAILABLE", "PRINTER_SYSTEM_CONFIG", "PrinterSystemClient"]
    )
    if not models:
    ...
    try:
        ...
        if not os.path.exists(hd_image_path):
            return (
                jsonify({"success": False, "message": f"高清图片文件不存在: {hd_image_path}"}),
                400,
            )
        ...
        if PrinterSystemClient:
            ...
            if result["success"]:
                ...
                return jsonify(
                    {"success": False, "message": f'发送失败: {result.get("message", "未知错误")}'}
                )
        else:
...
def admin_view_send_data(order_id):
    ...
    try:
        ...
        logger.info(f"获取发送数据包时发生错误: {str(e)}")
        import traceback
...
def admin_check_image_size(order_id):
    ...
    try:
        ...
        logger.info(f"检查图片尺寸时发生错误: {str(e)}")
        import traceback
...
def admin_manual_logistics(order_id):
    ...
    try:
        ...
        logger.info(f"✅ 订单 {order.order_number} 手动录入快递信息成功:")
        logger.info(f"   快递公司: {company}")
        logger.info(f"   快递单号: {tracking_number}")
        logger.info(f"   状态: {status}")
        logger.info(f"   备注: {remark}")

        return jsonify(
            {"success": True, "message": "快递单号录入成功", "logistics_info": logistics_data}
        )

    except Exception as e:
        if "db" in locals():
            db.session.rollback()
        logger.info(f"手动录入快递单号失败: {str(e)}")
        import traceback
