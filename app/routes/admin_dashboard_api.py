...
def get_revenue_detail():
    ...
    try:
        ...
        ).order_by(Order.completed_at.desc()).limit(100).all()
...
def get_processing_orders():
    ...
    try:
        ...
        # 查询处理中的订单
        orders = Order.query.filter(Order.status == 'processing').order_by(Order.created_at.desc()).limit(50).all()
        ...
        for order in orders:
            ...
            if AITask:
                ai_task = AITask.query.filter_by(order_id=order.id).order_by(AITask.created_at.desc()).first()
                if ai_task:
            ...
            orders_list.append({
                ...
                'shooting_completed_at': order.shooting_completed_at.strftime('%Y-%m-%d %H:%M:%S') if hasattr(order, 'shooting_completed_at') and order.shooting_completed_at else None,
                'retouch_completed_at': order.retouch_completed_at.strftime('%Y-%m-%d %H:%M:%S') if hasattr(order, 'retouch_completed_at') and order.retouch_completed_at else None,
                'completed_at': order.completed_at.strftime('%Y-%m-%d %H:%M:%S') if hasattr(order, 'completed_at') and order.completed_at else None,
...
def get_completed_orders():
    ...
    try:
        ...
        # 查询已完成的订单（最近50条）
        orders = Order.query.filter(Order.status == 'completed').order_by(Order.completed_at.desc()).limit(50).all()
...
def get_error_orders():
    ...
    try:
        ...
        for order in orders:
            ...
            if not error_message and AITask:
                ai_task = AITask.query.filter_by(order_id=order.id).order_by(AITask.created_at.desc()).first()
