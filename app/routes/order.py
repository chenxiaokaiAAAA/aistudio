...
def create_order():
    ...
    if "test_server" in sys.modules:
        ...
        from app import db
        from app.models import Order, OrderImage, Product, ProductSize, User
    ...
    if request.method == "POST":
        try:
            ...
            images = request.files.getlist("images") or (
                [] if "image" not in request.files else [request.files["image"]]
            )
            ...
            if images:
                ...
                order_number = (
                    f"PET{datetime.now().strftime('%Y%m%d%H%M%S')}{uuid.uuid4().hex[:4].upper()}"
                )
                ...
                if franchisee and product_type:
                    product_prices = {"standard": 299.0, "premium": 399.0, "luxury": 599.0}
                    product_names = {"standard": "标准版", "premium": "高级版", "luxury": "豪华版"}
                    ...
                    if product_type in product_prices:
                        ...
                        if franchisee.remaining_quota >= price:
                            ...
                            logger.info(
                                f"加盟商额度扣除成功: {franchisee.company_name}, 扣除金额: {price}, 剩余额度: {franchisee.remaining_quota}"
                            )
                        else:
                            flash(
                                f"加盟商额度不足，当前剩余额度: {franchisee.remaining_quota} 元",
                                "error",
                            )
                ...
                if size and not product_type:
                    ...
                    try:
                        ...
                        if size in SIZE_MAPPING:
                            ...
                            # 通过尺寸名称查找对应的尺寸配置
                            size_config = ProductSize.query.filter_by(
                ...
                logger.info(f"✅ 订单创建成功，ID: {new_order.id}, 订单号: {order_number}")
                ...
                # 提交所有更改
                db.session.commit()
                logger.info(f"✅ 订单数据已持久化到数据库，订单号: {order_number}")
            ...
            db.session.rollback()
            logger.error("订单创建失败: {str(e)}")
            flash(f"订单创建失败: {str(e)}", "error")
            return render_template(
                "order.html", merchant=merchant, franchisee=franchisee, size_options=product_sizes
            )
...
def order_status():
    ...
    if (
        request.method == "GET"
        and request.args.get("order_number")
        and request.args.get("phone_last4")
    ):
    ...
    if request.method == "POST":
        ...
        if order_number:
            ...
            if phone_last4 and name:
                ...
                orders = (
                    Order.query.filter(Order.customer_name == name)
...
def complete_free_order():
    ...
    try:
        ...
        if "test_server" in sys.modules:
            ...
            from app import db
            from app.models import Coupon, Order, UserCoupon
        ...
        if not all([order_id, coupon_code, user_id]):
            return (
                jsonify({"success": False, "message": "缺少必要参数：orderId、couponCode、userId"}),
                400,
            )
        ...
        if order.status != "unpaid":
            return (
                jsonify({"success": False, "message": f"订单状态不正确，当前状态：{order.status}"}),
                400,
            )
        ...
        if not user_coupon:
            return jsonify({"success": False, "message": "优惠券不存在或已使用"}), 400
        ...
        if coupon.status != "active":
            return jsonify({"success": False, "message": "优惠券已失效"}), 400

        # 检查优惠券是否在有效期内
        if now < coupon.start_time or now > coupon.end_time:
            return jsonify({"success": False, "message": "优惠券不在有效期内"}), 400

        # 检查用户优惠券是否过期
        if (
            hasattr(user_coupon, "expire_time")
            and user_coupon.expire_time
            and now > user_coupon.expire_time
        ):
        ...
        logger.info(
            f"✅ 0元订单完成支付成功: 订单={order_id}, 优惠券={coupon_code}, 用户={user_id}"
        )
        ...
        logger.info(f"完成0元订单支付失败: {str(e)}")
        import traceback
