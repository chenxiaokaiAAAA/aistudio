...
def get_models():
    ...
    return {
        ...
        'ShopProduct': test_server_module.ShopProduct,
        'ShopProductImage': test_server_module.ShopProductImage,
        'ShopProductSize': test_server_module.ShopProductSize,
        'ShopOrder': test_server_module.ShopOrder,
...
def admin_shop_products():
        ...
        if action == 'add_product':
            ...
            if 'product_image' in request.files:
                ...
                if file and file.filename:
                    ...
                    static_products_dir = os.path.join(app.root_path, 'static', 'images', 'shop_products')
            ...
            for i, file in enumerate(uploaded_files):
                if file and file.filename:
                    ...
                    db.session.add(ShopProductImage(
            ...
            for i, (size_name, size_price, size_stock) in enumerate(zip(size_names, size_prices, size_stocks)):
                if size_name and size_price:
                    try:
                        ...
                        db.session.add(ShopProductSize(
                            product_id=new_product.id,
                            size_name=size_name,
                            price=price,
                            stock=stock,
            ...
            print(f"📝 处理规格数据: 共 {max_len} 个规格")
            print(f"   - size_ids: {size_ids}")
            print(f"   - size_names: {size_names}")
            print(f"   - size_prices: {size_prices}")
            print(f"   - size_stocks: {size_stocks}")
                ...
                try:
                    ...
                    if size_id:
                        ...
                        if size:
                            ...
                            size.sort_order = i
                            print(f"✅ 更新规格 ID={size_id}: {size_name}, 价格={price}, 效果图={effect_image_url}")
                        else:
                            print(f"⚠️ 规格 ID={size_id} 不存在，将创建新规格")
                            # 如果ID不存在，创建新规格
                            db.session.add(ShopProductSize(
                                product_id=product_id,
                                size_name=size_name,
                        ...
                        print(f"✅ 添加新规格: {size_name}, 价格={price}, 效果图={effect_image_url}")
                except (ValueError, TypeError) as e:
                    print(f"❌ 处理第 {i+1} 个规格时出错: {e}")
                    import traceback
                    traceback.print_exc()
                    pass
    ...
    for product in products:
        product.images_list = ShopProductImage.query.filter_by(
            product_id=product.id, is_active=True
        ).order_by(ShopProductImage.sort_order.asc()).all()
        product.sizes_list = ShopProductSize.query.filter_by(
            product_id=product.id, is_active=True
...
def admin_shop_product_detail(product_id):
    ...
    return jsonify({
        ...
        'data': {
            ...
            'sizes': [{'id': size.id, 'size_name': size.size_name, 'price': size.price, 'stock': size.stock, 'effect_image_url': size.effect_image_url, 'sort_order': size.sort_order} for size in sizes]
        }
    })
...
def admin_shop_order_detail(order_id):
    ...
    if original_order:
        # 查询该原始订单的所有商城订单
        all_shop_orders = ShopOrder.query.filter_by(original_order_id=original_order.id).order_by(ShopOrder.created_at.asc() if hasattr(ShopOrder, 'created_at') else ShopOrder.id.asc()).all()
    ...
    return render_template('admin/shop_order_detail.html', 
