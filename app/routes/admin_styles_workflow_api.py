...
def test_workflow(image_id):
    ...
    try:
        ...
        # 检查Playground使用次数限制
        models = get_models(['StyleCategory', 'StyleImage', 'AIConfig', 'User', 'Order', 'OrderImage', 'AITask', 'APITemplate', 'db'])
        if models:
            db = models['db']
            User = models['User']
            ...
            if user:
                ...
                if hasattr(user, 'playground_used_today'):
                    ...
                    if not hasattr(user, 'playground_last_reset_date') or not user.playground_last_reset_date:
                        user.playground_last_reset_date = today
                    db.session.commit()
        ...
        try:
            ...
            if user and hasattr(user, 'playground_used_today'):
                ...
                db.session.commit()
                limit_text = f"{user.playground_daily_limit}" if user.playground_daily_limit > 0 else "无限制"
                print(f"📊 Playground使用次数已更新: {user.playground_used_today}/{limit_text}")
    ...
    try:
        ...
        if file:
            ...
            try:
                ...
                print(f"✅ 图片下载成功: {len(file_content)} 字节")
            except Exception as e:
                return jsonify({
                    'status': 'error',
        ...
        upload_files = {
            'file': (upload_filename, file_content, f'image/{ext}')
        }
