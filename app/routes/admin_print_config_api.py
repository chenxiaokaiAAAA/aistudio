...
def api_get_payment_config():
    ...
    try:
        ...
        return jsonify({"status": "error", "message": f"获取支付配置失败: {str(e)}"}), 500
...
def api_save_payment_config():
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "支付配置保存成功"})
    except Exception as e:
        if "db" in locals():
            db.session.rollback()
        return jsonify({"status": "error", "message": f"保存支付配置失败: {str(e)}"}), 500
...
def api_get_printer_config():
    ...
    try:
        ...
        proxy_key_config = AIConfig.query.filter_by(
            config_key="local_printer_proxy_api_key"
        ).first()
        if proxy_key_config:
        ...
        return jsonify({"status": "error", "message": f"获取打印机配置失败: {str(e)}"}), 500
...
def api_save_printer_config():
    ...
    try:
        ...
        configs_to_save = [
            ...
            (
                "printer_file_access_base_url",
                data.get("file_access_base_url", ""),
                "外部可访问的文件基础URL",
            ),
            ...
            (
                "local_printer_path",
                data.get("local_printer_path", ""),
                "本地打印机路径（用于电子照片打印，本地部署时使用）",
            ),
            (
                "local_printer_proxy_url",
                data.get("local_printer_proxy_url", ""),
                "打印代理服务地址（远程部署时使用，如 http://192.168.1.100:8888）",
            ),
            (
                "local_printer_proxy_api_key",
                data.get("local_printer_proxy_api_key", ""),
                "打印代理服务API密钥（可选，用于安全验证）",
            ),
        ]
        ...
        return jsonify({"status": "success", "message": "打印机配置保存成功"})
    except Exception as e:
        if "db" in locals():
            db.session.rollback()
        return jsonify({"status": "error", "message": f"保存打印机配置失败: {str(e)}"}), 500
...
def api_get_machine_printer_configs():
    """获取所有自拍机的打印机配置"""
    try:
        models = get_models(["AIConfig", "SelfieMachine", "FranchiseeAccount"])
        if not models:
            logger.error("获取门店打印机配置失败: 系统未初始化")
            return jsonify({"status": "error", "message": "系统未初始化"}), 500
        ...
        logger.info(f"获取到 {len(configs)} 个门店打印机配置")
        return jsonify({"status": "success", "data": configs})
    except Exception as e:
        import traceback

        logger.error(f"获取门店打印机配置失败: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({"status": "error", "message": f"获取门店打印机配置失败: {str(e)}"}), 500
...
def api_save_machine_printer_config():
    """保存指定自拍机的打印机配置"""
    try:
        models = get_models(["AIConfig", "db", "SelfieMachine"])
        if not models:
            logger.error("保存门店打印机配置失败: 系统未初始化")
            return jsonify({"status": "error", "message": "系统未初始化"}), 500
        ...
        if SelfieMachine:
            machine = SelfieMachine.query.filter_by(machine_serial_number=machine_id).first()
            if not machine:
                return jsonify({"status": "error", "message": f"自拍机 {machine_id} 不存在"}), 404
        ...
        if success:
            logger.info(f"成功保存自拍机 {machine_id} 的打印机配置")
            return jsonify(
                {"status": "success", "message": f"自拍机 {machine_id} 的打印机配置保存成功"}
            )
        else:
            logger.error(f"保存自拍机 {machine_id} 的打印机配置失败")
            return jsonify({"status": "error", "message": "保存失败，请查看服务器日志"}), 500
    except Exception as e:
        import traceback

        logger.error(f"保存门店打印机配置失败: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({"status": "error", "message": f"保存门店打印机配置失败: {str(e)}"}), 500
...
def api_test_print():
    ...
    try:
        ...
        logger.info(f"✅ 收到文件: {file.filename}, 类型: {file.content_type}")
        ...
        if not file_ext:
            file_ext = ".txt"
            logger.warning("未检测到文件扩展名，默认使用: {file_ext}")
        ...
        try:
            ...
            if local_printer_proxy_url:
                ...
                try:
                    ...
                    logger.info(f"✅ 文件已读取: {len(file_content)} 字节, 扩展名: {file_ext}")
                    ...
                    return jsonify({"status": "error", "message": "打印代理客户端模块未找到"}), 500
                except Exception as e:
                ...
                return (
                    jsonify({"status": "error", "message": "未配置本地打印机或打印代理服务"}),
                    400,
                )
        finally:
            # 清理临时文件
            if "temp_file" in locals():
                try:
                    ...
                    logger.warning("清理临时文件失败: {str(e)}")

    except Exception as e:
        import traceback

        error_trace = traceback.format_exc()
        logger.error("测试打印异常: {str(e)}")
        logger.info(error_trace)
        return jsonify({"status": "error", "message": f"测试打印失败: {str(e)}"}), 500
...
def api_get_print_size_configs():
    """获取打印尺寸配置列表"""
    try:
        # 尝试获取所有模型，然后检查PrintSizeConfig
        models = get_models()
        if not models:
            logger.error("获取打印尺寸配置失败: 系统未初始化")
            return jsonify({"status": "error", "message": "系统未初始化"}), 500
        ...
        if not PrintSizeConfig:
            logger.error("获取打印尺寸配置失败: PrintSizeConfig模型未找到。")
            logger.error(
                f"可用模型列表: {sorted([k for k in models.keys() if models[k] is not None])}"
            )
            # 尝试直接导入
            try:
                import sys
                ...
                if "test_server" in sys.modules:
                    ...
                    if PrintSizeConfig:
                        logger.info("通过test_server模块直接获取PrintSizeConfig成功")
                    else:
                        # 尝试从app.models直接导入
                        try:
                            from app.models import PrintSizeConfig as PrintSizeConfigModel

                            PrintSizeConfig = PrintSizeConfigModel
                            logger.info("通过app.models直接导入PrintSizeConfig成功")
                        except ImportError as import_err:
                            logger.error(
                                f"test_server模块中也没有PrintSizeConfig，从app.models导入也失败: {import_err}"
                            )
                            return (
                                jsonify(
                                    {
                                        "status": "error",
                                        "message": "打印配置模型未找到，请检查数据库表是否存在",
                                    }
                                ),
                                500,
                            )
                    ...
                    return (
                        jsonify({"status": "error", "message": "打印配置模型未找到，系统未初始化"}),
                        500,
                    )
            except Exception as import_error:
                logger.error(f"尝试直接导入PrintSizeConfig失败: {import_error}")
                return (
                    jsonify(
                        {"status": "error", "message": f"打印配置模型未找到: {str(import_error)}"}
                    ),
                    500,
                )

        configs = PrintSizeConfig.query.order_by(
            PrintSizeConfig.sort_order, PrintSizeConfig.product_id
        ).all()
        ...
        logger.error(f"获取打印尺寸配置失败: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({"status": "error", "message": f"获取打印尺寸配置失败: {str(e)}"}), 500
...
def api_save_print_size_config():
    ...
    try:
        ...
        models = get_models()
        if not models:
            logger.error("保存打印尺寸配置失败: 系统未初始化")
            return jsonify({"status": "error", "message": "系统未初始化"}), 500
        ...
        if not db:
            logger.error("保存打印尺寸配置失败: db对象未找到")
            return jsonify({"status": "error", "message": "数据库对象未找到"}), 500

        if not PrintSizeConfig:
            logger.error("保存打印尺寸配置失败: PrintSizeConfig模型未找到。")
            logger.error(
                f"可用模型列表: {sorted([k for k in models.keys() if models[k] is not None])}"
            )
            # 尝试直接导入
        ...
        return jsonify(
            {"status": "success", "message": "打印尺寸配置保存成功", "data": {"id": config.id}}
        )
    except Exception as e:
        import traceback

        logger.error(f"保存打印尺寸配置失败: {str(e)}")
        logger.error(traceback.format_exc())
        if "db" in locals():
            db.session.rollback()
        return jsonify({"status": "error", "message": f"保存打印尺寸配置失败: {str(e)}"}), 500
...
def api_delete_print_size_config(config_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "message": "打印尺寸配置删除成功"})
    except Exception as e:
        if "db" in locals():
            db.session.rollback()
        return jsonify({"status": "error", "message": f"删除打印尺寸配置失败: {str(e)}"}), 500
...
def api_get_printer_orders():
    ...
    try:
        ...
        if not Order:
            ...
            return jsonify({"status": "error", "message": "Order模型未找到"}), 500
        ...
        if ShopOrder:
            try:
                ...
                # 检查模型是否有printer_send_status字段
                has_printer_status = hasattr(ShopOrder, "printer_send_status")
                ...
                if status_filter and has_printer_status:
                    try:
                        ...
                        # 如果字段不存在于数据库表中，忽略过滤
                        logger.warning(
                            f"过滤printer_send_status失败（字段可能不存在）: {filter_error}"
                        )
                ...
                for shop_order in pagination.items:
                    try:
                        order_data = {
                            ...
                            "printer_send_status": getattr(
                                shop_order, "printer_send_status", "not_sent"
                            )
                            or "not_sent",
                            "printer_send_time": (
                                shop_order.printer_send_time.isoformat()
                                if hasattr(shop_order, "printer_send_time")
                                and shop_order.printer_send_time
                                else None
                            ),
                            "printer_error_message": getattr(
                                shop_order, "printer_error_message", None
                            ),
                            "image_url": shop_order.image_url,
                            "created_at": (
                                shop_order.created_at.isoformat() if shop_order.created_at else None
                            ),
                        }
                        orders.append(order_data)
                    except Exception as order_error:
                        logger.warning(f"处理订单数据失败: {order_error}")
                        continue
            ...
            except Exception as shop_order_error:
                logger.error(f"查询ShopOrder失败: {shop_order_error}")
                # 如果ShopOrder查询失败，回退到Order表
                ShopOrder = None

        # 如果没有ShopOrder模型或查询失败，查询Order表
        if not ShopOrder:
            query = Order.query
            ...
            if status_filter:
                if status_filter == "not_sent":
                    query = query.filter(
                        (Order.printer_send_status.is_(None))
                        | (Order.printer_send_status == "not_sent")
                    )
                elif status_filter == "sending":
            ...
            for order in pagination.items:
                order_data = {
                    ...
                    "printer_send_time": (
                        order.printer_send_time.isoformat() if order.printer_send_time else None
                    ),
        ...
        traceback.print_exc()
        return jsonify({"status": "error", "message": f"获取打印任务列表失败: {str(e)}"}), 500
...
def api_resend_printer_order(order_id):
    ...
    try:
        ...
        if result.get("success"):
            return jsonify({"status": "success", "message": "打印任务已重新发送"})
        else:
            return jsonify(
                {"status": "error", "message": f'发送失败: {result.get("message", "未知错误")}'}
            )
        ...
        traceback.print_exc()
        return jsonify({"status": "error", "message": f"重新发送失败: {str(e)}"}), 500
