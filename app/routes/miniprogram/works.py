...
def miniprogram_get_works():
    ...
    try:
        ...
        if openid:
            ...
            logger.info(f"用户 {openid} 查询到 {len(orders)} 个作品")

            # 记录查询日志（用于安全审计）
            logger.info(
                f"作品查询日志: openid={openid}, 结果数量={len(orders)}, 时间={datetime.now()}"
            )
        ...
        # 临时支持手机号查询（兼容旧版本）
            ...
            logger.info(f"手机号 {phone} 查询到 {len(orders)} 个作品")

            # 记录查询日志（用于安全审计）
            logger.info(
                f"作品查询日志: phone={phone}, 结果数量={len(orders)}, 时间={datetime.now()}"
            )

        # 如果都没有提供，返回错误
        if not openid and not phone:
            return (
                jsonify({"status": "error", "message": "缺少openid或phone参数，无法查询作品"}),
                400,
            )

        logger.info(f"查询到 {len(orders)} 个作品订单")
        for order in orders:
            logger.info(f"订单 {order.order_number}: 状态={order.status}, 图片={order.final_image}")
