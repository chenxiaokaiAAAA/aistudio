...
def miniprogram_get_promotion_code():
    ...
    try:
        ...
        print(f"生成推广码小程序码: promotionCode={promotion_code}, userId={user_id}, env_version=release")
        ...
        if response.status_code == 200:
            ...
            if 'application/json' in content_type:
                ...
                return jsonify({'status': 'error', 'message': f'生成小程序码失败: {error_data.get("errmsg", "未知错误")}'}), 500
            ...
            print(f"微信API调用失败: HTTP {response.status_code}")
            return jsonify({'status': 'error', 'message': f'生成小程序码失败: HTTP {response.status_code}'}), 500
        
    except Exception as e:
        print(f"获取推广码失败: {str(e)}")
        import traceback
        traceback.print_exc()
