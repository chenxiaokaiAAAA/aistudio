...
def miniprogram_get_shop_products():
    ...
    try:
        ...
        if "ShopProduct" not in models:
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": "商城模型未初始化，请运行创建商城表.py脚本创建数据库表",
                    }
                ),
                500,
            )
        ...
        products = (
            ShopProduct.query.filter(or_(ShopProduct.is_active is True, ShopProduct.is_active == 1))
            .order_by(ShopProduct.sort_order.asc(), ShopProduct.id.asc())
            .all()
        )

        logger.info(f"DEBUG: 查询到 {len(products)} 个启用的产品")
        if len(products) == 0:
            # 如果查询结果为空，查询所有产品看看
            all_products = ShopProduct.query.all()
            logger.info(f"DEBUG: 数据库中共有 {len(all_products)} 个产品")
            for p in all_products:
                logger.info(
                    f"DEBUG: 产品ID={p.id}, 名称={p.name}, is_active={p.is_active} (类型: {type(p.is_active).__name__})"
                )
        ...
        if product_ids:
            ...
            all_images = (
                ShopProductImage.query.filter(
                    ShopProductImage.product_id.in_(product_ids), ShopProductImage.is_active is True
                )
            ...
            all_sizes = (
                ShopProductSize.query.filter(
                    ShopProductSize.product_id.in_(product_ids), ShopProductSize.is_active is True
                )
        ...
        logger.info(f"获取商城产品列表失败: 模型未找到 - {str(e)}")
        import traceback

        traceback.print_exc()
        return (
            jsonify({"status": "error", "message": "商城模型未初始化，请确保数据库表已创建"}),
            500,
        )
    except Exception as e:
        logger.info(f"获取商城产品列表失败: {str(e)}")
        import traceback
...
def miniprogram_get_shop_product_detail(product_id):
    ...
    try:
        ...
        # 获取产品图片
        images = (
            ShopProductImage.query.filter_by(product_id=product.id, is_active=True)
            .order_by(ShopProductImage.sort_order.asc())
            .all()
        )
        ...
        # 获取产品规格
        sizes = (
            ShopProductSize.query.filter_by(product_id=product.id, is_active=True)
            .order_by(ShopProductSize.sort_order.asc())
            .all()
        )
        ...
        logger.info(f"获取商城产品详情失败: {str(e)}")
        import traceback
...
def miniprogram_create_shop_order():
    ...
    try:
        ...
        # 验证规格
        size = ShopProductSize.query.filter_by(
            id=data["size_id"], product_id=data["product_id"], is_active=True
        ).first()
        if not size:
...
def miniprogram_get_shop_orders():
    ...
    try:
        ...
        # 获取用户的订单
        orders = (
            ShopOrder.query.filter_by(openid=openid).order_by(ShopOrder.created_at.desc()).all()
        )
        ...
        logger.info(f"获取商城订单列表失败: {str(e)}")
        import traceback
