...
# 统一导入公共函数（小程序使用miniprogram/common）
...
def request_refund():
    ...
    try:
        ...
        if not check_staff_permission(openid, user_id, phone):
            return (
                jsonify({"status": "error", "message": "权限不足，只有授权的店员可以申请退款"}),
                403,
            )
        ...
        if not reason:
            return jsonify({"status": "error", "message": "退款原因不能为空"}), 400
        ...
        if order.status in ["cancelled", "refunded"]:
            return jsonify({"status": "error", "message": "该订单已取消或已退款"}), 400
        ...
        logger.info(f"申请退款失败: {str(e)}")
        traceback.print_exc()
        if "db" in locals():
            db.session.rollback()
        return jsonify({"status": "error", "message": f"申请退款失败: {str(e)}"}), 500
...
def check_order():
    ...
    try:
        ...
        if not order_number and not phone:
            return jsonify({"status": "error", "message": "订单号或手机号不能为空"}), 400
...
def check_refund_permission():
    ...
    try:
        ...
        return jsonify({"status": "success", "data": {"has_permission": is_staff}})
    except Exception as e:
        traceback.print_exc()
