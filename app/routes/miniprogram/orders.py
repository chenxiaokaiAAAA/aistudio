...
def miniprogram_submit_order():
    ...
    try:
        ...
        logger.info(f"收到小程序订单数据: {data}")

        # 调用服务层函数创建订单
        success, result, error_message = create_miniprogram_order(data)

        if success:
            response_data = {"status": "success", "message": "订单提交成功", **result}
            logger.info(f"✅ 订单创建成功，返回数据: {response_data}")
            return jsonify(response_data)
        else:
            logger.error("订单创建失败: {error_message}")
            status_code = (
                400
                if "缺少" in error_message or "不足" in error_message or "不存在" in error_message
                else 500
            )
...
def miniprogram_get_orders():
    ...
    try:
        ...
        orders = []
        pagination = None  # 初始化分页对象

        # 优先使用openid查询
        if openid:
            invalid_openids = ["anonymous", "undefined", "null", ""]
            if openid in invalid_openids:
                logger.warning("openid无效: {openid}，开发模式：尝试查询所有小程序订单（最近10条）")
                orders = (
                    Order.query.filter(Order.source_type == "miniprogram")
                ...
                logger.warning(
                    "openid格式不正确: {openid}，开发模式：尝试查询所有小程序订单（最近10条）"
                )
                orders = (
                    Order.query.filter(Order.source_type == "miniprogram")
                    .order_by(Order.created_at.desc())
                ...
                pagination = (
                    Order.query.filter(Order.openid == openid, Order.source_type == "miniprogram")
                    .order_by(Order.created_at.desc())
                ...
                if len(orders) == 0 and openid == "test_openid_dev_mode_fixed":
                    logger.warning(
                        "开发模式：通过openid查询不到订单，尝试查询所有小程序订单（最近10条）"
                    )
                    orders = (
                        Order.query.filter(Order.source_type == "miniprogram")
                        .order_by(Order.created_at.desc())
            ...
            if user and user.open_id:
                ...
                pagination = (
                    Order.query.filter(
                        Order.openid == user.open_id, Order.source_type == "miniprogram"
                    )
                    .order_by(Order.created_at.desc())
                    .paginate(page=page, per_page=per_page, error_out=False)
                )
            ...
            pagination = (
                Order.query.filter(
                    Order.customer_phone == phone, Order.source_type == "miniprogram"
                )
                .order_by(Order.created_at.desc())
                .paginate(page=page, per_page=per_page, error_out=False)
            )
        ...
        # 优化N+1查询：批量获取所有订单的图片
        order_ids = [order.id for order_group in orders_by_number.values() for order in order_group]
        all_order_images = {}
        if order_ids:
            # 使用selectinload批量加载图片
            images_query = OrderImage.query.filter(OrderImage.order_id.in_(order_ids)).all()
            for img in images_query:
                if img.order_id not in all_order_images:
                    all_order_images[img.order_id] = []
                all_order_images[img.order_id].append(img)
        ...
        for order_number, order_group in orders_by_number.items():
            ...
            for order in order_group:
                images = all_order_images.get(order.id, [])
                for img in images:
            ...
            if main_order.final_image:
                ...
                if main_order.status in ["manufacturing", "completed", "shipped", "delivered"]:
                    final_image_no_watermark_url = (
                        f"{get_media_url()}/final/clean_{main_order.final_image}"
                    )
            ...
            if main_order.hd_image:
                ...
                if main_order.status in ["manufacturing", "completed", "shipped", "delivered"]:
                    ...
                    hd_image_no_watermark_url = (
                        f"{get_base_url()}/public/hd/{encoded_clean_filename}"
                    )
            ...
            total_price = 0
            for order in order_group:
                item_images = all_order_images.get(order.id, [])
            ...
            for order in order_group:
                if order.status in status_priority:
                    if main_status not in status_priority or status_priority[
                        order.status
                    ] < status_priority.get(main_status, 999):
            ...
            order_list.append(
                {
                    ...
                    "styleName": main_order.style_name
                    or "威廉国王",  # 向后兼容：使用第一个商品的风格
                    "productName": main_order.product_name
                    or "艺术钥匙扣",  # 向后兼容：使用第一个商品的产品名
                    ...
                    "completeTime": (
                        main_order.completed_at.isoformat() if main_order.completed_at else None
                    ),
        ...
        # 构建响应（包含分页信息，如果使用了分页）
        response_data = {"status": "success", "orders": order_list}
...
def miniprogram_get_order_by_number(order_number):
    ...
    try:
        ...
        logger.info(f"通过订单号查询订单失败: {str(e)}")
...
def android_check_order():
    ...
    try:
        ...
        machine_serial_number = (
            request.args.get("machineSerialNumber")
            or request.args.get("machine_serial_number")
            or request.args.get("selfie_machine_id")
        )
        ...
        # 调用服务层函数
        success, order_data, error_message = check_order_for_verification(
            order_id, machine_serial_number
        )
        ...
        if success:
            ...
            if "已经拍摄过" in error_message:
                return (
                    jsonify({"success": False, "message": error_message, "order": order_data}),
                    400,
                )
            else:
                status_code = 404 if "不存在" in error_message else 500
        ...
        logger.info(f"检查订单状态失败: {str(e)}")
        import traceback

        traceback.print_exc()
        return jsonify({"success": False, "message": f"检查订单失败: {str(e)}"}), 500
...
def generate_order_qrcode():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "orderId": order_id, "qrContent": qr_content, "qrImage": qr_image_url}
        )

    except Exception as e:
        logger.info(f"生成订单二维码失败: {str(e)}")
        import traceback
...
def generate_shooting_qrcode(order_id):
    ...
    try:
        ...
        if not order:
            logger.error("订单不存在: order_id={order_id}, 类型={type(order_id)}")
            # 尝试查找最近创建的订单（用于调试）
            recent_orders = Order.query.order_by(Order.created_at.desc()).limit(5).all()
            logger.info(f"最近5个订单: {[(o.id, o.order_number, o.status) for o in recent_orders]}")
            return jsonify({"success": False, "message": f"订单不存在: {order_id}"}), 404

        # 对于多个商品订单（同一订单号有多条记录），需要更新所有相关订单
        order_number = order.order_number
        all_orders = Order.query.filter_by(order_number=order_number).all()

        logger.info(f"🔍 找到 {len(all_orders)} 条订单记录，订单号: {order_number}")

        # 更新所有相关订单的 order_mode
        for o in all_orders:
            o.order_mode = "shooting"
            logger.info(
                f"✅ 已更新订单 ID={o.id}, order_mode='shooting', order_number={o.order_number}"
            )
        ...
        logger.info(f"生成拍摄二维码失败: {str(e)}")
        import traceback

        traceback.print_exc()
        return (
            jsonify({"success": False, "status": "error", "message": f"生成二维码失败: {str(e)}"}),
            500,
        )
...
def update_order_mode(order_id):
    ...
    try:
        ...
        if order_mode not in ["shooting", "making"]:
            return (
                jsonify({"success": False, "message": "订单类型无效，必须是 shooting 或 making"}),
                400,
            )
        ...
        if order.status not in ["paid", "已支付"]:
            return jsonify({"success": False, "message": "只有已支付的订单才能切换类型"}), 400
        ...
        for o in all_orders:
            ...
            if order_images:
                return jsonify({"success": False, "message": "订单已上传图片，无法切换类型"}), 400
        ...
        if order_mode == "making" and order_number.startswith("MP"):
            ...
            logger.info(
                f"✅ 订单类型切换为'立即制作'，将更新订单号: {order_number} -> {new_order_number}"
            )
        # 如果切换为"立即拍摄"且订单号是XSDD-开头，改回MP-开头
            ...
            logger.info(
                f"✅ 订单类型切换为'立即拍摄'，将更新订单号: {order_number} -> {new_order_number}"
            )
        ...
        for o in all_orders:
            ...
            logger.info(
                f"✅ 已更新订单 ID={o.id}, order_mode='{order_mode}', order_number={o.order_number}"
            )
            ...
            try:
                ...
                if AITask:
                    ...
                    updated_tasks = AITask.query.filter(AITask.order_id.in_(order_ids)).update(
                        {AITask.order_number: new_order_number}, synchronize_session=False
                    )
                    if updated_tasks > 0:
                        logger.info(
                            f"✅ 已更新 {updated_tasks} 个AI任务的订单号: {order_number} -> {new_order_number}"
                        )
            except Exception as e:
                logger.warning("更新AI任务订单号失败（不影响主流程）: {str(e)}")

        db.session.commit()

        logger.info(
            f"✅ 订单类型已更新: {order_number} -> {new_order_number}, {old_mode} -> {order_mode}"
        )
        ...
        if "db" in locals():
            db.session.rollback()
        logger.info(f"更新订单类型失败: {str(e)}")
        import traceback

        traceback.print_exc()
        return (
            jsonify(
                {"success": False, "status": "error", "message": f"更新订单类型失败: {str(e)}"}
            ),
            500,
        )
...
def set_order_as_manual(order_id):
    ...
    try:
        ...
        if order_number and order_number.startswith("MP"):
            ...
            new_order_number = (
                f"XSDD-{datetime.now().strftime('%Y%m%d%H%M%S')}{uuid.uuid4().hex[:4].upper()}"
            )
            old_order_number = order_number
            logger.info(f"✅ 立即制作订单，将更新订单号: {old_order_number} -> {new_order_number}")
        elif order_number and order_number.startswith("XSDD-"):
            logger.info(f"ℹ️ 订单号已经是XSDD-开头，无需更新: {order_number}")
        else:
            logger.warning("订单号格式异常: {order_number}")
        ...
        for o in all_orders:
            ...
            if new_order_number != order_number:
                o.order_number = new_order_number
            logger.info(
                f"✅ 已更新订单 ID={o.id}, order_mode='making', order_number={o.order_number}"
            )
        ...
        logger.info(f"标记人工订单失败: {str(e)}")
        import traceback

        traceback.print_exc()
        if "db" in locals():
            db.session.rollback()
        return jsonify({"success": False, "status": "error", "message": f"标记失败: {str(e)}"}), 500
...
def android_upload_photos():
    ...
    try:
        ...
        if "test_server" in sys.modules:
            ...
            logger.info("✅ 从test_server模块获取db和app实例成功")
        else:
            ...
            db = app_db
            app = current_app
            logger.info("✅ 从current_app获取db和app实例成功")

        order_id = request.form.get("orderId") or request.form.get("order_id")
        machine_serial_number = (
            request.form.get("machineSerialNumber")
            or request.form.get("machine_serial_number")
            or request.form.get("selfie_machine_id")
        )

        logger.info(f"订单ID: {order_id}")
        logger.info(f"自拍机序列号: {machine_serial_number}")
        ...
        if not order.franchisee_id and machine_serial_number:
            try:
                machine = SelfieMachine.query.filter_by(
                    machine_serial_number=machine_serial_number, status="active"
                ).first()
                ...
                if machine and machine.franchisee:
                    ...
                    order.external_platform = (
                        machine.machine_name or order.external_platform or "miniprogram"
                    )
                    order.external_order_number = machine_serial_number

                    logger.info(
                        f"✅ 订单 {order_id} 已通过自拍机序列号 {machine_serial_number} 关联到加盟商: {franchisee.company_name} (门店: {franchisee.store_name}, 设备: {machine.machine_name})"
                    )
            except Exception as e:
                logger.warning("通过自拍机序列号关联加盟商失败: {e}")

        # 检查是否有上传的文件（Android App使用'photos'作为字段名）
        logger.info(f"检查上传文件，request.files.keys(): {list(request.files.keys())}")

        if "photos" not in request.files:
            logger.error("request.files中没有'photos'字段")
            # 尝试查找所有包含'photo'的字段
            photo_keys = [key for key in request.files.keys() if "photo" in key.lower()]
            if photo_keys:
                logger.warning("找到类似的字段: {photo_keys}")
            return (
                jsonify(
                    {
                        "success": False,
        ...
        for idx, file in enumerate(files):
            if file.filename == "":
                logger.warning("文件 {idx} 的文件名为空，跳过")
                continue
            ...
            try:
                ...
                logger.error("文件保存失败: {save_error}")
                raise
            ...
            if file_size > 5 * 1024 * 1024:  # 5MB
                try:
                    ...
                    logger.info(f"图片压缩失败: {compress_error}")
        ...
        if order.order_mode == "shooting":
            ...
            try:
                MeituAPIConfig = (
                    test_server_module.MeituAPIConfig if "test_server" in sys.modules else None
                )
                if MeituAPIConfig:
                    ...
                    if meitu_config and meitu_config.enable_in_workflow:
                        ...
                        if order.status in ["pending", "unpaid", "paid", "shooting"]:
                            ...
                            logger.info(
                                f"✅ 订单 {order.order_number} 状态已更新为: retouching（美颜处理中）- 已接收全部图片，开始美图API处理"
                            )
                    else:
                        ...
                        if order.status in ["pending", "unpaid", "paid", "shooting"]:
                            ...
                            logger.info(
                                f"✅ 订单 {order.order_number} 状态已更新为: ai_processing（AI任务处理中）- 已接收全部图片，跳过美图API"
                            )
                else:
                    ...
                    if order.status in ["pending", "unpaid", "paid", "shooting"]:
                        order.status = "ai_processing"  # AI任务处理中
                        logger.info(
                            f"✅ 订单 {order.order_number} 状态已更新为: ai_processing（AI任务处理中）- 已接收全部图片"
                        )
            except Exception as e:
                logger.warning("检查美图API配置失败: {e}，默认更新为AI任务处理中")
                if order.status in ["pending", "unpaid", "paid", "shooting"]:
                    order.status = "ai_processing"  # AI任务处理中
        else:
            ...
            if order.status in ["pending", "unpaid", "paid"]:
                ...
                logger.info(f"✅ 订单 {order.order_number} 状态已更新为: shooting（正在拍摄）")
        ...
        if order.order_mode == "shooting":
            ...
            try:
                ...
                # 获取风格分类ID（从订单中获取）
                style_category_id = (
                    order.style_category_id if hasattr(order, "style_category_id") else None
                )
                ...
                logger.info("✅ 立即拍摄订单，已启动后台图片处理流程（美图API + AI工作流）")
            except Exception as e:
                logger.warning("启动图片处理流程失败: {str(e)}")
                # 不影响上传成功的返回
        elif order.order_mode == "making":
            # 立即制作：不进行任何自动化处理，等待人工处理
            logger.info("ℹ️ 立即制作订单，跳过自动化处理流程，等待人工处理")
        else:
            # 未设置订单类型：默认进入自动化流程（兼容旧订单）
            logger.warning("订单类型未设置，默认进入自动化流程")
            try:
                from app.services.image_processing_service import process_order_images
                ...
                processing_thread.daemon = True
                processing_thread.start()
                logger.info("✅ 已启动后台图片处理流程（美图API + AI工作流）")
            except Exception as e:
                logger.warning("启动图片处理流程失败: {str(e)}")
        ...
        if "test_server" in sys.modules:
            ...
            if hasattr(test_server_module, "db"):
                test_server_module.db.session.rollback()
        logger.info(f"上传订单照片失败: {str(e)}")
        import traceback

        traceback.print_exc()
        return jsonify({"success": False, "message": f"上传照片失败: {str(e)}"}), 500
...
def miniprogram_update_order_status(order_id):
    ...
    try:
        ...
        app = models.get("app")
        send_order_completion_notification_auto = helpers.get(
        ...
        if status == "completed" and send_order_completion_notification_auto:
            # 检查是否为加盟商订单且需要确版
            if (
                hasattr(order, "franchisee_id")
                and order.franchisee_id
                and hasattr(order, "need_confirmation")
                and order.need_confirmation
                and not getattr(order, "franchisee_confirmed", False)
            ):
                # 加盟商订单需要确版，不发送通知
                pass
            else:
                    ...
                    if hasattr(order, "hd_image") and order.hd_image:
                        ...
                        if os.path.exists(hd_image_path):
                            ...
                            result = printer_client.send_order_to_printer(
                                order, hd_image_path, order_obj=order
                            )
                            ...
                            if result["success"]:
                                logger.info(
                                    f"订单 {order.order_number} 高清图片已成功发送到冲印系统"
                                )
                                # 发送成功后，更新状态为已发货
                                ...
                                logger.info(
                                    f"订单 {order.order_number} 高清图片发送到冲印系统失败: {result['message']}"
                                )
                        else:
                            logger.info(
                                f"订单 {order.order_number} 高清图片不存在: {hd_image_path}"
                            )
                        ...
                        logger.info(f"订单 {order.order_number} 没有高清图片，跳过冲印系统发送")
                        if hasattr(order, "printer_send_status"):
                            order.printer_send_status = "sent_failed"
                            order.printer_error_message = "订单没有高清图片"
                            db.session.commit()
            except ImportError:
                logger.info("冲印系统模块未找到，跳过冲印系统发送")
            except Exception as e:
                logger.info(f"发送订单到冲印系统时发生错误: {str(e)}")
                if hasattr(order, "printer_send_status"):
                    order.printer_send_status = "sent_failed"
        ...
        logger.info(f"订单 {order_id} 状态更新为: {status} ({mapped_status_text})")
...
def update_order_images(order_id):
    ...
    try:
        ...
        logger.info(
            f"图片更新请求: 订单={order_id}, 替换模式={is_replace_mode}, 替换索引={replace_index}"
        )
        ...
        if is_replace_mode and replace_index >= 0:
            # 替换模式：只替换指定索引的图片
            existing_images = (
                OrderImage.query.filter_by(order_id=order.id).order_by(OrderImage.id).all()
            )
            ...
            logger.info(f"✅ 图片替换成功: 索引{replace_index}, 新路径={new_image_path}")
        ...
        if uploaded_images:
            logger.info(f"处理uploadedImages字段中的图片，数量: {len(uploaded_images)}")
            for img_data in uploaded_images:
                if img_data.get("filename"):
                    # 使用已上传的文件名
                    filename = img_data["filename"]
                    logger.info(f"保存已上传图片: {filename}")

                    order_image = OrderImage(order_id=order.id, path=filename, is_main=False)
                    db.session.add(order_image)
                    processed_images.append(filename)
                elif img_data.get("url"):
                    # 处理图片URL
                    img_url = img_data["url"]
                    logger.info(f"处理图片URL: {img_url}")
                    ...
                    if img_url.startswith("http://tmp/") or img_url.startswith("tmp/"):
                        ...
                        logger.info(f"创建占位符图片: {img_path}")

                        # 保存本地路径
                        order_image = OrderImage(order_id=order.id, path=img_filename)
                        db.session.add(order_image)
                        ...
                        placeholder_img.save(img_path, "JPEG")
                        logger.info(f"创建网络图片占位符: {img_path}")
                        ...
                        # 本地图片
                        order_image = OrderImage(order_id=order.id, path=img_url)
                        db.session.add(order_image)
                        processed_images.append(img_url)
                else:
                    logger.info(f"图片数据格式不正确: {img_data}")

        # 处理images字段（兼容直接传入图片路径列表）
        elif images:
            logger.info(f"处理images字段中的图片，数量: {len(images)}")
            for image_url in images:
                ...
                if image_url.startswith("http://tmp/") or image_url.startswith("tmp/"):
                    ...
                    # 本地图片路径
                    order_image = OrderImage(order_id=order.id, path=image_url)
                    db.session.add(order_image)
        ...
        if processed_images and order.order_mode == "making":
            if order.status in ["paid", "已支付"]:
                ...
                logger.info(f"✅ 立即制作订单 {order.order_number} 已上传图片，状态已更新为待制作")
        ...
        if processed_images and order.status in ["unpaid"]:
            ...
            if order.payment_time:
                ...
                # 如果订单未支付，不应该更新状态
                logger.warning("警告：订单 {order_id} 未支付，但尝试更新状态为pending")
                db.session.rollback()
                return (
                    jsonify(
                        {
        ...
        if order.order_mode == "shooting":
            ...
            try:
                ...
                if app_instance:
                    ...
                    logger.warning("无法获取应用实例，跳过后台图片处理")
            except Exception as e:
                logger.warning("启动图片处理流程失败: {str(e)}")
        elif order.order_mode == "making":
            # 立即制作：不进行任何自动化处理，等待人工处理
...
def delete_order_image(order_id):
    ...
    try:
        ...
        logger.info(f"删除图片请求: 订单={order_id}, 索引={image_index}, URL={image_url}")
        ...
        if image_index >= 0 and image_index < len(existing_images):
            ...
            logger.info(f"✅ 图片删除成功: 索引{image_index}, 图片ID={deleted_image.id}")
            ...
            for img in existing_images:
                if img.path == filename or image_url.endswith(img.path):
                    ...
                    if img.id == existing_images[0].id:
                        remaining_images = (
                            OrderImage.query.filter_by(order_id=order.id)
                            .order_by(OrderImage.id)
                            .all()
                        )
                        if remaining_images:
                            order.original_image = remaining_images[0].path
                    ...
                    db.session.commit()
                    logger.info(f"✅ 图片删除成功: 图片ID={img.id}, 路径={img.path}")
                    break
