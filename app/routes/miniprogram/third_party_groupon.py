...
def _sign_request(data, secret):
    """可选：生成请求签名（HMAC-SHA256），供 require_signature 时使用"""
    import hashlib
    import hmac

    payload = json.dumps(data or {}, sort_keys=True, ensure_ascii=False)
    sig = hmac.new(
...
def call_third_party_api(endpoint, method="GET", data=None, config=None):
    ...
    try:
        ...
        # 可选：第三方API若要求请求签名，在 config 中设置 require_signature=true 和 sign_secret
        if config.get("require_signature") and config.get("sign_secret"):
            headers["X-Signature"] = _sign_request(data, config["sign_secret"])
...
def query_groupon_codes():
    ...
    try:
        ...
        if not phone:
            return jsonify({"status": "error", "message": "请提供手机号"}), 400

        config = get_third_party_config()
        if not config:
            return jsonify({"status": "error", "message": "第三方团购API未配置或已禁用"}), 400

        query_endpoint = config.get("query_endpoint", "/api/v1/query")

        # 调用第三方API查询券码
        result, error = call_third_party_api(query_endpoint, method="POST", data={"phone": phone})
        ...
        return jsonify({"status": "success", "data": {"codes": codes}})

    except Exception as e:
        logger.info(f"查询团购券码失败: {str(e)}")
        import traceback
...
def verify_groupon_code():
    ...
    try:
        ...
        if not code:
            return jsonify({"status": "error", "message": "请提供券码"}), 400
        ...
        # 调用第三方API验证券码
        result, error = call_third_party_api(verify_endpoint, method="POST", data={"code": code})
        ...
        if result and result.get("success"):
            ...
            return (
                jsonify({"status": "error", "message": result.get("message", "券码验证失败")}),
                400,
            )

    except Exception as e:
        logger.info(f"验证券码失败: {str(e)}")
        import traceback
...
def redeem_groupon_code():
    ...
    try:
        ...
        verify_result, verify_error = call_third_party_api(
            verify_endpoint, method="POST", data={"code": code}
        )

        if verify_error or not verify_result or not verify_result.get("success"):
            return jsonify({"status": "error", "message": verify_error or "券码验证失败"}), 400

        code_info = verify_result.get("data", {})
        amount = float(code_info.get("amount", 0))

        if amount <= 0:
            return jsonify({"status": "error", "message": "券码金额无效"}), 400

        # 检查是否已核销过
        existing_coupon = Coupon.query.filter_by(
            groupon_order_id=code, source_type="groupon"
        ).first()

        if existing_coupon:
            return jsonify({"status": "error", "message": "该券码已核销"}), 400

        # 调用第三方API核销券码
        redeem_endpoint = config.get("redeem_endpoint", "/api/v1/redeem")
        redeem_result, redeem_error = call_third_party_api(
            redeem_endpoint, method="POST", data={"code": code, "phone": phone}
        )

        if redeem_error or not redeem_result or not redeem_result.get("success"):
            return jsonify({"status": "error", "message": redeem_error or "券码核销失败"}), 400
        ...
        if openid or user_id:
            user_coupon = UserCoupon(
                user_id=user_id or openid, coupon_id=coupon.id, status="unused", obtained_at=now
            )
        ...
        logger.info(f"核销券码失败: {str(e)}")
        import traceback

        traceback.print_exc()
        if "db" in locals():
            db.session.rollback()
        return jsonify({"status": "error", "message": f"核销失败: {str(e)}"}), 500
