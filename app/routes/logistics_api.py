...
def printer_logistics_callback():
    ...
    try:
        ...
        logger.info(f"✅ 订单 {order.order_number} 物流信息已更新:")
        logger.info(f"   快递公司: {logistics_company}")
        logger.info(f"   快递单号: {tracking_number}")
        logger.info(f"   状态: {status}")

        return jsonify(
            {"success": True, "message": "物流信息更新成功", "order_number": order.order_number}
        )

    except Exception as e:
        if "db" in locals():
            db.session.rollback()
        logger.info(f"物流信息回传失败: {str(e)}")
        import traceback
...
def logistics_callback():
    ...
    try:
        ...
        logger.info(f"✅ 订单 {order.order_number} 物流信息已更新（回调）:")
        logger.info(f"   快递公司: {logistics_company}")
        logger.info(f"   快递单号: {tracking_number}")
