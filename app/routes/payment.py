...
from app.services.payment_service import get_user_openid as get_user_openid_service
from app.services.payment_service import (
...
def create_payment():
    ...
    try:
        ...
        logger.info(
            f"🎫 支付请求参数: orderId={order_id}, totalPrice={total_price}, couponCode={coupon_code}, discountAmount={discount_amount}, userId={user_id}, skipPayment={skip_payment}"
        )
        ...
        return jsonify({"success": False, "message": f"创建支付订单失败: {str(e)}"}), 500
...
def payment_notify():
    ...
    try:
        ...
        if success:
            ...
            return (
                response_xml
                or f'<xml><return_code><![CDATA[FAIL]]></return_code><return_msg><![CDATA[{error_message or "处理失败"}]]></return_msg></xml>',
                200,
