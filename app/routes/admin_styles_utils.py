...
def _is_from_playground(request):
    ...
    return '/playground' in referer or request.form.get('from_playground') == 'true' or request.get_json(silent=True) and request.get_json(silent=True).get('from_playground') == True
...
def _get_test_order_info(request):
    ...
    if is_playground:
        ...
        customer_name = current_user.username if current_user and current_user.is_authenticated else 'PLAYGROUND'
