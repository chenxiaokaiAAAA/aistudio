...
@bp.route("/franchisee/<int:franchisee_id>/staf")
@login_required
def staff_list(franchisee_id):
    ...
    staff_users = (
        StaffUser.query.filter_by(franchisee_id=franchisee_id)
        .order_by(StaffUser.created_at.desc())
        .all()
    )

    return render_template("admin/staff_list.html", franchisee=franchisee, staff_users=staff_users)
...
def staff_add(franchisee_id):
    ...
    if request.method == "POST":
        try:
            ...
            role = request.form.get("role", "staf").strip()
            ...
            if phone:
                existing = StaffUser.query.filter_by(
                    franchisee_id=franchisee_id, phone=phone
                ).first()
            elif openid:
                existing = StaffUser.query.filter_by(
                    franchisee_id=franchisee_id, openid=openid
                ).first()
        ...
        try:
            ...
            if not phone and not openid:
                ...
                return render_template(
                    "admin/staff_edit.html", franchisee=franchisee, staff_user=staff_user
                )
