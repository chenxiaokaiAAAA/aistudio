...
def api_get_system_config():
    ...
    try:
        ...
        for config in ai_configs:
            ...
            # 图片压缩配置
            elif config.config_key == "image_compress_max_size_mb":
                configs["image_compress_max_size_mb"] = config.config_value
            elif config.config_key == "image_compress_max_dimension":
                configs["image_compress_max_dimension"] = config.config_value
            elif config.config_key == "image_compress_quality":
                configs["image_compress_quality"] = config.config_value
            elif config.config_key == "image_thumbnail_max_size":
                configs["image_thumbnail_max_size"] = config.config_value
            elif config.config_key == "image_thumbnail_quality":
                configs["image_thumbnail_quality"] = config.config_value
            elif config.config_key == "image_auto_compress":
                configs["image_auto_compress"] = config.config_value
            elif config.config_key == "image_auto_thumbnail":
                configs["image_auto_thumbnail"] = config.config_value
            # 图片上传配置
...
def api_save_comfyui_config():
    ...
    try:
        ...
        logger.info(f"✅ ComfyUI配置已更新: {data.get('comfyui_base_url')}")

        return jsonify({"status": "success", "message": "ComfyUI配置保存成功"})
    except Exception as e:
        db.session.rollback()
...
def api_save_server_url_config():
    ...
    try:
        ...
        logger.info("✅ 服务器URL配置已更新")

        return jsonify({"status": "success", "message": "服务器URL配置保存成功"})
    except Exception as e:
        db.session.rollback()
        return jsonify({"status": "error", "message": f"保存失败: {str(e)}"}), 500
...
def api_save_brand_name_config():
    ...
    try:
        ...
        if config:
            ...
            config = AIConfig(
                config_key="brand_name", config_value=brand_name, description="系统品牌名称"
            )
            db.session.add(config)

        db.session.commit()

        logger.info(f"✅ 品牌名称配置已更新: {brand_name}")

        return jsonify({"status": "success", "message": "品牌名称配置保存成功"})
    except Exception as e:
        db.session.rollback()
        return jsonify({"status": "error", "message": f"保存失败: {str(e)}"}), 500
...
def api_save_concurrency_config():
    ...
    try:
        ...
        logger.info(
            f"✅ 并发和队列配置已更新: ComfyUI={data.get('comfyui_max_concurrency')}, API={data.get('api_max_concurrency')}, 队列大小={data.get('task_queue_max_size')}, 工作线程={data.get('task_queue_workers')}"
        )

        return jsonify(
            {"status": "success", "message": "并发和队列配置保存成功（需要重启服务才能生效）"}
        )
    except Exception as e:
        db.session.rollback()
        return jsonify({"status": "error", "message": f"保存失败: {str(e)}"}), 500
...
def api_save_image_upload_config():
    ...
    try:
        ...
        configs_to_save = [
            (
                "image_upload_strategy",
                data.get("image_upload_strategy"),
                "图片上传策略：grsai=上传到GRSAI服务器，direct=直接使用云端URL",
            ),
            (
                "image_upload_environment",
                data.get("image_upload_environment"),
                "运行环境：local=本地测试环境，production=云端生产环境",
            ),
        ]
        ...
        logger.info(
            f"✅ 图片上传配置已更新: strategy={data.get('image_upload_strategy')}, environment={data.get('image_upload_environment')}"
        )

        return jsonify({"status": "success", "message": "图片上传配置保存成功"})
    except Exception as e:
        db.session.rollback()
        return jsonify({"status": "error", "message": f"保存失败: {str(e)}"}), 500
    ...
    try:
        ...
        configs_to_save = [
            ...
            (
                ...
                data.get("compress_max_size_mb"),
                "图片压缩触发大小（MB），超过此大小才压缩",
            ),
            (
                "image_compress_max_dimension",
                data.get("compress_max_dimension"),
                "图片压缩后最大尺寸（像素）",
            ),
            (
                "image_compress_quality",
                data.get("compress_quality"),
                "图片压缩质量（1-100，数值越大质量越好）",
            ),
            (
                "image_auto_compress",
                data.get("auto_compress", "true"),
                "是否自动压缩图片（true/false）",
            ),
            # 缩略图配置
            ("image_thumbnail_max_size", data.get("thumbnail_max_size"), "缩略图最大尺寸（像素）"),
            (
                "image_thumbnail_quality",
                data.get("thumbnail_quality"),
                "缩略图质量（1-100，数值越大质量越好）",
            ),
            (
                "image_auto_thumbnail",
                data.get("auto_thumbnail", "true"),
                "是否自动生成缩略图（true/false）",
            ),
        ]
        ...
        logger.info(f"✅ 图片路径配置已更新: 存储类型={data.get('storage_type', 'local')}")

        return jsonify(
            {"status": "success", "message": "图片路径配置保存成功（需要重启服务才能生效）"}
        )
    except Exception as e:
        db.session.rollback()
        return jsonify({"status": "error", "message": f"保存失败: {str(e)}"}), 500
...
def api_get_latest_orders():
    ...
    try:
        ...
        return jsonify({"success": True, "orders": order_list, "latest_time": latest_time})
    except Exception as e:
